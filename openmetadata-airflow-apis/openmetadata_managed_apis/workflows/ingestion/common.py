...
def build_dag_configs(ingestion_pipeline: IngestionPipeline) -> dict:
    ...
    return {
        ...
        "description": (
            ingestion_pipeline.description.__root__
            if ingestion_pipeline.description is not None
            else None
        ),
        "start_date": (
            ingestion_pipeline.airflowConfig.startDate.__root__
            if ingestion_pipeline.airflowConfig.startDate
            else airflow.utils.dates.days_ago(1)
        ),
        "end_date": (
            ingestion_pipeline.airflowConfig.endDate.__root__
        ...
        "dagrun_timeout": (
            timedelta(ingestion_pipeline.airflowConfig.workflowTimeout)
