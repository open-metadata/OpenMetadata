...
public final class PipelineShapeProfile implements EntityShapeProfile {
  ...
  public EntityInterface minimal(final ShapeContext ctx) {
    return new Pipeline()
        .withId(ctx.id())
        .withName("pipeline")
        .withFullyQualifiedName("shapeCanary.orchestration." + ctx.unique("pipeline"))
        .withTasks(List.of(new Task().withName("t0")));
  }

  @Override
  public List<PlannedCase> entitySpecificCases(final ShapeContext ctx) {
    return new EntityCases(entityType(), this::minimal, ctx)
        .add("tasks.count", Rung.of("1k", 1_000), this::tasks)
        .add("tasks.count", Rung.of("10k", 10_000), this::tasks)
        .add("tasks.count", Rung.of("50k", 50_000), this::tasks)
        .build();
  }
  ...
  private EntityInterface tasks(final EntityInterface entity, final Rung rung) {
    ...
    for (int i = 0; i < rung.magnitude(); i++) {
      tasks.add(new Task().withName("t_" + i));
    }
