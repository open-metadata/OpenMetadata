...
def get_create_service(entity: type[T], name: EntityName | None = None) -> C:
...
def get_create_user_entity(name: EntityName | None = None, email: str | None = None):
    if not name:
        name = generate_name().root
    if not email:
...
def get_create_team_entity(name: EntityName | None = None, users=list[str]):
    if not name:
        name = generate_name().root
