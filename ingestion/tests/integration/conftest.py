...
def pytest_runtest_setup(item):
    ...
    if (
        _last_package["name"] not in (None, package)
        and shutil.disk_usage("/").free < FREE_DISK_FLOOR_BYTES
    ):
...
def run_workflow():
    def _run(
        workflow_type: Type[IngestionWorkflow], config, raise_from_status=True
    ):  # noqa: UP006
    ...
    for attempt in range(retries):
        try:
            ...
            if _is_not_found(exc):
                logger.debug(
                    "Skipping %s %s delete — already gone", entity.__name__, entity_id
                )
                return
