...
@pytest.fixture
def load_metadata(
    run_workflow, ingestion_config, metadata, table_fqn
) -> Generator[MetadataWorkflow, None, None]:
    ...
    if table:
        metadata.delete(
            entity=Table, entity_id=table.id, recursive=True, hard_delete=True
        )
    ...
    recognizer = Recognizer(
        ...
        recognizerConfig=RecognizerConfig(
            root=PatternRecognizer(
                type="pattern",
                patterns=[
                    Pattern(name="address-column", regex=r"^address$", score=1.0)
                ],
...
def test_column_name_classification_when_sample_data_access_disabled(
    ...
    has_sample_data = (
        ...
        and sample_data.sampleData is not None
        and len(sample_data.sampleData.rows) > 0
    )
    assert (
        not has_sample_data
    ), "Expected no sample data when reading and storing are both disabled"
    ...
    matching_tags = [
        tag
        for tag in address_column.tags or []
        if tag.tagFQN.root == column_name_tag.fullyQualifiedName
    ]

    assert (
        len(matching_tags) == 1
    ), f"Expected the column-name tag on `address`, got {address_column.tags}"
