...
def nats_streams(nats_container):
    ...
    async def _create_streams():
        ...
        nc = await nats_client.connect(f"nats://{host}:{port}")
        try:
            js = nc.jetstream()
            await js.add_stream(
                name="crawler-jobs",
                subjects=["crawler.>"],
                max_age=86400,  # 24h in seconds; nats-py converts to ns internally
            )
...
def nats_service(metadata, nats_container, nats_streams):
    ...
    request = CreateMessagingServiceRequest(
        name=EntityName(root=f"docker_test_nats_{uuid.uuid4().hex[:8]}"),
        serviceType=MessagingServiceType.Nats,
        connection=MessagingConnection(
            config=NatsConnection(
                natsServers=f"nats://{host}:{port}",
            )
        ),
    )
