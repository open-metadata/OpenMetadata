...
def _column(name: str) -> Column:
    return Column(
        ...
        fullyQualifiedName=FullyQualifiedEntityName(
            root=f"service.database.schema.table.{name}"
        ),
    )
...
def _table(columns: list[Column]) -> Table:
    return Table(
        ...
        fullyQualifiedName=FullyQualifiedEntityName(
            root="service.database.schema.table"
        ),
        columns=columns,
    )
...
def _nested_columns(fqn_prefix: str) -> tuple[list[Column], list[Column]]:
    ...
    city = Column(
        ...
        dataType=DataType.STRING,
        fullyQualifiedName=FullyQualifiedEntityName(
            root=f"{fqn_prefix}.contact.address.city"
        ),
    )
    postal_code = Column(
        ...
        dataType=DataType.STRING,
        fullyQualifiedName=FullyQualifiedEntityName(
            root=f"{fqn_prefix}.contact.address.postal_code"
        ),
    )
    address = Column(
        ...
        fullyQualifiedName=FullyQualifiedEntityName(
            root=f"{fqn_prefix}.contact.address"
        ),
...
@pytest.mark.parametrize("entity_case", [_table_case, _container_case])
def test_metadata_fallback_supports_each_classifiable_entity(
    workflow_config, entity_case
):
    ...
    assert [column_tag.column_fqn for column_tag in result.column_tags] == [
        column.fullyQualifiedName.root
    ]
...
@pytest.mark.parametrize(
    "entity_case",
    [_nested_table_case, _nested_container_case],
)
def test_classifies_all_metadata_leaves_in_canonical_order(
    workflow_config, entity_case
):
...
def test_leaf_iteration_treats_depth_limit_as_leaf():
    ...
    leaves = list(
        RecordingClassificationProcessor._iter_leaf_columns(columns[0], max_depth=2)
    )
...
@pytest.mark.parametrize("has_literal_dotted_column", [False, True])
def test_sampled_dotted_name_only_matches_top_level_column(
    workflow_config, has_literal_dotted_column
):
    ...
    parent = Column(
        ...
        dataType=DataType.RECORD,
        fullyQualifiedName=FullyQualifiedEntityName(
            root="service.database.schema.table.contact"
        ),
        children=[nested_email],
    )
    literal_column = _column("contact.email")
    columns = [parent, literal_column] if has_literal_dotted_column else [parent]
    record = SamplerResponse(
        entity=_table(columns),
        sample_data=SampleData(
            data=TableData(
    ...
    expected_inputs = (
        [(literal_column, ["first@example.com", "second@example.com"])]
        if has_literal_dotted_column
        else []
    )
    assert processor.classifier_inputs == expected_inputs
    assert [tag.column_fqn for tag in result.column_tags] == (
        [literal_column.fullyQualifiedName.root] if has_literal_dotted_column else []
    )
