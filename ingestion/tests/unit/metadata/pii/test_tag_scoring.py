...
class TestTagScorer:
    ...
    def test_score_cutoff_filtering(self, tag_analyzers):
        ...
        scored_tags = high_cutoff_classifier.predict_scores(
            sample_data, run_column_analysis=True
        )
...
class TestTagAnalyzer:
    ...
    def test_analyze_content_rejects_epoch_timestamp_as_date(
        self, date_tag_analyzer: TagAnalyzer
    ):
    ...
    def test_analyze_content_preserves_textual_date(
        self, date_tag_analyzer: TagAnalyzer
    ):
        analysis = date_tag_analyzer.analyze(str_values=["2025-01-15"])

        assert analysis.score > 0.0
        assert [result.entity_type for result in analysis.recognizer_results] == [
