...
class MockEntity(BaseModel):
    ...
    tableConstraints: list[TableConstraint] | None = None  # noqa: N815
...
class TableConstraintsHandlerTest(TestCase):
    ...
    def test_source_empty_destination_with_constraints(self):
        ...
        destination = MockEntity(
            tableConstraints=[TableConstraint(constraintType=ConstraintType.PRIMARY_KEY, columns=["id"])]
        )
        ...
        self.assertEqual(destination.tableConstraints[0].constraintType, ConstraintType.PRIMARY_KEY)
    ...
    def test_preserve_constraint_order_from_source(self):
        ...
        self.assertEqual(destination.tableConstraints[0].constraintType, ConstraintType.PRIMARY_KEY)
        self.assertEqual(destination.tableConstraints[0].columns, ["id"])
        self.assertEqual(destination.tableConstraints[1].constraintType, ConstraintType.UNIQUE)
    ...
    def test_multiple_columns_in_constraints(self):
        ...
        self.assertEqual(len(destination.tableConstraints), 1)
        self.assertEqual(destination.tableConstraints[0].constraintType, ConstraintType.UNIQUE)
        # Column order in destination should be preserved
        self.assertEqual(destination.tableConstraints[0].columns, ["last_name", "first_name"])
    ...
    def test_complex_constraint_rearrangement(self):
        ...
        source = MockEntity(
            tableConstraints=[
                TableConstraint(constraintType=ConstraintType.PRIMARY_KEY, columns=["id"]),
                TableConstraint(constraintType=ConstraintType.FOREIGN_KEY, columns=["department_id"]),
        ...
        self.assertEqual(destination.tableConstraints[2].constraintType, ConstraintType.UNIQUE)
    ...
    def test_same_constraint_type_different_columns(self):
        ...
        source = MockEntity(
            tableConstraints=[
                TableConstraint(constraintType=ConstraintType.UNIQUE, columns=["email"]),
                TableConstraint(constraintType=ConstraintType.UNIQUE, columns=["username"]),
            ]
        )
