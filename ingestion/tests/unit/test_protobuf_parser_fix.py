...
sys.modules.update(
    {
        ...
        "metadata.generated.schema.entity": types.ModuleType(
            "metadata.generated.schema.entity"
        ),
        "metadata.generated.schema.entity.data": types.ModuleType(
            "metadata.generated.schema.entity.data"
        ),
        "metadata.generated.schema.entity.data.table": table_mod,
        "metadata.generated.schema.type": types.ModuleType(
...
class TestResolveMessageClass:
    ...
    def test_non_message_attribute_not_mistakenly_returned(self):
        ...
        real_cls = type(
            "MyLoanRecord", (Message,), {"DESCRIPTOR": MagicMock(fields=[])}
        )
    ...
    def test_descriptor_fallback_used_when_name_mismatch(self):
        mod, _ = _make_pb2_module(["MyLoanRecord"])
        instance = _resolve_message_class(mod, "loans")
        assert instance is not None
        assert instance.__class__.__name__ == "MyLoanRecord"
...
class TestProtobufDataTypes:
    ...
    def test_int_aliases_all_resolve_to_INT(self):
        ...
        for val in [3, 4, 5, 13, 17, 18]:
            assert (
                ProtobufDataTypes(val) == ProtobufDataTypes.INT
    ...
    def test_fixed_aliases_all_resolve_to_FIXED(self):
        ...
        for val in [6, 7, 15, 16]:
            assert (
                ProtobufDataTypes(val) == ProtobufDataTypes.FIXED
...
class TestGetProtobufFields:
    ...
    def test_multiple_scalar_fields(self):
        ...
        assert len(result) == 3
        assert result[0].name == "amount"
        assert result[1].name == "rate"
        assert result[2].name == "label"
    ...
    def test_nested_record_field_recurses(self):
        ...
        result = self.parser.get_protobuf_fields([address_field])
        assert len(result) == 1
        assert result[0].name == "address"
        assert result[0].children is not None
        assert len(result[0].children) == 1
        assert result[0].children[0].name == "zip_code"
...
class TestIssue15274Regression:
    ...
    def test_topic_address_book_message_AddressBook_still_works(self):
        ...
        assert (
            result is not None
        ), "Backward compat regression: address_book/AddressBook broke."
        assert isinstance(result, classes["AddressBook"])
