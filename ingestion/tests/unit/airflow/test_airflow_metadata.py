...
class TestPipelineObservabilityScope:
    ...
    def test_emits_only_the_current_dag_tables(self):
        ...
        source = self._source_with_context(
            self._pipeline_entity("service.dag_a"), ["svc.db.sch.table_a"], run
        )

        first = list(
            source.get_table_pipeline_observability(self._dag_details("dag_a"))
        )
        ...
        second = list(
            source.get_table_pipeline_observability(self._dag_details("dag_b"))
        )
    ...
    def test_emits_single_record_built_from_the_newest_run(self):
        ...
        """
        newest = self._dag_run(
            "run_new", datetime(2024, 3, 1, tzinfo=timezone.utc), state="success"
        )
        source = self._source_with_context(
            self._pipeline_entity(), ["svc.db.sch.table_a"], newest
        )
        ...
        assert observability_list[0].lastRunTime.root == int(
            newest.logical_date.timestamp() * 1000
        )
    ...
    def test_emits_nothing_when_lineage_context_is_incomplete(
        self, pipeline_entity, table_fqns, latest_dag_run
    ):
        """
        ...
        """
        run = (
            self._dag_run("run_a", datetime(2024, 1, 15, tzinfo=timezone.utc))
            if latest_dag_run
            else None
        )
...
class TestLineageObservabilityContext:
    ...
    def test_hands_resolved_table_entities_to_the_observability_stage(self):
        ...
        assert set(ctx.current_table_fqns) == {
            "svc.db.sch.in_table",
            "svc.db.sch.out_table",
        }
