...
from metadata.data_insight.processor.entity_report_data_processor import EntityReportDataProcessor
...
class EntityReportProcessorTest(unittest.TestCase):
    ...
    def test__flatten_results(self, mocked_om):
        ...
        for flat_result in EntityReportDataProcessor(mocked_om)._flatten_results(data):
            ...
            assert all(
                k in flat_result.data.dict() for k in [
                    "entityType", "entityTier", "team", "organization", "completedDescriptions", "missingDescriptions", "hasOwner", "missingOwner"
                    ]
                )
