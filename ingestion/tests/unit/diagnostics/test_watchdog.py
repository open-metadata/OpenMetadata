...
def _make_handler_with_hung_op(started_seconds_ago: float):
    ...
    tid = threading.get_ident()
    registry._stacks[tid] = [
        ("source.iter", {"entity": "t"}, _time.monotonic() - started_seconds_ago, 1)
    ]
    aspects = [
        registry,
        HttpTracker(),
...
def test_handler_run_watchdog_logs_warn_and_dumps_on_hang(
...
def test_handler_run_watchdog_logs_warn_without_dump_on_stuck(
    caplog: pytest.LogCaptureFixture,
) -> None:
