...
"""Dump output format and routing (logger record vs signal-safe stderr)."""
...
def _renderables(registry=None, http=None, memory=None):
    return [registry or OperationRegistry(), http or HttpTracker(), memory or MemoryTracker()]


def _capture_logger_full(renderables, workflow=None, *, caplog: pytest.LogCaptureFixture) -> str:
    with caplog.at_level(logging.WARNING, logger="metadata.Diagnostics"):
        emit_full_dump("unit-test", renderables, workflow, signal_safe=False)
    return "\n".join(r.getMessage() for r in caplog.records)


def _capture_stderr_full(renderables, workflow=None) -> str:
    stderr = io.StringIO()
    with patch.object(sys, "stderr", stderr):
        emit_full_dump("unit-test", renderables, workflow, signal_safe=True)
    return stderr.getvalue()


def test_full_dump_has_section_markers(caplog):
    out = _capture_logger_full(_renderables(), caplog=caplog)
...
def test_full_dump_is_one_log_record_for_shipping(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="metadata.Diagnostics"):
        emit_full_dump("unit", _renderables(), None, signal_safe=False)
...
def test_full_dump_lists_active_ops(caplog):
    ...
    out = _capture_logger_full(_renderables(registry=registry), caplog=caplog)
...
def test_full_dump_lists_inflight_http(caplog):
    ...
    with http.request("PUT", "/api/v1/tables"):
        out = _capture_logger_full(_renderables(http=http), caplog=caplog)
...
def test_incremental_dump_excludes_thread_section(caplog):
    with caplog.at_level(logging.INFO, logger="metadata.Diagnostics"):
        emit_incremental_dump(_renderables(), signal_safe=False)
    out = "\n".join(r.getMessage() for r in caplog.records)
    assert "diag.dump.threads" not in out  # incremental skips the expensive thread tracebacks
...
def test_full_dump_renders_workflow_step_summary(caplog):
    ...
    out = _capture_logger_full(_renderables(), workflow=_Workflow(), caplog=caplog)
...
def test_signal_safe_full_dump_writes_to_stderr():
    out = _capture_stderr_full(_renderables())
    for marker in ("diag.dump.begin", "diag.dump.ops", "diag.dump.http", "diag.dump.memory", "diag.dump.end"):
        assert marker in out
...
def test_signal_safe_path_does_not_emit_log_records(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="metadata.Diagnostics"):
        _capture_stderr_full(_renderables())
