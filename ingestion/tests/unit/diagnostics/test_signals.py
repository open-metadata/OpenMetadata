...
def _renderables(registry=None, http=None, memory=None):
    return [
        registry or OperationRegistry(),
        http or HttpTracker(),
        memory or MemoryTracker(),
    ]


def _capture_logger_full(
    renderables, workflow=None, *, caplog: pytest.LogCaptureFixture
) -> str:
...
def test_incremental_dump_excludes_thread_section(caplog):
    ...
    out = "\n".join(r.getMessage() for r in caplog.records)
    assert (
        "diag.dump.threads" not in out
    )  # incremental skips the expensive thread tracebacks
