...
def test_multithread_main_idle_worker_active_counts_as_active():
    ...
    registry._stacks[threading.get_ident()] = [
        ("workflow.execute", {}, time.monotonic(), 1)
    ]
    # Worker thread on snowflake.query
    fake_worker_tid = threading.get_ident() + 1
    registry._stacks[fake_worker_tid] = [
        ("snowflake.query", {"sql": "x"}, time.monotonic(), 2)
    ]
...
class _FakeFrame:
    def __init__(
        self, filename: str, qualname: str, back: "_FakeFrame | None" = None
    ) -> None:
...
def _frame(filename: str, qualname: str, back: FrameType | None = None) -> FrameType:
    ...
    return cast(
        "FrameType", _FakeFrame(filename, qualname, cast("_FakeFrame | None", back))
    )
...
def test_method_sampling_caps_distinct_methods_per_op(monkeypatch):
    ...
    for index in range(5):
        sampler.sample(
            0.1, {tid: _frame(f"/env/src/metadata/m{index}.py", f"fn{index}")}
        )
    db_methods = dict(sampler.snapshot()["by_op"]["db"])
...
def test_db_method_attributed_to_project_frame_over_library():
    ...
    registry.push("snowflake.query", {})
    om_frame = _frame(
        "/env/src/metadata/ingestion/source/database/common_db_source.py",
        "CommonDbSource.get_columns",
    )
    library_leaf = _frame(
        "/usr/lib/python3.11/socket.py", "SocketIO.readinto", back=om_frame
    )
    sampler.sample(0.2, {tid: library_leaf})
    db_methods = dict(sampler.snapshot()["by_op"]["db"])
    assert (
        "metadata/ingestion/source/database/common_db_source.py:CommonDbSource.get_columns"
        in db_methods
    )
...
def test_db_method_falls_back_to_deepest_frame_when_no_project_frame():
    ...
    registry.push("snowflake.query", {})
    sampler.sample(
        0.2, {tid: _frame("/usr/lib/python3.11/socket.py", "SocketIO.readinto")}
    )
    db_methods = dict(sampler.snapshot()["by_op"]["db"])
...
def test_ometa_http_keyed_by_endpoint_not_frame():
    ...
    registry.push(
        ...
        {
            "method": "put",
            "url": "http://localhost:8585/api/v1/tables/bulk?async=false",
        },
    )
    chokepoint = _frame(
        "/env/src/metadata/ingestion/ometa/client.py", "REST._one_request"
    )
