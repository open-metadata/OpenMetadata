...
class OpenLineageUnitTest(unittest.TestCase):
    ...
    def test_build_ol_name_to_fqn_map_aliases_symlink_identities(
        self, mock_get_table_fqn
    ):
        ...
        mock_get_table_fqn.side_effect = (
            lambda table_details, namespace=None: f"svc.db.schema.{table_details.name}"
        )
    ...
    def test_get_column_lineage_skips_when_input_unresolved(
        ...
        on the input side."""
        mock_get_table_fqn.side_effect = (
            lambda table_details, namespace=None: f"svc.schema.{table_details.name}"
        )
        # Only the output resolves; the input is intentionally absent from the map.
        mock_build_map.return_value = {
            "hive:///schema.output_table": "svc.schema.output_table"
        }

        inputs = [
            {"name": "schema.missing_input", "facets": {}, "namespace": "hive://"}
        ]
        outputs = [
            {
                "name": "schema.output_table",
    ...
    def test_get_column_lineage_tolerates_null_facets_at_any_level(self):
        ...
        outputs_null_facets = [
            {"name": "schema.t", "facets": None, "namespace": "hive://"}
        ]
        outputs_null_column_lineage = [
            {
                "name": "schema.t",
                "facets": {"columnLineage": None},
                "namespace": "hive://",
            },
        ]
        outputs_null_fields = [
        ...
        with patch.object(
            ...
            for outputs in (
                ...
                self.assertEqual(
                    self.open_lineage_source._get_column_lineage([], outputs), {}
                )
    ...
    def test_get_column_lineage_tolerates_non_dict_facets_at_any_level(self):
        ...
        outputs_facets_list = [
            {"name": "schema.t", "facets": [], "namespace": "hive://"}
        ]
    ...
    def test_get_entity_details_tolerates_null_namespace(self):
        ...
        result = OpenlineageSource._get_entity_details(
            {"namespace": None, "name": "schema.t"}
        )
    ...
    def test_iter_table_candidates_with_symlinks(self):
        ...
        data = {
            "facets": {
                "symlinks": {
                    "identifiers": [{"name": "project.schema.table", "type": "TABLE"}]
                }
            }
        }
    ...
    def test_iter_table_candidates_normalizes_caps_symlinks_to_lowercase(self):
        ...
        data = {
            "facets": {
                "symlinks": {
                    "identifiers": [
                        {"name": "PROJECT.SCHEMA.CASE_TEST_SOURCE", "type": "TABLE"}
                    ]
                }
            }
        }
    ...
    def test_iter_table_candidates_unparseable_name_is_empty(self):
        ...
        self.assertEqual(
            OpenlineageSource._iter_table_candidates({"name": "invalidname"}), []
        )
            ...
            "facets": {
                "symlinks": {
                    "identifiers": [
                        {
                            "namespace": "s3://bucket",
                            "name": "warehouse/analytics/daily_revenue",
                            "type": "LOCATION",
                        }
                    ]
                }
    ...
    def test_iter_table_candidates_keeps_same_table_under_different_namespaces(self):
        ...
        self.assertEqual(
            {ns for _, ns in candidates}, {"arn:aws:glue:r:a", "trino://host:8080"}
        )
    ...
    def test_symlink_identifiers_tolerates_malformed_facets(self):
        ...
        self.assertEqual(
            OpenlineageSource._symlink_identifiers({"facets": {"symlinks": None}}), []
        )
        self.assertEqual(
            OpenlineageSource._symlink_identifiers(
                {"facets": {"symlinks": {"identifiers": {}}}}
            ),
            [],
        )
    ...
    def test_iter_table_candidates_tolerates_null_namespace_and_name(self):
        ...
        symlink_null_ns = {
            "facets": {
                "symlinks": {
                    "identifiers": [
                        {"namespace": None, "name": "schema.tbl", "type": "TABLE"}
                    ]
                }
            }
        }
        ...
        symlink_null_name = {
            "facets": {
                "symlinks": {
                    "identifiers": [
                        {"namespace": "arn:aws:glue:r:a", "name": None, "type": "TABLE"}
                    ]
                }
            }
        }
        self.assertEqual(
            OpenlineageSource._iter_table_candidates(symlink_null_name), []
        )
    ...
    def test_resolve_table_returns_none_when_nothing_resolves(self):
        ...
        with patch.object(
            self.open_lineage_source, "_get_table_fqn", return_value=None
        ):
    ...
    def test_resolve_table_memoizes_within_event(self):
        ...
        sentinel = ResolvedTable(
            fqn="svc.schema.tbl", details=TableDetails(name="tbl", schema="schema")
        )
        with patch.object(
            self.open_lineage_source, "_resolve_table_uncached", return_value=sentinel
        ) as mock_uncached:
    ...
    def test_resolve_table_memoizes_negative_result(self):
        ...
        with patch.object(
            self.open_lineage_source, "_resolve_table_uncached", return_value=None
        ) as mock_uncached:
    ...
    def test_resolve_table_cache_distinguishes_symlink_only_datasets(self):
        ...
        def fake_resolution(data, ol_name):
            ...
            return ResolvedTable(
                fqn=f"svc.schema.{name}",
                details=TableDetails(name=name, schema="schema"),
            )

        with patch.object(
            self.open_lineage_source,
            "_resolve_table_uncached",
    ...
    def test_yield_pipeline_lineage_details_resets_resolution_cache(self):
        ...
        with patch.object(
            OpenMetadataConnection, "get_by_name", create=True, return_value=None
        ):
    ...
    def test_yield_pipeline_lineage_details_glue_symlink_datasets(
        ...
        def mock_get_by_name(entity, fqn):
            ...
            return Mock(
                id=Mock(root=ids.get(fqn, "33333333-3333-3333-3333-333333333333"))
            )
        ...
        self.assertEqual(
            len(lineage),
            1,
            "Glue-symlink datasets must produce exactly one lineage edge",
        )
        edge = lineage[0].edge
    ...
    def test_table_found_in_multiple_services_raises_ambiguous(self):
        ...
        with patch(
            "metadata.utils.fqn.build", side_effect=mock_fqn_build
        ):  # noqa: SIM117
    ...
    def test_yield_pipeline_lineage_skips_when_pipeline_fqn_unbuildable(self):
        ...
        ol_event = OpenLineageEvent(
            ...
            event_type="COMPLETE",
            inputs=[
                {"name": "schema.in", "namespace": "postgres://h:5432", "facets": {}}
            ],
            outputs=[
                {"name": "schema.out", "namespace": "postgres://h:5432", "facets": {}}
            ],
        )
        ...
        self.assertEqual(
            [r for r in results if r.right and isinstance(r.right, AddLineageRequest)],
            [],
        )
        ...
        for call in mock_metadata.get_by_name.call_args_list:
            self.assertIsNotNone(
                call.kwargs.get("fqn", call.args[1] if len(call.args) > 1 else "ok")
            )
    ...
    def test_parse_table_identity_glue_namespace_uses_glue_parser(self):
        ...
        result = OpenlineageSource._parse_table_identity(
            "arn:aws:glue:us-east-1:123456789012", "table/sales/users"
        )
        self.assertEqual(result.schema, "sales")
        self.assertEqual(result.name, "users")
    ...
    def test_parse_table_identity_falls_back_to_dotted_for_nonconformant_name(self):
        ...
        result = OpenlineageSource._parse_table_identity(
            "arn:aws:glue:us-east-1:123456789012", "sales.users"
        )
        self.assertIsNotNone(result)
        self.assertEqual(result.schema, "sales")
        self.assertEqual(result.name, "users")
    ...
    def test_get_ol_table_name_preserves_uri_scheme(self):
        ...
        leading_slash = OpenlineageSource._get_ol_table_name(
            {"namespace": "s3://bucket", "name": "/path/obj"}
        )
        no_leading_slash = OpenlineageSource._get_ol_table_name(
            {"namespace": "s3://bucket", "name": "path/obj"}
        )
    ...
    def test_entity_details_glue_namespace_classified_as_table(self):
        ...
        self.assertEqual(
            OpenlineageSource._get_entity_details(data).entity_type, "table"
        )
        details, _ = OpenlineageSource._iter_table_candidates(data)[0]
        self.assertEqual(details.name, "users")
        self.assertEqual(details.schema, "sales")
...
class TestKinesisMultiShardPolling(unittest.TestCase):
    ...
    def test_poll_kinesis_reads_events_from_every_shard(self, _mock_sleep):
        ...
        client.get_shard_iterator.side_effect = lambda **kw: {
            "ShardIterator": f"{kw['ShardId']}-start"
        }
