...
class TestFivetranLineage:
    ...
    def test_falls_back_to_cross_service_search_when_unconfigured(
        self, mock_get_services, fivetran_source
    ):
        source, _ = fivetran_source
        ...
        with patch.object(source, "metadata") as mock_metadata:
            ...
            result = source._resolve_table(
                table_name="orders", schema_name="crm", database_name=None
            )
    ...
    def test_configured_services_do_not_use_fallback(
        self, mock_get_services, fivetran_source
    ):
        source, _ = fivetran_source
        mock_get_services.return_value = ["pg"]
        ...
        with patch.object(source, "metadata") as mock_metadata:
            ...
            result = source._resolve_table(
                table_name="orders", schema_name="crm", database_name="db"
            )
    ...
    def test_configured_but_not_found_still_skips(
        self, mock_get_services, fivetran_source
    ):
        source, _ = fivetran_source
        mock_get_services.return_value = ["pg"]
    ...
    def test_topic_search_string_quotes_dotted_name(
        self, mock_get_services, fivetran_source
    ):
        source, _ = fivetran_source
        mock_get_services.return_value = []
    ...
    @patch("metadata.utils.fqn.build")
    def test_table_to_topic_lineage(
        self, mock_build, mock_db, mock_msg, fivetran_source
    ):
        source, client = fivetran_source
            ...
            client.get_connector_schema_details.return_value = {
                "hr": {
                    ...
                    "tables": {
                        "employees": {
                            "enabled": True,
                            "name_in_destination": "employee_events",
                        }
                    },
                }
            }
    ...
    def test_warns_when_lineage_enabled_but_no_edges(
        self, mock_get_services, fivetran_source, caplog
    ):
        source, client = fivetran_source
        mock_get_services.return_value = ["pg"]
        client.get_connector_schema_details.return_value = {
    ...
    def test_no_warning_when_only_fivetran_side_disables_skipped(
        self, mock_get_services, fivetran_source, caplog
    ):
    ...
    def test_warning_names_actionable_reason_over_disabled(
        self, mock_get_services, fivetran_source, caplog
    ):
    ...
    @patch("metadata.utils.fqn.build")
    def test_no_warning_when_edges_produced_despite_skips(
        self, mock_build, mock_get_services, fivetran_source, caplog
    ):
    ...
    def test_entity_edge_survives_unresolvable_column(
        ...
        mock_col_fqn.return_value = (
            None  # unresolvable -> _fetch_column_lineage yields []
        )
        ...
        with patch.object(source, "metadata") as mock_metadata:
            ...
            client.get_connector_column_lineage.return_value = {
                "id": {"enabled": True, "name_in_destination": "id"}
            }
            result = list(
                source.yield_pipeline_lineage_details(EXPECTED_FIVETRAN_DETAILS)
            )
...
class TestSearchAnyService:
    ...
    def test_padding_contract_matches_real_helper(self, fivetran_source):
        source, _ = fivetran_source
        search_string = self._capture_search_string(
            source, Table, [None, "crm", "orders"]
        )
        assert (
            prefix_entity_for_wildcard_search(Table, search_string) == "*.*.crm.orders"
        )
...
class TestFivetranColumnLineage:
    ...
    @patch("metadata.ingestion.source.pipeline.fivetran.metadata.get_topic_field_fqn")
    @patch("metadata.ingestion.source.pipeline.fivetran.metadata.get_column_fqn")
    def test_table_to_topic_uses_topic_field_resolver(
        self, mock_col_fqn, mock_topic_fqn, fivetran_source
    ):
        source, client = fivetran_source
        client.get_connector_column_lineage.return_value = {
            "src": {"enabled": True, "name_in_destination": "dst"}
        ...
        result = source._fetch_column_lineage(
            EXPECTED_FIVETRAN_DETAILS, "test", "public", "users", Mock(), topic
        )
    ...
    @patch("metadata.ingestion.source.pipeline.fivetran.metadata.get_topic_field_fqn")
    @patch("metadata.ingestion.source.pipeline.fivetran.metadata.get_column_fqn")
    def test_unresolvable_topic_field_is_skipped(
        self, mock_col_fqn, mock_topic_fqn, fivetran_source
    ):
        source, client = fivetran_source
        client.get_connector_column_lineage.return_value = {
