...
class TestKafkaConnectTopicRoutingTransforms(TestCase):
    ...
    def test_apply_regex_router_rewrites_topic_name(self):
        ...
        result = apply_topic_routing_transforms(
            "ecommerce.sales.prod.sales.outbox", config
        )
    ...
    def test_resolve_outbox_topics_matches_ingested_topic(self):
        ...
        topics = source._resolve_outbox_topics(
            connector_config=config, messaging_service_name="Kafka"
        )
    ...
    def test_resolve_outbox_topics_no_messaging_service_returns_empty(self):
        ...
        topics = source._resolve_outbox_topics(
            connector_config=config, messaging_service_name=None
        )
    ...
    def test_apply_regex_router_named_group(self):
        ...
        assert (
            apply_topic_routing_transforms("outbox.event.orderCreated", config)
            == "prod.global.orderCreated_v1"
        )
    ...
    def test_outbox_fanout_identifies_outbox_table_by_event_columns(self):
        ...
        assert (
            source._is_outbox_fanout(pipeline, outbox, topics, single_dataset=False)
            is True
        )
        assert (
            source._is_outbox_fanout(pipeline, orders, topics, single_dataset=False)
            is False
        )
        # Single-table connector is unambiguously the outbox.
        assert (
            source._is_outbox_fanout(pipeline, orders, topics, single_dataset=True)
            is True
        )
...
class TestKafkaConnectTransformLineageEdges(TestCase):
    ...
    def _run_lineage(self, config, ingested_topic_names, pipeline_topics=None):
        ...
        source.metadata.list_all_entities.return_value = [
            self._ingested_topic(name) for name in ingested_topic_names
        ]
