...
def test_find_view_traverses_nested_includes():
    ...
    assert (
        parser.find_view(view_name=ViewName("cats"), path=Includes("root.lkml"))
        is not None
    )
...
def test_wildcard_include_expands_from_the_file_tree():
    ...
    assert (
        parser.find_view(view_name=ViewName("dogs"), path=Includes("cats.explore.lkml"))
        is not None
    )
...
def test_external_includes_are_never_read():
    ...
    reader = InMemoryReader(
        {"cats.explore.lkml": 'include: "//other-project/cats.view.lkml"\n'}
    )
...
def test_missing_view_returns_none():
    ...
    assert (
        parser.find_view(
            view_name=ViewName("hamsters"), path=Includes("cats.explore.lkml")
        )
        is None
    )
