...
EXPLORE_CONDITIONAL_TABLE = _conditional_table(
    "_explore._name == 'table_details_explore'"
)
...
def _stubbed_db_services(
    ...
    with (
        ...
        patch.object(
            source, "parse_db_service_prefix", side_effect=lambda prefix: (prefix,)
        ),
        patch.object(source, "_get_db_dialect", return_value=Dialect.ANSI),
        patch.object(
            source, "build_lineage_request", return_value=lineage_request
...
def test_column_lineage_is_extracted_once_per_view() -> None:
    ...
    with (
        patch.object(
            source, "_extract_column_lineage", return_value=[]
        ) as extract_column_lineage,
...
def test_model_liquid_context_selects_source_table_for_standalone_view_lineage() -> None:
    ...
    with _stubbed_db_services(source, lineage_request) as build_lineage_request:
        assert list(
            source._add_standalone_view_lineage(
                view, "finance_project", "finance_reports"
            )
        ) == [lineage_request]
