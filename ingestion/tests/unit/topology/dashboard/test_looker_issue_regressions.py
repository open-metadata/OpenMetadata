...
MODEL_CONDITIONAL_TABLE = """
{% if _model._name == 'finance_reports' %}
...
def test_model_liquid_context_selects_source_table_for_view_lineage() -> None:
    source = object.__new__(LookerSource)
    ...
    explore = SimpleNamespace(model_name="finance_reports", name="table_details")
    lineage_request = MagicMock()

    with (
        patch.object(source, "get_db_service_prefixes", return_value=["trino_service"]),
        patch.object(source, "parse_db_service_prefix", return_value=("trino_service",)),
