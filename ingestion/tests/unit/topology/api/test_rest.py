...
# Canned OpenAPI document fed to the source instead of fetching the live petstore
# URL declared in mock_rest_config, so the collection/endpoint ordering tests stay
...
def build_large_openapi_schema() -> dict:
    return {
        "tags": [
            {"name": f"collection_{index}"} for index in range(LARGE_SCHEMA_COLLECTIONS)
        ],
        "paths": {
...
class RESTTest(TestCase):
    ...
    def _hermetic_source(self, config=None, schema=None):
        """Build a RestSource that reads a canned document instead of the live URL.

        ``RestSource._get_openapi_schema`` uses a dict connection as the already
        parsed document, so assigning one keeps these tests off the network. The
        OM client is stubbed too: driving the topology makes the runner look each
        entity up on the server before sinking it.
        """
        with patch(
            ...
            source = RestSource.create(
                (config or mock_rest_config)["source"],
                self.config.workflowConfig.openMetadataServerConfig,
            )
        source.metadata = MagicMock()
        source.metadata.get_by_name.return_value = None
        source.metadata.config.forceEntityOverwriting = False
        source.connection = deepcopy(MOCK_OPENAPI_SCHEMA) if schema is None else schema
    ...
    def test_path_parameters_are_not_ingested_as_operations(self):
        ...
        records = [
            ...
            for record in self.rest_source.yield_api_endpoint(collection)
        ]
        ...
        assert {
            (endpoint.name.root, endpoint.requestMethod, endpoint.apiCollection.root)
    ...
    def test_operations_on_one_path_use_their_own_collections(self):
        ...
        assert all(record.left is None for record in records)
        assert {
            (endpoint.requestMethod, endpoint.apiCollection.root)
            for endpoint in endpoints
        } == {
    ...
    def test_get_api_collections_on_unparseable_schema(self):
        ...
        """
        source = self._hermetic_source(schema=object())
    ...
    def test_yield_api_collection_returns_error(self):
        with patch.object(
            ...
            result = list(
                self.rest_source.yield_api_collection(deepcopy(MOCK_COLLECTIONS[0]))
            )
    ...
    def test_api_collections_flush_before_api_endpoints(self):
        ...
        source = self._hermetic_source()
        records = list(
            source.process_nodes(
                [source.topology.api_collection, source.topology.api_endpoint]
            )
        )
        ...
        collection_count = len(
            [entry for entry in right_types if entry is CreateAPICollectionRequest]
        )

        assert all(record.left is None for record in records)
        assert (
            collection_count == len(MOCK_COLLECTIONS) + 1
        )  # + the "default" collection
        assert (
            right_types[:barrier_index]
            == [CreateAPICollectionRequest] * collection_count
        )
    ...
    def test_large_document_never_sends_an_endpoint_before_its_collection(self):
        ...
        for url, payloads in put_calls:
            if "/apiCollections/bulk" in url:
                created_collections.update(
                    f"{payload['service']}.{payload['name']}" for payload in payloads
                )
            elif "/apiEndpoints/bulk" in url:
                orphan_endpoints.extend(
                    payload["name"]
                    for payload in payloads
                    if payload["apiCollection"] not in created_collections
                )

        put_sequence = [
            "C" if "/apiCollections/bulk" in url else "E" for url, _ in put_calls
        ]
    ...
    def test_get_api_collections_derives_once(self):
        ...
        source = self._hermetic_source(config=exclude_config)

        with patch.object(
            RestSource, "_derive_collections", wraps=source._derive_collections
        ) as derive:
        ...
        assert [collection.name.root for collection in first_pass] == [
            "pet",
            "user",
            "default",
        ]
    ...
    def test_one_malformed_tag_does_not_drop_the_document(self):
        ...
        """
        bad_tags = [
            ("name-over-entity-name-max-length", {"name": "x" * 300}, 1),
            (
                "non-string-description",
                {"name": "bad", "description": {"nested": "object"}},
                1,
            ),
            ("tag-entry-is-a-string-not-an-object", "bad", 0),
        ]
        for case_id, bad_tag, expected_failures in bad_tags:
            with self.subTest(case_id):
                schema = deepcopy(MOCK_OPENAPI_SCHEMA)
                schema["tags"].insert(1, bad_tag)
                source = self._hermetic_source(schema=schema)

                collection_names = [
                    collection.name.root for collection in source.get_api_collections()
                ]
    ...
    def test_url_helpers_use_fallbacks(self):
        ...
        # mock_rest_config points at an openAPISchemaURL, so the fallback is that URL
        fallback_url = self.rest_source._get_fallback_url()
        assert fallback_url == AnyUrl(
            "https://petstore3.swagger.io/api/v3/openapi.json"
        )

        connection_config.docURL = AnyUrl("https://example.com/#")
        assert (
            str(self.rest_source._generate_collection_url("store"))
            == "https://example.com/#/store"
        )

        connection_config.docURL = None
        assert self.rest_source._generate_collection_url("store") == fallback_url

        collection = deepcopy(MOCK_SINGLE_COLLECTION)
        collection.url = None
        assert (
            self.rest_source._generate_endpoint_url(collection, MOCK_SINGLE_ENDPOINT)
            == fallback_url
        )
    ...
    def test_url_helpers_recover_from_errors(self):
        ...
        with patch(
            ...
            assert (
                self.rest_source._generate_endpoint_url(
    ...
    def test_endpoint_helpers_handle_invalid_input(self):
        ...
        assert (
            self.rest_source._filter_collection_endpoints(MOCK_SINGLE_COLLECTION)
            is None
        )
        assert (
            self.rest_source._prepare_endpoint_data(
                "/store", "get", None, MOCK_SINGLE_COLLECTION
            )
            is None
        )
    ...
    def test_convert_parameter_to_field_uses_top_level_type_with_non_object_schema(
        self,
    ):
        for schema in (None, "invalid"):
    ...
    def test_resolve_parameter_ref_variants(self):
        ...
        self.rest_source.json_response = {"parameters": {"Limit": parameter}}
        assert (
            self.rest_source._resolve_parameter_ref("#/parameters/Limit") == parameter
        )
    ...
    def test_parse_openapi_type_empty_values(self):
        for openapi_type in ([], ""):
            with self.subTest(openapi_type=openapi_type):
                assert (
                    self.rest_source._parse_openapi_type(openapi_type)
                    == DataTypeTopic.UNKNOWN
                )
    ...
    def test_process_inline_schema_expands_untyped_object_properties(self):
        ...
        assert metadata_field.children[0].description == Markdown(
            root="Metadata identifier"
        )
    ...
    def test_schema_helpers_handle_invalid_input(self):
        ...
        assert (
            self.rest_source._convert_parameter_to_field(
                {"name": "field", "schema": None}
            )
            is None
        )
    ...
    def test_get_response_schema_referenced_inline_object_array_properties(self):
        schema = deepcopy(
            MOCK_RESPONSE_INLINE_OBJECT_ARRAY["responses"]["200"]["content"][
                "application/json"
            ]["schema"]
        )
