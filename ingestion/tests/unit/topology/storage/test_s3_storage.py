...
class StorageUnitTest(TestCase):
    ...
    def test_no_metadata_file_returned_when_file_not_present(self):
        ...
        with (
            patch.object(
                self.s3_reader.client, "get_object", side_effect=missing_file
            ) as get_object,
        ...
        self.assertIn(
            f"Error fetching file [{OPENMETADATA_TEMPLATE_FILE_NAME}] from S3",
            str(raised.exception),
        )
        self.assertIn("NoSuchKey", str(raised.exception))
        get_object.assert_called_once_with(
            Bucket="test", Key=OPENMETADATA_TEMPLATE_FILE_NAME
        )
    ...
    def test_get_sample_file_path_returns_none_when_prefix_has_no_files(self):
        ...
        with patch.object(
            self.object_store_source.s3_client, "list_objects_v2", return_value={}
        ) as list_objects:
