...
ExternalRow = namedtuple(
    "ExternalRow", ["table_catalog", "table_schema", "table_name", "storage_path"]
)
...
def pairs_json(column_pairs):
    ...
    if column_pairs is None:
        return None
    return json.dumps(
        [{"source": source, "target": target} for source, target in column_pairs]
    )
...
def run(lineage_source, rows=(), tables=None, external_rows=(), probe_error=None):
    ...
    stub_rows(
        lineage_source, rows, external_rows=external_rows, probe_error=probe_error
    )
...
def table_edges(results, tables):
    ...
    return sorted(
        (
            names[str(result.right.edge.fromEntity.id.root)],
            names[str(result.right.edge.toEntity.id.root)],
        )
...
class TestNativeLineageQuery:
    ...
    def test_column_mappings_join_on_null_safe_equality(self):
        ...
        join = query.split("LEFT JOIN column_edges")[1].split(
            "LEFT JOIN system.query.history"
        )[0]
        assert join.count("<=>") == 4
        assert " = " not in join
...
class TestNativeLineageStream:
    ...
    def test_lineage_is_read_in_a_single_query(self, lineage_source):
        ...
        """
        executed = stub_rows(
            lineage_source, [table_row("cat.schema.src", "cat.schema.tgt")]
        )
        ...
        lineage_queries = [
            sql
            for sql in executed
            if "system.access.table_lineage" in sql and "WHERE 1=0" not in sql
        ]
    ...
    def test_column_mappings_of_an_edge_are_emitted_sorted(self, lineage_source):
        ...
        tables = {
            "cat.schema.src": a_table(
                "src",
                columns=[
                    a_column("col_a", "svc.src.col_a"),
                    a_column("col_b", "svc.src.col_b"),
                ],
            ),
            "cat.schema.tgt": a_table(
                "tgt",
                columns=[
                    a_column("col_x", "svc.tgt.col_x"),
                    a_column("col_y", "svc.tgt.col_y"),
                ],
            ),
        }
        ...
        assert [
            (pair.fromColumns[0].root, pair.toColumn.root) for pair in columns_lineage
        ] == [
    ...
    def test_unreadable_column_pairs_keep_the_edge(self, lineage_source):
        ...
        stub_rows(
            ...
            [
                TableRow(
                    "cat.schema.src", None, "cat.schema.tgt", None, "not json", None
                )
            ],
        )
        resolve_tables(lineage_source, tables)
    ...
    def test_a_self_referencing_row_is_not_an_edge(self, lineage_source):
        ...
        results = run(
            lineage_source,
            [table_row("cat.schema.stream", "cat.schema.stream")],
            tables,
        )
    ...
    def test_only_the_self_reference_is_dropped(self, lineage_source):
        tables = {
            "cat.schema.events": a_table("events"),
            "cat.schema.snapshot": a_table("snapshot"),
        }
        rows = [
        ...
        assert table_edges(results, tables) == [
            ("cat.schema.events", "cat.schema.snapshot")
        ]
    ...
    def test_a_malformed_source_name_is_skipped(self, lineage_source):
        ...
        results = run(
            lineage_source, [table_row("malformed_name", "cat.schema.tgt")], tables
        )
    ...
    def test_an_upstream_that_was_never_ingested_is_skipped(self, lineage_source):
        ...
        results = run(
            lineage_source, [table_row("cat.schema.src", "cat.schema.tgt")], tables
        )
...
class TestSqlEnrichment:
    ...
    def test_sql_is_attached_to_the_edge(self, lineage_source):
        ...
        assert (
            results[0].right.edge.lineageDetails.sqlQuery.root
    ...
    def test_column_mappings_and_sql_ride_the_same_edge(self, lineage_source):
        tables = {
            "cat.schema.src": a_table(
                "src", columns=[a_column("col_a", "svc.src.col_a")]
            ),
            "cat.schema.tgt": a_table(
                "tgt", columns=[a_column("col_x", "svc.tgt.col_x")]
            ),
        }
...
class TestColumnLineageDetails:
    def test_self_loop_prevention(self, lineage_source):
        table = a_table(
            "tgt",
            columns=[a_column("col_a", "local_unitycatalog.cat.schema.tgt.col_a")],
        )
        same_table_as_source = a_table(
            "src",
            columns=[a_column("col_a", "local_unitycatalog.cat.schema.src.col_a")],
        )
    ...
    def test_no_column_lineage_returns_none(self, lineage_source):
        result = lineage_source._get_column_lineage_details(
            a_table("src"), a_table("tgt"), column_pairs={}
        )
...
class TestExternalLocationLineage:
    ...
    def test_cache_external_locations_builds_the_inverse_map(self, lineage_source):
        ...
        assert lineage_source.path_to_table_map["s3://bucket/path2"] == {
            "cat.schema.ext2"
        }
        assert "cat.schema.no_path" not in {
            table
            for tables in lineage_source.path_to_table_map.values()
            for table in tables
        }
    ...
    def test_process_external_location_lineage_from_cache(self, lineage_source):
        ...
        results = list(
            lineage_source._process_external_location_lineage("cat.schema.test_table")
        )
    ...
    def test_external_tables_are_emitted_without_any_lineage_row(self, lineage_source):
        ...
        assert container_edges(results, tables) == [
            (str(container_entity.id.root), "cat.schema.ext")
        ]
...
class TestContainerColumnLineage:
    ...
    def test_get_container_column_lineage(self, lineage_source):
        ...
        table_entity = a_table(
            "test_table", columns=[a_column("id", "service.db.schema.test_table.id")]
        )
...
class TestPathBasedLineage:
    ...
    def test_path_source_resolves_to_external_table(self, lineage_source):
        ...
        lineage_source.path_to_table_map[raw_path] = {
        ...
        results = run(
            ...
            [
                table_row(
                    target="bronze_ns.deltalake_ns.managed_table_ns",
        ...
        assert table_edges(results, tables) == [
            (
                "bronze_ns.deltalake_ns.external_table",
                "bronze_ns.deltalake_ns.managed_table_ns",
            )
        ]

    def test_path_source_matches_despite_trailing_slash_and_scheme_alias(
        self, lineage_source
    ):
        tables = {
    ...
    def test_path_shared_by_two_external_tables_yields_both_upstreams(
        self, lineage_source
    ):
        tables = {
    ...
    def test_target_path_resolves_to_external_table(self, lineage_source):
        ...
        tables = {
            "cat.schema.src": a_table("src"),
            "cat.schema.gold_ext": a_table("gold_ext"),
        }
        ...
        assert table_edges(results, tables) == [
            ("cat.schema.src", "cat.schema.gold_ext")
        ]
    ...
    def test_column_lineage_keyed_by_path_resolved_table(self, lineage_source):
        tables = {
            "cat.schema.ext": a_table(
                "ext", columns=[a_column("col_a", "svc.ext.col_a")]
            ),
            "cat.schema.tgt": a_table(
                "tgt", columns=[a_column("col_x", "svc.tgt.col_x")]
            ),
    ...
    def test_an_edge_reported_by_both_name_and_path_is_emitted_once(
        self, lineage_source
    ):
        """
    ...
    def test_process_path_lineage_emits_container_edge(self, lineage_source):
        ...
        results = list(
            lineage_source._process_path_lineage(
                table_entity, "cat.schema.test_table", {"s3://bucket/unregistered"}
            )
        )
        ...
        assert results[0].right.edge.toEntity.id == table_entity.id
        assert (
            results[0].right.edge.lineageDetails.source
    ...
    def test_process_path_lineage_falls_back_to_the_de_aliased_scheme(
        self, lineage_source
    ):
        """A container ingested by the S3 connector is stored as s3://, never s3a://"""
        container_entity = a_container()
        lineage_source.metadata.es_search_container_by_path.side_effect = [
            [],
            [container_entity],
        ]
        ...
        results = list(
            lineage_source._process_path_lineage(
                a_table(), "cat.schema.test_table", {"s3a://bucket/data"}
            )
        )

        assert len(results) == 1
        assert [
            call.kwargs["full_path"]
            for call in lineage_source.metadata.es_search_container_by_path.call_args_list
        ] == ["s3a://bucket/data", "s3://bucket/data"]

    def test_process_path_lineage_without_a_container_yields_nothing(
        self, lineage_source
    ):
        lineage_source.metadata.es_search_container_by_path.return_value = []
        ...
        results = list(
            lineage_source._process_path_lineage(
                a_table(),
                "cat.schema.test_table",
                {"abfss://raw@storage.dfs.core.windows.net/t"},
            )
        )
    ...
    def test_process_path_lineage_no_paths_for_table(self, lineage_source):
        results = list(
            lineage_source._process_path_lineage(
                a_table(), "cat.schema.test_table", set()
            )
        )
    ...
    def test_process_path_lineage_carries_container_column_lineage(
        self, lineage_source
    ):
        table_entity = a_table(
            columns=[a_column("id", "service.db.schema.test_table.id")]
        )
        container_entity = a_container(
            data_model=ContainerDataModel(
                columns=[
        ...
        results = list(
            lineage_source._process_path_lineage(
                table_entity, "cat.schema.test_table", {"s3://bucket/data"}
            )
        )
        ...
        assert (
            details.columnsLineage[0].toColumn.root == "service.db.schema.test_table.id"
        )

    def test_process_path_lineage_survives_a_failing_container_lookup(
        self, lineage_source
    ):
        """One unreachable path must not take the rest of the table's lineage down."""
        lineage_source.metadata.es_search_container_by_path.side_effect = RuntimeError(
            "elasticsearch down"
        )

        results = list(
            lineage_source._process_path_lineage(
                a_table(), "cat.schema.test_table", {"s3://bucket/boom"}
            )
        )

        assert len(results) == 0

    def test_an_unresolved_path_reaches_the_container_of_its_target(
        self, lineage_source
    ):
        tables = {"cat.schema.tgt": a_table("tgt")}
        container_entity = a_container()
        ...
        results = run(
            ...
            [
                table_row(
                    target="cat.schema.tgt", source_path="s3a://bucket/unregistered/"
                )
            ],
            tables,
        )
        ...
        assert container_edges(results, tables) == [
            (str(container_entity.id.root), "cat.schema.tgt")
        ]
    ...
    def test_iter_emits_a_path_resolved_edge_end_to_end(self, lineage_source):
        ...
        tables = {
            external_table: a_table(
                "external_table", columns=[a_column("id", f"svc.{external_table}.id")]
            ),
            managed_table: a_table(
                "managed_table_ns", columns=[a_column("id", f"svc.{managed_table}.id")]
            ),
        }
        lineage_source.metadata.es_search_container_by_path.return_value = []
        ...
        results = run(
            ...
            tables=tables,
            external_rows=[
                ExternalRow("bronze_ns", "deltalake_ns", "external_table", raw_path)
            ],
        )
        ...
        assert (
            edge.lineageDetails.columnsLineage[0].fromColumns[0].root
            == f"svc.{external_table}.id"
        )
        assert (
            edge.lineageDetails.columnsLineage[0].toColumn.root
            == f"svc.{managed_table}.id"
        )
        assert edge.lineageDetails.sqlQuery.root.startswith(
            "CREATE TABLE managed_table_ns"
        )
...
class TestLineageDrivenIteration:
    ...
    def test_an_upstream_named_by_two_targets_is_resolved_once(self, lineage_source):
        ...
        resolved = [
            call.kwargs["fqn"]
            for call in lineage_source.metadata.get_by_name.call_args_list
        ]
    ...
    def test_a_failing_lookup_is_not_cached(self, lineage_source):
        ...
        lineage_source.metadata.get_by_name.side_effect = RuntimeError(
            "connection reset"
        )
    ...
    def test_filters_apply_to_the_names_the_system_tables_report(self, lineage_source):
        ...
        lineage_source.source_config.databaseFilterPattern = FilterPattern(
            excludes=["excluded_cat"]
        )
        lineage_source.source_config.schemaFilterPattern = FilterPattern(
            excludes=["excluded_schema"]
        )
        lineage_source.source_config.tableFilterPattern = FilterPattern(
            excludes=["excluded_table"]
        )
    ...
    def test_a_filtered_table_is_reported_once(self, lineage_source):
        ...
        assert lineage_source.status.filtered == [
            {"local_unitycatalog.cat.schema.ext": "Table Filtered Out"}
        ]
