...
def _source(upstreams: dict[str, Table]):
    ...
    source.metadata.get_by_name.side_effect = (
        lambda entity=None, fqn=None, **_: upstreams.get(str(fqn).split(".", 1)[1])
    )
    return source
...
def _edges(source, target: str, upstream_columns: dict[str, dict]):
    return [
        ...
        for result in source._process_table_lineage(
            _table(target), target, upstream_columns, {}
        )
    ]
