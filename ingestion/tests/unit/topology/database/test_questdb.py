...
# ── utils: dialect patching ───────────────────────────────────────────────────


def test_patch_questdb_dialect_returns_empty_constraints_and_indexes():
    """QuestDB has no primary keys, foreign keys, unique constraints, or
    secondary indexes. Patched dialect methods must return the SQLAlchemy
    empty-shape values so introspection doesn't try pg_catalog queries that
    QuestDB cannot evaluate."""
    engine = MagicMock()
    engine.dialect = MagicMock()

    patch_questdb_dialect(engine)

    assert engine.dialect.get_pk_constraint(engine, "t", "public") == {
        "constrained_columns": [],
        "name": None,
    }
    assert engine.dialect.get_foreign_keys(engine, "t", "public") == []
    assert engine.dialect.get_unique_constraints(engine, "t", "public") == []
    assert engine.dialect.get_indexes(engine, "t", "public") == []
    assert engine.dialect.get_check_constraints(engine, "t", "public") == []
    assert engine.dialect.get_table_comment(engine, "t", "public") == {"text": None}
    assert engine.dialect.get_view_definition(engine, "v", "public") == ""
