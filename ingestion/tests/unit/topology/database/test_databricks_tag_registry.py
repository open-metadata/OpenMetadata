...
@pytest.mark.parametrize(
    ...
    [
        (None, "Value", None),
        ("", "Value", None),
        ("Class", "Value", TagDefinition("Class", "Value", "Classification", "Tag")),
        ("Class", " Value ", TagDefinition("Class", " Value ", "Classification", "Tag")),
        ("Key", None, TagDefinition("NativeTags", "Key", "Key-only", "Key-only")),
        ("Key", "", TagDefinition("NativeTags", "Key", "Key-only", "Key-only")),
        ("Key", " \t ", TagDefinition("NativeTags", "Key", "Key-only", "Key-only")),
    ],
)
def test_native_mapping_preserves_names_and_descriptions(name, value, expected):
    config = TagMappingConfig("Classification", "Tag", "NativeTags", "Key-only")
    assert map_databricks_tag(name, value, config) == expected
...
def test_key_only_tags_use_connector_specific_classification_and_descriptions(source, value):
    sql_rows(source, table=[row("Restricted", value)])
    records = database_stage(source) + schema_stage(source) + table_stage(source)
    assert len(records) == 1
    if isinstance(source, DatabricksSource):
        classification = "DATABRICKS_TAGS"
        description = "Databricks tags ingested as key-only (no associated value)."
    else:
        classification = "UNITY_CATALOG_TAGS"
        description = "Unity Catalog tags ingested as key-only (no associated value)."
    assert records[0].right.classification_request.name.root == classification
    assert records[0].right.classification_request.description.root == description
    assert records[0].right.tag_request.description.root == description
    assert fqns(source.get_tag_labels("table")) == [f"{classification}.Restricted"]
...
def test_concurrent_schemas_keep_labels_separate(source):
    ...
    def process_schema(name, expected):
        ...
        ready.wait()
        if name == "second":
            assert first_cleared.wait(timeout=10)
