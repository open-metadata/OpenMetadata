...
class TestBigqueryPerSchemaCaching:
    ...
    DATASETS: ClassVar[dict] = {
        "clean_identity_resolution": (
            "Clean Identity Resolution data",
            {"tier": "bronze"},
        ),
    ...
    def setup_method(self):
        ...
        self.bq_source.engine.connect.return_value.__enter__ = MagicMock(
            return_value=mock_conn
        )
        self.bq_source.engine.connect.return_value.__exit__ = MagicMock(
            return_value=False
        )
    ...
    def test_each_schema_gets_its_own_description(self):
        ...
        ingested = {
            schema: self._walk_schema_then_tables(schema)[0] for schema in self.DATASETS
        }
    ...
    def test_each_schema_gets_its_own_dataset_labels(self):
        ...
        ingested = {
            schema: self._walk_schema_then_tables(schema)[1] for schema in self.DATASETS
        }

        assert ingested == {
            schema: labels for schema, (_, labels) in self.DATASETS.items()
        }
    ...
    def test_interleaved_schemas_keep_their_own_description(self):
        ...
        descriptions = {
            schema: self.bq_source.get_schema_description(schema) for schema in schemas
        }
    ...
    def test_table_obj_survives_entry_evicted_before_read(self):
        ...
        self.bq_source._table_obj_cache.put(
            f"{MOCK_DB_NAME}.clean_mac.events", object()
        )
