...
from metadata.ingestion.source.database.bigquery.connection import (
    test_connection as bigquery_test_connection,
)
...
def test_policy_tag_client_uses_impersonated_credentials(
    mock_impersonate, mock_client, base_config
):
    """
...
@patch(f"{_HELPER_MODULE}.PolicyTagManagerClient")
@patch(f"{_HELPER_MODULE}.get_gcp_impersonate_credentials")
def test_policy_tag_client_unchanged_without_impersonation(
    mock_impersonate, mock_client, base_config
):
...
def _policy_tag_connection(taxonomy_project_ids=None) -> BigQueryConnection:
    config = deepcopy(ADC_CONFIG)
...
def _run_get_tags_step(service_connection: BigQueryConnection, host="proj-adc"):
    """
    Invoke the GetTags step of ``test_connection``.

    The step is a closure built inside ``test_connection``, so it is reached by
    capturing the ``test_fn`` map handed to the steps runner and calling it.
    """
    engine = MagicMock()
    engine.url.host = host
    captured = {}

    with patch(
        f"{_CONNECTION_MODULE}.test_connection_steps",
        side_effect=lambda **kwargs: captured.update(kwargs["test_fn"]),
    ):
...
def test_get_tags_step_uses_the_impersonation_aware_client(mock_get_client):
    ...
    service_connection = _policy_tag_connection(
        taxonomy_project_ids=["governance-proj"]
    )
    _run_get_tags_step(service_connection)

    mock_get_client.assert_called_once_with(service_connection)
    assert [
        call.kwargs["parent"] for call in client.list_taxonomies.call_args_list
    ] == [
        "projects/proj-adc/locations/eu",
        "projects/governance-proj/locations/eu",
    ]
    client.list_policy_tags.assert_called_once_with(
        parent="projects/proj-adc/locations/eu/taxonomies/1"
    )
    client.transport.close.assert_called_once()


@patch(f"{_CONNECTION_MODULE}.get_policy_tag_client")
def test_denied_taxonomy_read_is_reported_as_a_policy_tag_failure(mock_get_client):
    """
    A Data Catalog 403 must not surface as a bare BigQuery permission error -
    that sends users to grant roles on the wrong API and, under impersonation,
    to the wrong identity.
    """
    client = mock_get_client.return_value
    client.list_taxonomies.side_effect = PermissionDenied(
        "The caller does not have permission"
    )

    with pytest.raises(PolicyTagAccessError) as raised:
        _run_get_tags_step(_policy_tag_connection())

    assert "proj-adc" in str(raised.value)
    assert "in eu" in str(raised.value)
    client.transport.close.assert_called_once()
...
def _workflow_source(**connection_overrides) -> dict:
    return {
        ...
        "serviceConnection": {
            "config": {**deepcopy(JSON_KEY_CONFIG), **connection_overrides}
        },
...
@patch(f"{_CONNECTION_MODULE}.get_connection")
...
def test_ingestion_source_builds_the_policy_tag_client_through_the_helper(
    mock_test_connection, mock_set_project_id, mock_get_connection, mock_get_client
):
    """
...
def test_prefetch_policy_tags_is_not_repeated_for_the_same_projects():
    ...
    assert [
        call.kwargs["parent"]
        for call in source._policy_tag_client.list_taxonomies.call_args_list
    ] == [
...
def test_prefetch_policy_tags_does_not_retry_a_denied_project_per_schema():
    ...
    source = _prefetch_source()
    source._policy_tag_client.list_taxonomies.side_effect = PermissionDenied(
        "The caller does not have permission"
    )
