...
"""Database tag registration, lookup and cleanup."""
...
from metadata.ingestion.source.database.incremental_metadata_extraction import IncrementalConfig
...
def test_early_schema_attachments_have_independent_lifetimes(source):
    source.incremental = IncrementalConfig(enabled=False)
    source.status = Status()
    source.source_config.markDeletedSchemas = False
    source.source_config.markDeletedTables = False
    source.source_config.markDeletedStoredProcedures = False
    tag = TagDefinition("Class", "Value", "", "")
    source.context.get().upsert("database", "db")
    for schema in ("a", "b", "filtered"):
        source.attach_tag(entity_fqn=f"svc.db.{schema}", tag=tag)
    source.context.get().upsert("database_schema", "a")
    ...
    list(source._run_node_post_process(source.topology.databaseSchema))
...
def test_schema_cleanup_handles_quoted_fqn_components(source):
    source.context.get().upsert("database_service", "my.service")
    source.context.get().upsert("database", "my.database")
    source.context.get().upsert("database_schema", "my.schema")
    tag = TagDefinition("Class", "Value", "", "")
...
def test_database_cleanup_preserves_entities_outside_database(source, entity):
