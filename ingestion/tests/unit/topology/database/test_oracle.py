...
class TestOracleViewDefinitionFallback:
    ...
    def _bulk_query_lowercase():
        ...
        # are lowercased.
        return 'SELECT LOWER(v.view_name) AS "view_name", LOWER(v.owner) AS "schema", text AS "view_def" FROM DBA_VIEWS v'

    def _make_connection(
        self, names_rows, texts=None, ddls=None, fail_text=(), fail_ddl=()
    ):
        ...
        def execute(clause, params=None):
            ...
            raise DatabaseError(
                ...
                None,
                Exception(
                    "DPI-1037: column at array position 13 fetched with error 1406"
                ),
            )
    ...
    def test_bulk_truncation_recovers_views_via_per_view_fallback(self):
        ...
        conn = self._make_connection(
            names_rows,
            texts={
                "GOOD_VIEW": "SELECT * FROM t",
                "MV1": "SELECT count(*) FROM t",
                "NULL_TEXT": None,
            },
        ...
        assert (
            defs[("good_view", "sam")]
            == "CREATE OR REPLACE VIEW GOOD_VIEW AS SELECT * FROM t"
        )
        assert (
            defs[("mv1", "sam")]
            == "CREATE OR REPLACE VIEW MV1 AS SELECT count(*) FROM t"
        )
        # NULL text falls back to GET_DDL, mirroring the bulk query.
        assert (
            defs[("null_text", "sam")]
            == "CREATE FORCE VIEW SAM.NULL_TEXT AS SELECT 1 FROM dual"
        )
    ...
    def test_fallback_keeps_native_case_for_preserve_identifier_case(self):
        ...
        conn = self._make_connection(
            [("Sam", "MyView", "VIEW")], texts={"MyView": "SELECT 1"}
        )

        get_all_view_definitions(dialect, conn, self._bulk_query_lowercase())

        assert (
            dialect.all_view_definitions[("MyView", "Sam")]
    ...
    def test_successful_bulk_fetch_does_not_trigger_fallback(self):
        ...
        assert (
            dialect.all_view_definitions[("v1", "sam")]
