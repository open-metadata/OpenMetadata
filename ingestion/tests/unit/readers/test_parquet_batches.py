...
class TestParquetBatchReading(unittest.TestCase):
    ...
    def test_fallback_when_iter_batches_not_available(self):
        """Test fallback to regular reading when iter_batches is not available"""
        # Create mock parquet file without iter_batches capability
        # Use spec to control available attributes
        mock_parquet_file = Mock(spec=["read"])  # Only allow 'read' method

        sample_data = pd.DataFrame(
            {"id": [1, 2, 3], "name": ["Alice", "Bob", "Charlie"]}
        )

        # Mock the read() method and arrow table
        ...
        with patch(
            ...
            with patch("metadata.readers.dataframe.parquet.logger") as mock_logger:
                ...
                # Verify regular read was called
