...
def test_rest_client_applies_configured_user_agent():
    client = REST(
        ClientConfig(base_url=BASE_URL, user_agent="snowflake_metadata (v1.10.0.0)")
    )
...
def test_user_agent_includes_connector_workflow_and_service(*_):
    assert (
        _user_agent_for(SNOWFLAKE_SOURCE)
        == "snowflake_metadata (service: local_snowflake; v1.10.0.0)"
    )
...
def test_user_agent_reflects_workflow_type(*_):
    ...
    assert (
        _user_agent_for(lineage_source)
        == "snowflake_lineage (service: local_snowflake; v1.10.0.0)"
    )
    assert (
        _user_agent_for(profiler_source)
        == "snowflake_profiler (service: local_snowflake; v1.10.0.0)"
    )
...
def test_user_agent_falls_back_to_raw_type_for_unmapped_workflow(*_):
    ...
    auto_classification = {
        **SNOWFLAKE_SOURCE,
        "sourceConfig": {"config": {"type": "AutoClassification"}},
    }

    assert (
        _user_agent_for(auto_classification)
        == "snowflake_AutoClassification (service: local_snowflake; v1.10.0.0)"
    )
...
def test_user_agent_omits_service_when_absent(*_):
    assert (
        _user_agent_for({**SNOWFLAKE_SOURCE, "serviceName": None})
        == "snowflake_metadata (v1.10.0.0)"
    )


@patch(
    "metadata.workflow.ingestion.get_client_version",
    side_effect=RuntimeError("no version"),
)
def test_user_agent_omits_version_when_unavailable(*_):
    assert (
        _user_agent_for(SNOWFLAKE_SOURCE)
        == "snowflake_metadata (service: local_snowflake)"
    )
...
@patch(
    "metadata.workflow.ingestion.IngestionWorkflow._resolve_workflow_type",
    return_value=None,
)
...
def test_base_workflow_user_agent_includes_version(*_):
    assert (
        BaseWorkflow._build_user_agent(object()) == "openmetadata-ingestion (v1.10.0.0)"
    )


@patch(
    "metadata.workflow.base.get_client_version", side_effect=RuntimeError("no version")
)
...
def test_sanitize_user_agent_strips_crlf():
    assert (
        sanitize_user_agent("snowflake\r\nInjected: header")
        == "snowflakeInjected: header"
    )
...
def test_sanitize_user_agent_strips_control_characters():
    ...
    assert (
        sanitize_user_agent("snow\x00fla\x07ke\x1bmeta\tdata\x7f")
        == "snowflakemetadata"
    )
...
def test_rest_client_sanitizes_user_agent_before_setting_header():
    client = REST(
        ClientConfig(
            base_url=BASE_URL, user_agent="snowflake_metadata\r\nX-Injected: 1"
        )
    )
...
def _capture_sse_headers(client: SSEClient) -> dict:
    ...
    with patch(
        "metadata.ingestion.ometa.sse_client.httpx.Client", return_value=fake_client
    ):
...
def test_user_agent_sanitizes_service_name(*_):
    ...
    assert (
        _user_agent_for(poisoned)
        == "snowflake_metadata (service: local_snowflakeX-Injected: 1; v1.10.0.0)"
    )
