...
class TestMatchProcedureDisambiguation:
    ...
    def _procedure(name, database, schema):
        return StoredProcedure(
            ...
            storedProcedureCode=StoredProcedureCode(
                code=f"CREATE PROCEDURE {name}", language="SQL"
            ),
    ...
    def test_single_candidate_matches_regardless_of_context(self):
        ...
        assert (
            StoredProcedureLineageMixin._match_procedure(
                [procedure], self._query("usp_load")
            )
            is procedure
        )
    ...
    def test_same_name_disambiguated_by_database(self):
        ...
        assert (
            StoredProcedureLineageMixin._match_procedure(
                candidates, self._query("usp_load", "hyb_qs_b", "dbo")
            )
            is procedure_b
        )
        assert (
            StoredProcedureLineageMixin._match_procedure(
                candidates, self._query("usp_load", "hyb_qs_a", "dbo")
            )
            is procedure_a
        )
    ...
    def test_same_name_disambiguated_by_schema(self):
        ...
        result = StoredProcedureLineageMixin._match_procedure(
            candidates, self._query("usp_load", "db_a", "sales")
        )
    ...
    def test_ambiguous_without_context_returns_none(self):
        ...
        assert (
            StoredProcedureLineageMixin._match_procedure(
                candidates, self._query("usp_load")
            )
            is None
        )
    ...
    def test_no_candidates_returns_none(self):
        assert (
            StoredProcedureLineageMixin._match_procedure(None, self._query("usp_load"))
            is None
        )
        assert (
            StoredProcedureLineageMixin._match_procedure([], self._query("usp_load"))
            is None
        )
    ...
    def test_producer_attributes_collided_names_by_reporting_database(self):
        ...
        mixin.metadata.paginate_es.return_value = iter(
            [
                self._procedure("usp_load", database, "dbo")
                for database in ("db_a", "db_b", "db_c")
            ]
        )
        mixin.engine._mock_conn.execute.return_value.all.return_value = [
        ...
        attributed = {
            result.query_by_procedure.query_database_name: result.procedure.database.name
