...
class TestStoredProcedures:
    ...
    def test_get_procedure_name_from_call(self):
        ...
        assert (
            get_procedure_name_from_call(query_text="CALL schema.procedure_name(...)")
            == "procedure_name"
        )

        assert (
            get_procedure_name_from_call(query_text="CALL procedure_name(...)")
            == "procedure_name"
        )
        ...
        assert (
            get_procedure_name_from_call(query_text="BEGIN schema.procedure_name; END;")
            == "procedure_name"
        )

        assert (
            get_procedure_name_from_call(query_text="BEGIN procedure_name; END;")
            == "procedure_name"
        )
    ...
    def test_get_procedure_name_with_nested_function_args(self):
        ...
        assert (
            get_procedure_name_from_call(
                query_text="BEGIN SALES.INSERT_NUMBER(TO_NUMBER(:1)); END;"
            )
            == "insert_number"
        )
        ...
        assert (
            get_procedure_name_from_call(
                query_text="CALL SALES.INSERT_NUMBER(TO_NUMBER(12345))"
            )
            == "insert_number"
        )
        ...
        assert (
            get_procedure_name_from_call(
                query_text="BEGIN SCHEMA.PROC(TO_DATE('2024-01-01'), NVL(x, 0)); END;"
            )
            == "proc"
        )
