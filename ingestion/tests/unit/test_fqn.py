...
class TestFqnGrammar(TestCase):
    ...
    def test_unicode_handling(self):
        ...
        unicode_test_cases = [
            ...
            UnicodeFqnTestCase("service.데이터베이스.스키마.테이블", 4, "Korean Hangul", "Korean"),
            # European Languages
            UnicodeFqnTestCase(
            ...
            UnicodeFqnTestCase(
                "service.βάση_δεδομένων.σχήμα.πίνακας", 4, "Greek characters", "Greek"
            ),
            UnicodeFqnTestCase(
                "service.datenbank.schema.tabelle", 4, "German with umlauts", "German"
            ),
            UnicodeFqnTestCase(
                "service.base_de_données.schéma.table",
                4,
                "French with accents",
                "French",
            ),
            UnicodeFqnTestCase(
                "service.banco_de_dados.esquema.tabela",
                4,
                "Portuguese with accents",
                "Portuguese",
            ),
            UnicodeFqnTestCase(
                "service.baza_danych.schemat.tabela",
                4,
                "Polish with special characters",
                "Polish",
            ),
            # Middle Eastern and RTL Languages
            UnicodeFqnTestCase(
                "service.قاعدة_البيانات.مخطط.جدول",
                4,
                "Arabic characters (RTL)",
                "Arabic",
            ),
            UnicodeFqnTestCase(
                "service.מסד_נתונים.סכמה.טבלה", 4, "Hebrew characters (RTL)", "Hebrew"
            ),
            UnicodeFqnTestCase(
                "service.دیتابیس.اسکیما.جدول", 4, "Persian/Farsi characters", "Persian"
            ),
            # Indian Languages
            UnicodeFqnTestCase(
                "service.डेटाबेस.स्कीमा.तालिका", 4, "Hindi Devanagari script", "Hindi"
            ),
            UnicodeFqnTestCase(
                "service.ডেটাবেস.স্কিমা.টেবিল", 4, "Bengali script", "Bengali"
            ),
            UnicodeFqnTestCase(
                "service.డేటాబేస్.స్కీమా.టేబుల్", 4, "Telugu script", "Telugu"
            ),
            # Special Characters and Symbols
            UnicodeFqnTestCase("service.🌟.⭐.✨", 4, "Emoji characters", "Emoji"),
            UnicodeFqnTestCase("service.🚀.🛸.👽", 4, "More emoji characters", "Emoji"),
            UnicodeFqnTestCase(
                "service.α.β.γ.δ.ε", 6, "Greek mathematical symbols", "Greek Math"
            ),
            UnicodeFqnTestCase("service.∑.∫.∆.∇", 5, "Mathematical symbols", "Math"),
            # Mixed Scripts
            UnicodeFqnTestCase(
                "service_eng.数据库_db.スキーマ_schema.表_table",
                4,
                "Mixed English and Asian scripts",
                "Mixed",
            ),
            UnicodeFqnTestCase(
                "service.café_データ.schéma_схема.table_表",
                4,
                "Mixed European and Asian scripts",
                "Mixed",
            ),
            # Complex Unicode Cases
            UnicodeFqnTestCase(
                "service.naïve_résumé.café_señor.table",
                4,
                "Combined diacritical marks",
                "Diacritics",
            ),
            UnicodeFqnTestCase(
                "service.ä̈ö̈ü̈.ñ̃ç̧.ß̂",
                4,
            ...
            UnicodeFqnTestCase(
                'service."数据库.with.dots".schema.table',
                4,
                "Quoted Unicode name with dots",
                "Quoted Unicode",
            ),
            UnicodeFqnTestCase(
                '"🌟.service"."データベース.db".schema.table',
                4,
                "Multiple quoted Unicode names",
                "Quoted Unicode",
            ),
        ]
        ...
        for test_case in unicode_test_cases:
            with self.subTest(
                ...
                self.assertIsInstance(
                    test_case.fqn, str, f"FQN should be string: {test_case.description}"
                )
                self.assertTrue(
                    test_case.fqn, f"FQN should not be empty: {test_case.description}"
                )
                ...
                for i, part in enumerate(parts):
                    self.assertIsInstance(
                        part, str, f"Part {i} should be string: {test_case.description}"
                    )
                    self.assertTrue(
                        part, f"Part {i} should not be empty: {test_case.description}"
                    )
        ...
        for fqn_str, description in edge_cases:
            with self.subTest(fqn=fqn_str, description=description):
                try:
                    parts = fqn.split(fqn_str)
                    self.assertGreater(
                        len(parts), 0, f"Should split into parts: {description}"
                    )
