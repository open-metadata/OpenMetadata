...
class DbtUnitTest(TestCase):
    ...
    def test_dbt_generate_entity_link_with_case_insensitive_matching(self):
        """
        Test generate_entity_link with case-insensitive column matching for Snowflake-like databases.
        This tests the scenario where DBT manifest has lowercase column names but
        OpenMetadata stores uppercase column names (Snowflake unquoted identifiers).
        """
        ...
        # Create a mock table entity with UPPERCASE column names (like Snowflake)
        mock_table = Table(
            id=uuid.uuid4(),
            name="stg_customers",
            databaseSchema=EntityReference(id=uuid.uuid4(), type="databaseSchema"),
            fullyQualifiedName="local_redshift_dbt2.dev.dbt_jaffle.stg_customers",
            columns=[
                Column(name="ORDER_ID", dataType="INT", dataLength=1),
                Column(name="CUSTOMER_ID", dataType="INT", dataLength=1),
                Column(name="ORDER_DATE", dataType="DATE", dataLength=1),
            ],
        )
        ...
        # Test with table entity provided - should match case-insensitively
        result = generate_entity_link(dbt_test=dbt_test, table_entity=mock_table)

        # Should use the uppercase column name from the table entity
        self.assertListEqual(
            [
                "<#E::table::local_redshift_dbt2.dev.dbt_jaffle.stg_customers::columns::ORDER_ID>"
            ],
    ...
    def test_dbt_generate_entity_link_with_quoted_snowflake_columns(self):
        """
        Test generate_entity_link with quoted Snowflake identifiers (case-preserved).
        This tests the scenario where Snowflake columns are quoted and stored
        in lowercase, matching the DBT manifest column names.
        """
        ...
        # Create a mock table entity with lowercase column names (quoted Snowflake identifiers)
        mock_table = Table(
            id=uuid.uuid4(),
            name="stg_customers",
            databaseSchema=EntityReference(id=uuid.uuid4(), type="databaseSchema"),
        ...
        # Test with table entity provided - should match exactly
        result = generate_entity_link(dbt_test=dbt_test, table_entity=mock_table)

        # Should use the lowercase column name from the table entity
        self.assertListEqual(
            [
                "<#E::table::local_redshift_dbt2.dev.dbt_jaffle.stg_customers::columns::order_id>"
            ],
    ...
    def test_dbt_generate_entity_link_with_mixed_case_columns(self):
        """
        Test generate_entity_link with mixed case column names.
        This tests quoted identifiers with preserved mixed case like "CustomerId".
        """
        ...
        # Modify manifest node to have lowercase column name
        manifest_node.column_name = "orderid"

        # Create a mock table entity with mixed case column names
        mock_table = Table(
            id=uuid.uuid4(),
            name="stg_customers",
            databaseSchema=EntityReference(id=uuid.uuid4(), type="databaseSchema"),
            fullyQualifiedName="local_redshift_dbt2.dev.dbt_jaffle.stg_customers",
            columns=[
                Column(name="OrderId", dataType="INT", dataLength=1),
                Column(name="CustomerId", dataType="INT", dataLength=1),
                Column(name="OrderDate", dataType="DATE", dataLength=1),
            ],
        )
        ...
        # Should use the mixed case column name from the table entity
        self.assertListEqual(
            [
                "<#E::table::local_redshift_dbt2.dev.dbt_jaffle.stg_customers::columns::OrderId>"
            ],
            result,
        )

    def test_dbt_generate_entity_link_column_not_found(self):
        """
        Test generate_entity_link when column name doesn't match any column in table entity.
        Should fall back to using the column name from DBT manifest.
        """
        ...
        # Modify manifest node to have a column name that doesn't exist
        manifest_node.column_name = "nonexistent_column"

        # Create a mock table entity without the column
        mock_table = Table(
            id=uuid.uuid4(),
            name="stg_customers",
            databaseSchema=EntityReference(id=uuid.uuid4(), type="databaseSchema"),
        ...
        # Test with table entity provided - should use original column name from manifest
        result = generate_entity_link(dbt_test=dbt_test, table_entity=mock_table)

        # Should fall back to the manifest column name
        self.assertListEqual(
            [
                "<#E::table::local_redshift_dbt2.dev.dbt_jaffle.stg_customers::columns::nonexistent_column>"
            ],
            result,
        )

    def test_dbt_generate_entity_link_table_level_test(self):
        """
        Test generate_entity_link for table-level tests (no column_name attribute).
        """
        ...
        # Remove column_name attribute to simulate table-level test
        ...
        # Test with table entity provided
        result = generate_entity_link(dbt_test=dbt_test, table_entity=mock_table)

        # Should generate table-level entity link (no column)
        self.assertListEqual(
            ["<#E::table::local_redshift_dbt2.dev.dbt_jaffle.stg_customers>"],
            result,
        )
