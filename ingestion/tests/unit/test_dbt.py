...
class DbtUnitTest(TestCase):
    ...
    def __init__(self, methodName, test_connection) -> None:
        ...
        self.dbt_files = DbtFiles(dbt_manifest=json.loads(MOCK_SAMPLE_MANIFEST))
        self.dbt_objects = DbtObjects(
            dbt_catalog=parse_catalog(self.dbt_files.dbt_catalog)
            if self.dbt_files.dbt_catalog
            else None,
            dbt_manifest=parse_manifest(self.dbt_files.dbt_manifest),
            dbt_run_results=parse_run_results(self.dbt_files.dbt_run_results)
            if self.dbt_files.dbt_run_results
            else None,
        )

    def test_dbt_validate(self):
        with self.assertLogs() as captured:
            self.dbt_source_obj.validate_dbt_files(dbt_files=self.dbt_files)
