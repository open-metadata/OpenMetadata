...
class TestAddDbtTestResultSkipsCompiledOnly(TestCase):
    ...
    def test_real_failure_result_is_ingested(self):
        ...
        source.metadata.add_test_case_results.assert_called_once()
        test_case_result = source.metadata.add_test_case_results.call_args.kwargs["test_results"]
        self.assertEqual(test_case_result.result, "Got 3 results, configured to fail if != 0")
...
class TestDbtMetricGovernanceMetadata:
    ...
    def test_metric_governance_metadata_is_propagated(self):
        ...
        assert request.name == "segmented_drivers"
    ...
    def test_metric_without_meta_governance_stays_unchanged(self):
        ...
        metric_requests = [
            item
            for item in DbtSource.yield_dbt_metrics(source, self._simple_metric())
            if item.right is not None
        ]
        assert len(metric_requests) == 1
        request = metric_requests[0].right
