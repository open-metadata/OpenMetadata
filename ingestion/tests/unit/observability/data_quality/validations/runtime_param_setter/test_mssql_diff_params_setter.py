...
class TestOdbcDriverDerivation:
    ...
    def test_pymssql_maps_to_freetds(self):
        ...
        """
        assert (
            _odbc_driver_for_data_diff(_config(MssqlScheme.mssql_pymssql))
            == FREETDS_ODBC_DRIVER
        )
    ...
    def test_pymssql_ignores_the_driver_field(self):
        ...
        assert (
            config.driver == DEFAULT_ODBC_DRIVER
        )  # schema default, never set by a pymssql user
    ...
    def test_pytds_keeps_the_default_driver(self):
        ...
        """
        assert (
            _odbc_driver_for_data_diff(_config(MssqlScheme.mssql_pytds))
            == DEFAULT_ODBC_DRIVER
        )
...
class TestMssqlConnectionDict:
    def test_carries_credentials_and_derived_driver(self):
        connection_dict = MssqlConnection(
            _config(MssqlScheme.mssql_pymssql)
        ).get_connection_dict()
    ...
    def test_carries_connection_options(self):
        ...
        config.connectionOptions = ConnectionOptions(
            root={"Encrypt": "no", "Connection Timeout": "60"}
        )
    ...
    def test_connection_options_cannot_override_the_derived_driver(self):
        ...
        config = _config(MssqlScheme.mssql_pymssql)
        config.connectionOptions = ConnectionOptions(
            root={"odbc_driver": "ODBC Driver 18 for SQL Server"}
        )

        assert (
            MssqlConnection(config).get_connection_dict()["odbc_driver"]
            == FREETDS_ODBC_DRIVER
        )
...
class TestMssqlTableParameter:
    ...
    def test_stamps_database_and_schema_without_mutating_the_service_dict(
        self, db_service, monkeypatch
    ):
        ...
        first = param_setter.get_data_diff_url(
            db_service, "mssql_service.DM_Master.dbo.DIM_ACCOUNT"
        )
        second = param_setter.get_data_diff_url(
            db_service, "mssql_service.Other_DB.staging.DIM_ACCOUNT"
        )
