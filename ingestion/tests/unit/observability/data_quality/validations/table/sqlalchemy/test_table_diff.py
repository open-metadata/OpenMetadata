...
class TestDuplicateKeysNamed:
    ...
    def test_it_names_the_key_when_joindiff_rejects_it(self) -> None:
        ...
        with pytest.raises(DuplicateKeyError) as excinfo, validator._duplicate_keys_named():
    ...
    def test_it_blames_the_table_data_diff_names(self, table_index: int, table: str) -> None:
        """hashdiff trips on the duplicate while counting stats, and says which table it is in."""
        validator = build_duplicate_key_validator()

        with pytest.raises(DuplicateKeyError) as excinfo, validator._duplicate_keys_named():
            raise DataDiffDuplicateKeyError("Key (id) is not unique in table test_schema.test_table.", table_index)
    ...
    def test_it_re_raises_an_assertion_that_is_not_about_duplicate_keys(self) -> None:
        ...
        with pytest.raises(AssertionError) as excinfo, validator._duplicate_keys_named():
...
THREE_DIFFERING_KEYS = (("-", ("1", "alice")), ("+", ("1", "alicia")), ("-", ("2", "bob")), ("+", ("3", "carol")))


def diff_rows(*rows: tuple[str, tuple[str, ...]]) -> Generator[tuple[str, tuple[str, ...]], None, None]:
...
class TestCalculateDiffsWithLimit:
    ...
    def test_it_stops_reading_the_diff_once_over_the_limit(self) -> None:
        ...
        assert validator.calculate_diffs_with_limit(diff_rows(*THREE_DIFFERING_KEYS), 2) == 3
    ...
    def test_it_counts_every_differing_key_up_to_the_limit(self, limit: int) -> None:
        ...
        assert validator.calculate_diffs_with_limit(iter(THREE_DIFFERING_KEYS), limit) == 3


def build_table_diff(
    diff: Generator[tuple[str, tuple[str, ...]], None, None], table1_rows: int, table2_rows: int
) -> DiffResultWrapper:
    """What data_diff.diff_tables returns: the tables are diffed only as `diff` is read."""
    tables = [Mock(key_columns=("id",), table_path=("test_schema", name)) for name in ("table1", "table2")]
    info_tree = InfoTree(SegmentInfo(tables, rowcounts={1: table1_rows, 2: table2_rows}))
...
def build_run_validator(threshold: int = 0) -> TableDiffValidator:
    ...
    validator.test_case = TestCase.model_construct(
        parameterValues=[TestCaseParameterValue(name="threshold", value=str(threshold))],
...
class TestRun:
    ...
    def test_it_counts_the_diff_without_keeping_its_rows(self, diff_tables: Mock) -> None:
    ...
    def test_a_repeated_key_is_still_blamed_on_its_table(self, diff_tables: Mock) -> None:
        ...
        diff_tables.return_value = build_table_diff(
            (row for row in (("+", ("7", "alicia")), ("+", ("7", "alicia")))), table1_rows=1, table2_rows=2
        )

        with pytest.raises(DuplicateKeyError) as excinfo:
            validator._run()

        assert "Key column 'id' is not unique in pg.db.schema.table2" in str(excinfo.value)

    def test_over_the_threshold_it_stops_reading_the_diff_and_closes_it(self, diff_tables: Mock) -> None:
        validator = build_run_validator(threshold=2)
        table_diff = build_table_diff(diff_rows(*THREE_DIFFERING_KEYS), table1_rows=4, table2_rows=4)
        diff_tables.return_value = table_diff

        result = validator._run()

        assert result == HasAttributes(testCaseStatus=TestCaseStatus.Failed, failedRows=3)
