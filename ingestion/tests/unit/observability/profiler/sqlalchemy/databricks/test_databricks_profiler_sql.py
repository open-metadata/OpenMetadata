...
def test_lowercase_identifiers_are_left_unquoted(dialect):
    ...
    table = sa.Table(
        "my_table", sa.MetaData(), sa.Column("my_col", sa.Boolean), schema="my_schema"
    )
...
def test_quoted_identifiers_keep_their_backticks(dialect):
    ...
    table = sa.Table(
        "MyTable", sa.MetaData(), sa.Column("MyCol", sa.Boolean), schema="MySchema"
    )
...
def test_struct_children_are_quoted_individually(dialect):
    ...
    table = sa.Table(
        "MyTable", sa.MetaData(), sa.Column(nested, sa.String), schema="MySchema"
    )
...
def test_modulo_compiles_to_a_spark_function(dialect):
    ...
    compiled = str(
        ModuloFn(RandomNumFn(), 100).compile(
