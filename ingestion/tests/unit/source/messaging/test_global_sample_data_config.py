...
def mock_messaging_source():
    ...
    source._is_sample_data_storing_globally_disabled = (
        MessagingServiceSource._is_sample_data_storing_globally_disabled.__get__(source, MessagingServiceSource)
    )
    return source
...
class TestIsSampleDataGloballyDisabled:
    ...
    def test_returns_false_when_settings_has_no_config_value(self, mock_messaging_source):
        settings = MagicMock(spec=Settings)
        settings.config_value = None
        mock_messaging_source.metadata.get_profiler_config_settings.return_value = settings

        assert mock_messaging_source._is_sample_data_storing_globally_disabled() is False

    def test_returns_false_when_no_sample_data_config(self, mock_messaging_source):
        profiler_config = ProfilerConfiguration(metricConfiguration=[], sampleDataConfig=None)
        settings = Settings(
    ...
    def test_returns_false_when_store_enabled(self, mock_messaging_source):
        sample_config = SampleDataIngestionConfig(storeSampleData=True, readSampleData=True)
        profiler_config = ProfilerConfiguration(sampleDataConfig=sample_config)
        settings = Settings(
            config_type=SettingType.profilerConfiguration,
            config_value=profiler_config,
    ...
    def test_returns_false_when_store_enabled_read_disabled(self, mock_messaging_source):
        sample_config = SampleDataIngestionConfig(storeSampleData=True, readSampleData=False)
        profiler_config = ProfilerConfiguration(sampleDataConfig=sample_config)
        settings = Settings(
            config_type=SettingType.profilerConfiguration,
            config_value=profiler_config,
    ...
    def test_returns_false_on_api_exception(self, mock_messaging_source):
        mock_messaging_source.metadata.get_profiler_config_settings.side_effect = Exception("API error")
