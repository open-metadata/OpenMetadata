...
def test_falls_back_to_search_when_latest_versions_is_none():
    ...
    source = make_source(
        search_result=PagedList(
            [make_version("1"), make_version("3"), make_version("2")], None
        )
    )
...
def test_populated_latest_versions_still_selects_by_timestamp():
    """
    Pins this branch's selection rule, which deliberately differs from main:
    when `latest_versions` is populated the version whose timestamp matches the
    registered model's wins, even if a higher-numbered one is present. Only the
    unset/empty case -- the bug being fixed -- resolves by version number.
    """
    source = make_source(
        latest_versions=[
            make_version("9", last_updated_timestamp=100),
            make_version("4", last_updated_timestamp=200),
        ],
...
def test_empty_latest_versions_still_searches():
    ...
    """
    source = make_source(
        latest_versions=[], search_result=PagedList([make_version("5")], None)
    )
...
def test_exhausting_the_page_budget_does_not_ingest_stale_version():
    ...
    source.client.search_model_versions.side_effect = lambda **_: PagedList(
        [make_version("1")], "more"
    )
...
def test_search_filter_survives_quotes_in_model_name(model_name):
    ...
    source = make_source(
        search_result=PagedList([make_version("2")], None), model_name=model_name
    )
...
def test_version_without_run_id_is_recorded():
    source = make_source(
        search_result=PagedList([make_version("1", run_id=None)], None)
    )
...
def test_non_numeric_versions_are_skipped(bad_version):
    source = make_source(
        search_result=PagedList([make_version(bad_version), make_version("4")], None)
    )
