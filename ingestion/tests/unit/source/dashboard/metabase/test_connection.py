...
"""Unit tests for Metabase connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.dashboard.metabase.connection"


def test_metabase_connection_is_base_connection():
    assert issubclass(MetabaseConnection, BaseConnection)


def test_get_client_delegates_to_get_connection():
    with patch(f"{CONNECTION_MODULE}.get_connection") as mock_get:
        conn = MetabaseConnection(MagicMock())
        client = conn.client
