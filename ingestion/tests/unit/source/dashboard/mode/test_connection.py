...
"""Unit tests for Mode connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.dashboard.mode.connection"


def test_mode_connection_is_base_connection():
    assert issubclass(ModeConnection, BaseConnection)


def test_get_client_builds_the_client():
    with patch(f"{CONNECTION_MODULE}.ModeApiClient") as mock_builder:
        conn = ModeConnection(MagicMock())
        client = conn.client
