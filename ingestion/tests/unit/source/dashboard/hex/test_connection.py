...
"""Unit tests for Hex connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.dashboard.hex.connection"


def test_hex_connection_is_base_connection():
    assert issubclass(HexConnection, BaseConnection)


def test_get_client_delegates_to_get_connection():
    with patch(f"{CONNECTION_MODULE}.get_connection") as mock_get:
        conn = HexConnection(MagicMock())
        client = conn.client
