...
"""Unit tests for Qlik Sense connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.dashboard.qliksense.connection"


def test_qliksense_connection_is_base_connection():
    assert issubclass(QlikSenseConnection, BaseConnection)


def test_get_client_builds_the_client():
    with patch(f"{CONNECTION_MODULE}.QlikSenseClient") as mock_builder:
        conn = QlikSenseConnection(MagicMock())
        client = conn.client
