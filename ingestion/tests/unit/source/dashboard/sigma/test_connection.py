...
"""Unit tests for Sigma connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.dashboard.sigma.connection"


def test_sigma_connection_is_base_connection():
    assert issubclass(SigmaConnection, BaseConnection)


def test_get_client_delegates_to_get_connection():
    with patch(f"{CONNECTION_MODULE}.get_connection") as mock_get:
        conn = SigmaConnection(MagicMock())
        client = conn.client
