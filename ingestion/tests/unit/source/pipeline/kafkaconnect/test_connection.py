...
"""Unit tests for Kafka Connect connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.pipeline.kafkaconnect.connection"


def test_kafkaconnect_connection_is_base_connection():
    assert issubclass(KafkaConnectConnection, BaseConnection)


def test_get_client_builds_the_client():
    with patch(f"{CONNECTION_MODULE}.KafkaConnectClient") as mock_builder:
        conn = KafkaConnectConnection(MagicMock())
        client = conn.client
