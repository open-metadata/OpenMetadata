# ingestion/tests/unit/source/pipeline/test_kafkaconnect.py
import pytest
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"collection.name.format": "coll_${topic}", "table.name.format": "tab_${topic}"},
            "aw.Person", "coll_aw_person", True
        ),
        # Scenario 2: Fallback to table.name.format
        (
            {"table.name.format": "bronze_${topic}"},
            "aw.Person.Address", "bronze_aw_person_address", True
        ),
        ...
        (
            {"table.name.format": "bronze_${topic}"},
            "aw.Person", "gold_aw_person", False
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("sink", {"table.name.format": "bronze_${topic}"}, "bronze_aw_person_person", "aw.Person.Person", True),
