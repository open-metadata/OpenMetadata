...
"""Unit tests for Spline connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.pipeline.spline.connection"


def test_spline_connection_is_base_connection():
    assert issubclass(SplineConnection, BaseConnection)


def test_get_client_builds_the_client():
    with patch(f"{CONNECTION_MODULE}.SplineClient") as mock_builder:
        conn = SplineConnection(MagicMock())
        client = conn.client
