...
"""Unit tests for NiFi connection handling."""
...
CONNECTION_MODULE = "metadata.ingestion.source.pipeline.nifi.connection"


def test_nifi_connection_is_base_connection():
    assert issubclass(NifiConnection, BaseConnection)


def test_get_client_builds_the_client():
    with patch(f"{CONNECTION_MODULE}.NifiClient") as mock_builder:
        conn = NifiConnection(MagicMock())
        client = conn.client
