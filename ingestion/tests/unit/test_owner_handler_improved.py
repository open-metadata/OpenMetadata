...
class TestOwnerHandlerImproved:
    ...
    def mock_metadata(self):
        ...
        user_ref = EntityReference(
            id="user-123",
            name="john.doe",
            type="user"
        )
        metadata.get_reference_by_name.return_value = EntityReferenceList(root=[user_ref])
        metadata.get_reference_by_email.return_value = EntityReferenceList(root=[user_ref])
    ...
    def test_strategy_pattern(self, mock_metadata):
        ...
        handler2 = OwnerHandler(
            metadata=mock_metadata,
            owner_config=config_config_first
        )
...
class TestBackwardCompatibility:
    ...
    def test_simple_mode_compatibility(self, mock_metadata):
        ...
        owner_ref = handler.resolve_owner(
            entity_level=EntityLevel.TABLE,
            entity_name="users"
        )
