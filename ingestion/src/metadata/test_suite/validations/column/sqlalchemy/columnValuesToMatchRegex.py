...
class ColumnValuesToMatchRegexValidator(BaseTestHandler, SQAValidatorMixin):
    ...
    def get_match_count(self, column: Column, regex: str):
        ...
        """
        try:
            return self.run_query_results(self.runner, Metrics.REGEX_COUNT, column, expression=regex)
        except CompileError as err:
            logger.warning(f"Could not use `REGEXP` due to - {err}. Falling back to `LIKE`")
            return self.run_query_results(self.runner, Metrics.LIKE_COUNT, column, expression=regex)
