...
def column_value_length_to_be_between(
    ...
    try:
        ...
        max_value_length_value_dict = dict(runner.dispatch_query_select_first(Metrics.MAX_LENGTH.value(col).fn()))
        max_value_length_value_res = max_value_length_value_dict.get(Metrics.MAX_LENGTH.name)
        min_value_length_value_dict = dict(runner.dispatch_query_select_first(Metrics.MIN_LENGTH.value(col).fn()))
        min_value_length_value_res = min_value_length_value_dict.get(Metrics.MIN_LENGTH.name)
    ...
    min_bound = next((float(param.value) for param in test_case.parameterValues if param.name == "minLength"))
    max_bound = next((float(param.value) for param in test_case.parameterValues if param.name == "maxLength"))
    ...
    return TestCaseResult(
        timestamp=execution_date, testCaseStatus=status, result=result,
            testResultValue=[
                    TestResultValue(
                    name="minValueLength",
                    value=str(min_value_length_value_res)
                ),
                    TestResultValue(
                    name="maxValueLength",
                    value=str(max_value_length_value_res)
                )
            ]
    )
