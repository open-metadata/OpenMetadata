...
def column_values_sum_to_be_between(
    ...
    try:
        ...
        sum_value_dict = dict(runner.dispatch_query_select_first(Metrics.SUM.value(col).fn()))
    ...
    min_bound = next((float(param.value) for param in test_case.parameterValues if param.name == "minValueForColSum"))
    max_bound = next((float(param.value) for param in test_case.parameterValues if param.name == "maxValueForColSum"))
