...
def column_values_to_match_regex(
    ...
    try:
        ...
        like_count_value_dict = dict(
            runner.dispatch_query_select_first(like_count(col).fn())  # pylint: disable=abstract-class-instantiated
        )
    ...
    result = f"Found {like_count_value_res} value(s) matching regex pattern vs "\
