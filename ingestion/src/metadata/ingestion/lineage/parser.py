...
#   2. CREATE MATERIALIZED VIEW mv_name REFRESH EVERY n HOUR TO target (col1, col2)
#      [DEFINER = user] [SQL SECURITY ...] AS SELECT ...
#   3. ENGINE = ... clauses between TO target and AS SELECT are skipped.
#
# The character class for <target> stops at the first whitespace / opening paren so
# POPULATE, ENGINE = ..., DEFINER = ... and column lists cannot bleed into the capture.
_CLICKHOUSE_MV_TO_RE = re.compile(
    r"^\s*CREATE\s+MATERIALIZED\s+VIEW\s+"
    r"(?:IF\s+NOT\s+EXISTS\s+)?"  # optional IF NOT EXISTS
    r"(?:\S+\s+)"  # skip MV name (any non-whitespace token)
    r"(?:ON\s+CLUSTER\s+\S+\s+)?"  # optional ON CLUSTER <cluster_name>
    r"(?:REFRESH\s+EVERY\s+(?:(?!\bTO\b)[\s\S])*?)?"  # optional REFRESH EVERY n UNIT
    r"TO\s+([\w`\.\[\]\"]+)"  # capture target table -- stops at whitespace / paren
