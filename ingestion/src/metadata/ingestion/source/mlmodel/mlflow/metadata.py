...
class MlflowSource(MlModelServiceSource):
    ...
    def yield_mlmodel(  # pylint: disable=arguments-differ
        ...
        mlmodel_request = CreateMlModelRequest(
            ...
            mlFeatures=self._get_ml_features(run.data, latest_version.run_id, model.name, latest_version.source),
    ...
    def _get_ml_features(  # pylint: disable=arguments-differ
        self, data: RunData, run_id: str, model_name: str, model_source: str | None = None
    ) -> list[MlFeature] | None:
        """
        Extract ML features from the run data or model signature.

        First tries the MLflow 2.x approach (mlflow.log-model.history tag).
        Falls back to reading the model signature via mlflow.models.get_model_info()
        for MLflow 3.x compatibility where the tag was removed.
        """
        if data.tags:
            try:
                history_tag = data.tags.get("mlflow.log-model.history")
                if history_tag:
                    props = json.loads(history_tag)
        ...
        # MLflow 3.x: mlflow.log-model.history tag was removed.
        # Fall back to reading the model signature from the model info.
        # This fallback is outside the `if data.tags:` block so it is
        # reachable even when the run has no tags.
        return self._get_ml_features_from_model_info(model_source, model_name)

    def _get_ml_features_from_model_info(self, model_source: str | None, model_name: str) -> list[MlFeature] | None:
        """Extract features from the model's MLmodel file via get_model_info."""
        if not model_source:
            return None
        ...
        try:
            ...
            model_info = get_model_info(model_source)
            ...
            if signature and signature.inputs and signature.inputs.inputs:
                features = [
                    MlFeature(
                        name=col.name or "",
                        dataType=FeatureType.categorical if col.type == DataType.string else FeatureType.numerical,
                    )
            ...
            logger.debug(traceback.format_exc())
            reason = f"Cannot extract model signature from model info: {exc}"
            logger.warning(reason)
            self.status.warning(model_name, reason)
