...
class MlflowSource(MlModelServiceSource):
    ...
    def _get_latest_version(self, model: RegisteredModel) -> Optional[ModelVersion]:
        ...
        # A searched version has no timestamp worth matching against: the
        # registry's `last_updated_timestamp` tracks the model, not its newest
        # version, so ordering by version number is the only signal left.
        return self._pick_newest(self._search_versions(model.name))
