...
_SAMPLE_SIZE = 10
_SAMPLE_SCAN_LIMIT = 100
_SAMPLE_BYTE_LIMIT = 1024 * 1024
_NO_MESSAGE_FOUND_ERROR = 10037
...
class NatsSource(MessagingServiceSource):
    ...
    def get_topic_list(self) -> Iterable[BrokerTopicDetails]:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        while total is None or offset < total:
            ...
            _raise_for_api_error(response, "Could not list NATS JetStream streams")

            try:
                response_offset = int(response.get("offset", offset))
                total = int(response["total"])
            except (KeyError, TypeError, ValueError) as exc:
                raise NatsApiError("NATS returned an invalid stream-list response") from exc
            if response_offset != offset:
                raise NatsApiError(f"NATS returned stream-list offset {response_offset}, expected {offset}")
            if response.get("missing"):
                raise NatsApiError(f"NATS returned an incomplete stream list: {response['missing']}")

            stream_infos = response.get("streams") or []
            for stream_info in stream_infos:
                config_data = stream_info.get("config") or {}
                state_data = stream_info.get("state") or {}
                stream_name = config_data.get("name")
                if not stream_name:
                    raise NatsApiError("NATS returned stream metadata without a stream name")
            ...
            next_offset = response_offset + len(stream_infos)
            if not stream_infos:
                if next_offset < total:
                    raise NatsApiError("NATS returned an incomplete stream list before the reported total")
                break
            if next_offset <= offset:
                raise NatsApiError("NATS stream-list pagination did not advance")
            offset = next_offset
    ...
    def _fetch_schema_from_kv(self, stream_name: str) -> Optional[tuple]:  # noqa: UP045
        ...
        if error:
            if error.get("err_code") == _NO_MESSAGE_FOUND_ERROR:
                return None
            _raise_for_api_error(
                response,
                f"Could not fetch schema for stream '{stream_name}' from KV bucket '{bucket}'",
            )
        if "message" not in response:
            raise NatsApiError(f"NATS returned an invalid schema response for stream '{stream_name}'")
        raw = response["message"].get("data", "")
        if not raw:
            return None
    ...
    def yield_topic(self, topic_details: BrokerTopicDetails) -> Iterable[Either[CreateTopicRequest]]:
        try:
            logger.debug("Fetching topic details for NATS stream %s", topic_details.topic_name)
    ...
    def _fetch_sample_messages(self, topic_details: BrokerTopicDetails) -> list[str]:
        ...
        messages = []
        sample_bytes = 0
        start_seq = max(first_seq, last_seq - _SAMPLE_SCAN_LIMIT + 1)
        ...
        for seq in range(last_seq, start_seq - 1, -1):
            ...
            if error:
                ...
                _raise_for_api_error(
                    response,
                    f"Could not fetch sequence {seq} from stream '{topic_details.topic_name}'",
                )
            if "message" not in response:
                raise NatsApiError(f"NATS returned an invalid message response for stream '{topic_details.topic_name}'")
            raw = response["message"].get("data", "")
            if not raw:
                continue
            try:
                ...
                logger.debug(
                    "Skipping non-text sample at sequence %s from NATS stream %s",
                    seq,
                    topic_details.topic_name,
                )
                continue
            if sample_bytes + len(decoded_bytes) > _SAMPLE_BYTE_LIMIT:
                break
            messages.append(decoded)
            sample_bytes += len(decoded_bytes)
