...
def _try_parse_json(data_str: str | None) -> dict | None:
    if not data_str:
        return None
    try:
...
def _ts(dt: datetime | None) -> Timestamp | None:
    if dt is None:
        return None
...
def _handle_sync_start(sync: dict, _data_str: str | None, ts: datetime) -> None:
    sync["sync_start_ts"] = ts


def _handle_extract_summary(sync: dict, data_str: str | None, ts: datetime) -> None:
...
def _handle_write_start(sync: dict, _data_str: str | None, ts: datetime) -> None:
    sync["write_start_min"] = min(ts, sync.get("write_start_min", ts))


def _handle_write_end(sync: dict, _data_str: str | None, ts: datetime) -> None:
    sync["write_end_max"] = max(ts, sync.get("write_end_max", ts))


def _handle_sync_end(sync: dict, data_str: str | None, ts: datetime) -> None:
    sync["sync_end_ts"] = ts
    parsed = _try_parse_json(data_str)
    if parsed:
...
def _handle_sync_stats(sync: dict, data_str: str | None, _ts: datetime) -> None:
    parsed = _try_parse_json(data_str)
    if parsed:
