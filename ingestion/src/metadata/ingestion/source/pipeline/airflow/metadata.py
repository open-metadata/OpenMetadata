...
class AirflowSource(PipelineServiceSource):
    ...
    def get_pipelines_list(self) -> Iterable[AirflowDagDetails]:
        ...
        if hasattr(SerializedDagModel, "fileloc"):
            ...
            dag_model_join = (
                session_query.outerjoin
                if self.source_config.includeUnDeployedPipelines
                else session_query.join
            )
    ...
    def _set_observability_context(
        self,
        pipeline_entity: Optional[Pipeline] = None,
        table_fqns: Optional[List[str]] = None,
        latest_dag_run: Optional[DagRun] = None,
    ) -> None:
        """
        ...
        """
        self.context.get().current_pipeline_entity = (
            pipeline_entity  # pyright: ignore[reportAttributeAccessIssue]
        )
        self.context.get().current_table_fqns = (
            table_fqns or []
        )  # pyright: ignore[reportAttributeAccessIssue]
        self.context.get().latest_dag_run = (
            latest_dag_run  # pyright: ignore[reportAttributeAccessIssue]
        )
    ...
    def get_table_pipeline_observability(
        ...
        try:
            ...
            if not (pipeline_entity and table_fqns and latest_dag_run):
                logger.debug(
                    "No observability data to emit for dag %s", pipeline_details.dag_id
                )
                return
