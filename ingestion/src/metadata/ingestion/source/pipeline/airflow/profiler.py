...
class AirflowProfilerSource(PipelineProfilerSource):
    ...
    def __init__(self, config: SourceConfig, metadata, service_name: str):
        ...
        self.service_connection_config: AirflowConnection = config.serviceConnection.root.config
            ...
            for dag in dags:
                try:
                    ...
                    if recent_runs:
                        ...
                        table_fqns = self._extract_table_fqns_from_dag(session, dag.dag_id, latest_run.run_id)
                        ...
                        observability = PipelineObservability(
                            ...
                            schedule={
                                ...
                                "startTime": self._convert_to_om_datetime(dag.start_date) if dag.start_date else None,
                                "endTime": self._convert_to_om_datetime(dag.end_date) if dag.end_date else None,
                            },
                            lastRunTime=self._convert_to_om_datetime(latest_run.execution_date),
    ...
    def _extract_table_fqns_from_dag(self, session: Session, dag_id: str, run_id: str) -> List[str]:
        """
        ...
        try:
            ...
            for task_instance in task_instances:
                ...
                if any(keyword in task_instance.task_id.lower() for keyword in ['table', 'extract', 'transform', 'load']):
