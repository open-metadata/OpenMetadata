...
class NifiSource(PipelineServiceSource):
    ...
    def create(cls, config_dict, metadata: OpenMetadata, pipeline_name: Optional[str] = None):
        ...
        if not isinstance(connection, NifiConnection):
            raise InvalidSourceException(f"Expected NifiConnection, but got {connection}")
        ...
        try:
            return [
                Task(
                    ...
                    sourceUrl=SourceUrl(f"{clean_uri(self.service_connection.hostPort)}{processor.uri}"),
    ...
    def yield_pipeline(self, pipeline_details: NifiPipelineDetails) -> Iterable[Either[CreatePipelineRequest]]:
        """
        ...
        pipeline_request = CreatePipelineRequest(
            ...
            sourceUrl=SourceUrl(f"{clean_uri(self.service_connection.hostPort)}{pipeline_details.uri}"),
    ...
    def yield_pipeline_status(self, pipeline_details: NifiPipelineDetails) -> Iterable[Either[OMetaPipelineStatus]]:
        """
    ...
    def _get_processors_from_process_group(process_group: dict) -> List[NifiProcessor]:
        ...
        """
        processor_list = process_group.get(PROCESS_GROUP_FLOW).get("flow").get("processors")
        ...
        for process_group in self.connection.list_process_groups():
            try:
                nifi_pipeline_details = NifiPipelineDetails(
                    ...
                    name=process_group[PROCESS_GROUP_FLOW][BREADCRUMB][BREADCRUMB].get("name"),
                    uri=process_group[PROCESS_GROUP_FLOW].get("uri"),
                    processors=self._get_processors_from_process_group(process_group=process_group),
                    connections=self._get_connections_from_process_group(process_group=process_group),
                ...
                self.process_group_connections.extend(self.get_process_group_connections(process_group))
