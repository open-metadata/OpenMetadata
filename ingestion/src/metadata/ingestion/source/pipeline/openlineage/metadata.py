...
def deaggregate_kinesis_record(data: bytes) -> list[bytes]:
    ...
    if len(data) < len(KPL_AGGREGATED_MAGIC) + KPL_MD5_DIGEST_SIZE:
        raise ValueError(
            "Truncated KPL-aggregated Kinesis record: missing protobuf body or checksum"
        )
...
class OpenlineageSource(PipelineServiceSource):
    ...
    def prepare(self):
        ...
        self._db_service_type_map: Dict[
            str, str
        ] = self._build_db_service_type_map()  # noqa: UP006
    ...
    def _symlink_identifiers(data: Dict) -> List[Dict]:  # noqa: UP006
        ...
        identifiers = (
            symlinks.get("identifiers") if isinstance(symlinks, dict) else None
        )
        if not isinstance(identifiers, list):
            return []
        return [
            identifier for identifier in identifiers if isinstance(identifier, dict)
        ]
    ...
    def _iter_table_candidates(
        cls, data: Dict
    ) -> List[Tuple[TableDetails, str]]:  # noqa: UP006
        """
    ...
    def _resolve_table(
        self, data: Dict
    ) -> Optional[ResolvedTable]:  # noqa: UP006, UP045
        """
    ...
    def _resolve_table_uncached(
        self, data: Dict, ol_name: str
    ) -> Optional[ResolvedTable]:  # noqa: UP006, UP045
        """
    ...
    @staticmethod
    def _log_unmatched_dataset(
        ol_name: str, attempts: List[str]
    ) -> None:  # noqa: UP006
        logger.warning(
    ...
    def _find_service_fqn_by_broker(
        self, broker_hostname: str
    ) -> Optional[str]:  # noqa: UP045
        """
    ...
    def _get_topic_entity(
        self, topic_details: TopicDetails
    ...
    def _build_ol_name_to_fqn_map(self, tables: List):  # noqa: UP006
        ...
        for table in tables:
            ...
            for namespace, name in self._raw_table_identities(table):
                if not name:
                    continue
                ol_name = self._get_ol_table_name(
    ...
    def _get_column_lineage(
        ...
        for table in outputs:
            ...
            facets = (
                table.get("facets") if isinstance(table.get("facets"), dict) else None
            )
            column_lineage_facet = (
                facets.get("columnLineage") if isinstance(facets, dict) else None
            )
            fields = (
                column_lineage_facet.get("fields")
                if isinstance(column_lineage_facet, dict)
                else None
            )
    ...
    def yield_pipeline_lineage_details(
        self, pipeline_details: OpenLineageEvent
    ) -> Iterable[Either[AddLineageRequest]]:  # noqa: C901
                ...
                if entity_details.entity_type == "table":
                    ...
                    if resolved:
                        ...
                        if table_entity:
                            ...
                            logger.warning(
                                f"Table entity not found for: {resolved.fqn}"
                            )
    ...
    def _poll_kafka(self, broker: KafkaBrokerConfig) -> Iterable[OpenLineageEvent]:
        ...
        try:
            ...
            logger.debug(traceback.format_exc())
            raise InvalidSourceException(
                f"Failed to read from Kafka: {str(e)}"
            )  # noqa: B904, RUF010
    ...
    def _poll_kinesis(self, broker: KinesisBrokerConfig) -> Iterable[OpenLineageEvent]:
        ...
        try:
                ...
                while shard_iterator and empty_response_time <= session_timeout:
                    ...
                    for record in records:
                        try:
                            ...
                            logger.warning(
                                f"Failed to de-aggregate Kinesis record: {e}"
                            )
                            logger.debug(traceback.format_exc())
                            continue
            ...
            logger.debug(traceback.format_exc())
            raise InvalidSourceException(
                f"Failed to read from Kinesis: {str(e)}"
            )  # noqa: B904, RUF010
