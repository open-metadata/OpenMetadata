...
# Number of table lookups (resolved or not) kept in memory at once
TABLE_LOOKUP_CACHE_SIZE = 1024
...
class DbtcloudSource(PipelineServiceSource):
    ...
    def __init__(self, config: WorkflowSource, metadata: OpenMetadata):
        ...
        # Bounded cache for table entity lookups (hits and misses) to avoid
        # redundant API calls. Hits are also registered on the base-class cache
        # so the observability stage that follows does not re-fetch them.
        self._table_lookup_cache: LRUCache = LRUCache(maxsize=TABLE_LOOKUP_CACHE_SIZE)
    ...
    def _get_table_entity(self, table_fqn: str) -> Optional[Table]:
        ...
        if table_fqn not in self._table_lookup_cache:
            ...
            self._table_lookup_cache[table_fqn] = table
            if table:
                self.cache_table_entity(table)
        return self._table_lookup_cache[table_fqn]
    ...
    def _reset_observability_context(self) -> None:
        """
        Blank what the observability stage reads. The topology context is shared
        across sibling jobs while ``latest_run`` is refreshed for every job, so any
        path that leaves this stage without repopulating the context would hand the
        previous job's entity and tables to this job's run -- rewriting rows that
        were already correct.
        """
        ctx = self.context.get()
        ctx.current_pipeline_entity = (  # pyright: ignore[reportAttributeAccessIssue]
            None
        )
        ctx.current_table_fqns = []  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def yield_pipeline_lineage_details(
        ...
        try:  # pylint: disable=too-many-nested-blocks
            # Before anything that can return or raise: the early return below and
            # the except branch both leave this stage without touching the context
            # again.
    ...
    def _yield_column_lineage(
        self, model: DBTModel, to_entity_fqn: str
    ) -> Iterable[Either[AddLineageRequest]]:
        """
        ...
        try:
            ...
            logger.debug(traceback.format_exc())
            logger.warning(
                f"Failed to parse compiled SQL for column lineage of model {model.name}: {exc}"
            )
    ...
    def get_table_pipeline_observability(
        ...
        try:
            ...
            if not (pipeline_entity and table_fqns and latest_run):
                logger.debug(
                    "No observability data to emit for job %s", pipeline_details.id
                )
                return
