...
DYNAMODB_STREAM_ARN_PATTERN = r"arn:aws:dynamodb:([^:]+):([^:]+):table/([^/]+)/stream/.*"
...
class FirehoseSource(PipelineServiceSource):
    ...
    def get_pipelines_list(self) -> Iterable[dict]:
        try:
            ...
            while has_more:
                ...
                response = self.firehose.list_delivery_streams(**kwargs)
                ...
                for stream_name in streams_response.DeliveryStreamNames:
                    try:
                        stream_details = self.firehose.describe_delivery_stream(
                ...
                if has_more and streams_response.DeliveryStreamNames:
                    exclusive_start_stream_name = (
                        streams_response.DeliveryStreamNames[-1]
                    )
        except Exception as exc:
            logger.debug(traceback.format_exc())
    ...
    def _extract_dynamodb_table_from_stream_arn(
        self, stream_arn: str
    ) -> Optional[str]:
        """
    ...
    def _find_dynamodb_table_entity(
        self, table_name: str
    ...
    def _get_s3_path_from_destination(self, pipeline_details: dict) -> Optional[str]:
        """
        Extract S3 path from destination configuration
        """
        s3_dest = pipeline_details.get("S3DestinationDescription") or pipeline_details.get("ExtendedS3DestinationDescription")
    ...
    def yield_pipeline_lineage_details(
        ...
        """
        Get lineage between DynamoDB, Firehose pipeline and S3
        """
        try:
