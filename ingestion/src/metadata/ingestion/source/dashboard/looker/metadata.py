...
class LookerSource(DashboardServiceSource):
            ...
            try:
                ...
                for model in all_lookml_models:
                    ...
                    if filter_by_project(
                        self.source_config.projectFilterPattern, project_name
                    ):
                        self.status.filter(
                            model.name or project_name,
                            f"Project [{project_name}] filtered out.",
                        )
                        continue
        ...
        try:
            ...
            def resolve(field_name, visited=None):
                ...
                source_cols = {
                    ...
                    for match in table_col_pattern.finditer(sql)
                }
        ...
        try:
            ...
            if view.sql_table_name:
                ...
                sql_table_name = self._render_table_name(
                    sql_table_name, model_name, view_name=view.name
                )
    ...
    def build_lineage_request(
        ...
        to_entity: Union[Dashboard, DashboardDataModel],
        column_lineage: Optional[List[Tuple[str, str]]] = None,
