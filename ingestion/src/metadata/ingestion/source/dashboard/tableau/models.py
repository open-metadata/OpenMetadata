...
from ingestion.src.metadata.ingestion.lineage.models import Dialect
...
class DataSource(BaseModel):
    ...
    def generate_upstream_tables_if_unsupported_custom_sql(cls, values):
        if not values.get("upstreamTables") and values.get("containsUnsupportedCustomSql"):
    ...
    @staticmethod
    def _extract_query_from_fields(fields: Optional[List[DatasourceField]]) -> Optional[str]:
        if not fields:
            return None
