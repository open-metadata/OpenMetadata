...
class TableauSource(DashboardServiceSource):
        ...
        if data_model_entity:
            ...
            for data_model_column in data_model_entity.columns or []:
                ...
                child_column_fqns = [
                    ...
                    if child_column_name.casefold()
                    == model_str(child_column.name).casefold()
                ]
                columns.extend(
                    child_column_fqns
                    or [model_str(data_model_column.fullyQualifiedName)]
                )
        return columns
    ...
    def _get_table_datamodel_lineage(
        ...
        """
        try:
            upstream_column_map = self._build_upstream_column_map(
                data_model=upstream_data_model
            )
    ...
    @staticmethod
    def _get_mirrored_upstream_column(
        field: DatasourceField,
    ) -> Optional[UpstreamColumn]:
        """
    ...
    def get_column_info(self, data_source: DataSource) -> Optional[List[Column]]:
        ...
        for field in data_source.fields or []:
            try:
                ...
                if mirrored_column:
                    parsed_fields["dataTypeDisplay"] = (
                        mirrored_column.remoteType or DataType.UNKNOWN.value
                    )
                    parsed_fields["dataType"] = ColumnTypeParser.get_column_type(
                        mirrored_column.remoteType
                    )
