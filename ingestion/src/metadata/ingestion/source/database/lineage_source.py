...
class LineageSource(QueryParserSource, ABC):
    ...
    def yield_query_lineage(
        ...
        if result_limit is not None:
            self.manual_progress.seed_scope_total("Queries", "run", result_limit)
        produced = 0
        ...
        def producer_fn():
            ...
            for table_query in self.query_lineage_producer():
                produced += 1
                self.manual_progress.track("Queries")
                yield table_query
        ...
        self.manual_progress.reconcile_scope_total("Queries", "run", produced)
