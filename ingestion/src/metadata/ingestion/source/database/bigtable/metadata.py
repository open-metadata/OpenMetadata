...
class BigtableSource(CommonNoSQLSource, MultiDBSource):
    ...
    def get_schema_name_list(self) -> list[str]:
        ...
        try:
            ...
            logger.error(f"Failed to list BigTable instances in project {project_id}: {err}")  # noqa: G004
            raise
    ...
    def query_table_names_and_types(self, schema_name: str) -> Iterable[TableNameAndType]:
        ...
        try:
            ...
            logger.error(f"Failed to list BigTable table names in {project_id}.{schema_name}: {err}")  # noqa: G004
        return []
    ...
    def get_table_columns_dict(self, schema_name: str, table_name: str) -> list[dict] | dict:
        ...
        try:
            ...
            logger.debug(traceback.format_exc())
            logger.warning(f"Failed to read BigTable rows for [{project_id}.{schema_name}.{table_name}]: {err}")  # noqa: G004
        return []
