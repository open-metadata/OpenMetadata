...
class TrinoTableParameter(BaseTableParameter):
    ...
    The base get_data_diff_url returns the connection dict as-is when
    the connection class provides one (TrinoConnection.get_connection_dict).
    That dict carries the *service-level* catalog and no schema, but
    data_diff needs the *table-specific* catalog and schema so that the
    Trino session targets the correct catalog for each table in the diff.
    """
