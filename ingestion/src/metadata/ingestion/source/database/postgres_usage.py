    ...
    def get_table_query(self) -> Optional[Iterable[TableQuery]]:
        ...
        try:
            if self.config.sourceConfig.config.queryLogFilePath:
                df = pd.read_csv(
                    self.config.sourceConfig.config.queryLogFilePath, header=None
                )
                d = df.copy()
                ...
                d = (
                    d[(~d.user_name.isnull()) & (d.command_tag == "SELECT")]
                    .drop(["query"], axis=1)
                    .rename(
                        columns={
                            "message": "query",
                            "log_time": "end_time",
                            "sql_state_code": "aborted",
                        }
                    )
                )
                ...
                dcl = d[
                    [
                        ...
                        "user_name",
                        "end_time",
                        "aborted",  #'start_time', 'end_time', 'aborted'
                    ]
                ].copy()
