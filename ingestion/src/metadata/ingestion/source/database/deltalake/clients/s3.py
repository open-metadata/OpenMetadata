...
class DeltalakeS3Client(DeltalakeBaseClient):
    ...
    @classmethod
    def from_config(cls, service_connection: DeltaLakeConnection) -> "DeltalakeS3Client":
        # Get the credentials to pass to the storage options
        aws_client = AWSClient(service_connection.configSource.connection.securityConfig)
        ...
        endpoint_url = str(service_connection.configSource.connection.securityConfig.endPointURL)[:-1] \
        ...
        storage_options = {key: value for key, value in storage_options.items() if value}

        return cls(
            client=DatalakeS3Client.from_config(config=service_connection.configSource.connection),
            storage_options=storage_options
        )

    @staticmethod
    def _get_configured_bucket(service_connection: DeltaLakeConnection) -> Optional[str]:
         return service_connection.configSource.bucketName \
    ...
    def _get_table_info(self, schema_name: str, prefix: str) -> Iterable[TableInfo]:
        ...
        if table:
            name = table.metadata().name or [part for part in prefix.split("/") if part][-1] or table.metadata().id
            yield TableInfo(
                schema=schema_name,
                name=name,
                location=prefix,
    ...
    def get_table_info(
        ...
        yield from self._get_table_info(schema_name, self._get_configured_prefix(service_connection))
