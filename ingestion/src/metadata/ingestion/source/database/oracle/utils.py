...
get_oracle_table_prefix = lambda kw: kw.get("oracle_table_prefix", "ALL")
...
def get_table_comment(
    ...
    **kw,
    ...
    return get_table_comment_wrapper(
        ...
        query=ORACLE_TABLE_COMMENTS.format(
            oracle_table_prefix=get_oracle_table_prefix(kw=kw)
        ),
    )
...
def get_view_definition(
    ...
    **kw,
    ...
    return get_view_definition_wrapper(
        ...
        schema=schema.lower() if schema else None,
        query=ORACLE_VIEW_DEFINITIONS.format(
...
def get_columns(self, connection, table_name, schema=None, **kw):
    ...
    if self.server_version_info >= (12,):
        identity_cols = ORACLE_IDENTITY_TYPE.format(
            oracle_table_prefix=get_oracle_table_prefix(kw=kw), dblink=dblink
        )
...
def get_table_names_dialect(self, connection, schema=None, **kw):
    ...
    sql_str = ORACLE_GET_TABLE_NAMES.format(
        oracle_table_prefix=get_oracle_table_prefix(kw=kw), tablespace=tablespace
    )
    cursor = connection.execute(sql.text(sql_str), {"owner": schema})
    return [row[0] for row in cursor]


def get_mview_names(self, schema=None, **kwargs):
    ...
    with self._operation_context() as conn:
        return self.dialect.get_mview_names(
            conn, schema, info_cache=self.info_cache, **kwargs
        )
...
def get_mview_names_dialect(self, connection, schema=None, **kw):
    schema = self.denormalize_name(schema or self.default_schema_name)
    sql_query = sql.text(
        GET_MATERIALIZED_VIEW_NAMES.format(oracle_table_prefix=get_oracle_table_prefix(kw=kw))
    )
