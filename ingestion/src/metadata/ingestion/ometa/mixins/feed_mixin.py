...
class OMetaFeedMixin:
    ...
    def list_threads(
        ...
        task_status: ThreadTaskStatus | None = None,
    ) -> EntityList[Thread]:
        params = {
    ...
    def get_thread(self, thread_id: str | UUID) -> Thread:
        resp = self.client.get(f"{self._feed_path}/{model_str(thread_id)}")
        return Thread.model_validate(resp)

    def get_task_thread(self, task_id: str | int) -> Thread:
    ...
    def create_post(self, thread_id: str | UUID, create_request: CreatePostRequest) -> Post:
        resp = self.client.post(
    ...
    def resolve_feed_task(self, task_id: str | int, resolve_request: ResolveTaskRequest) -> Thread:
    ...
    def close_feed_task(self, task_id: str | int, close_request: CloseTaskRequest) -> Thread:
        resp = self.client.put(
