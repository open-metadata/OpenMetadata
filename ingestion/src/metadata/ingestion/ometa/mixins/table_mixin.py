...
class OMetaTableMixin:
    ...
    def ingest_table_life_cycle_data(
        self, table: Table, life_cycle_data: LifeCycleProperties
        ...
        try:
            resp = self.client.put(
                f"{self.get_suffix(Table)}/{table.fullyQualifiedName.__root__}/lifeCycle",
                data=life_cycle_data.json(),
            )
        except Exception as exc:
            logger.debug(traceback.format_exc())
            logger.warning(
                f"Error trying to PUT life cycle data for {table.fullyQualifiedName.__root__}: {exc}"
            )
        ...
        if resp:
            try:
                ...
                logger.debug(traceback.format_exc())
                logger.warning(
                    f"Unicode Error parsing the life cycle data response from {table.fullyQualifiedName.__root__}: {err}"
                )
            except Exception as exc:
                logger.debug(traceback.format_exc())
                logger.warning(
                    f"Error trying to parse life cycle data results from {table.fullyQualifiedName.__root__}: {exc}"
                )
