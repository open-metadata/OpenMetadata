...
class ColumnValuesToBeUniqueValidator(
    ...
    def _run_results(self, metric: Metrics, column: Column) -> Optional[int]:  # noqa: UP045
        ...
        grouped_cte = (
            select(count.label(UNIQUE_COUNT_VALUE_ALIAS))
            .select_from(self.runner.dataset)
            .group_by(column)
            .cte("grouped_cte")  # type: ignore
        )
        ...
        try:
            ...
            row = self.runner._select_from_dataset(
                grouped_cte,
                func.sum(grouped_cte.c[UNIQUE_COUNT_VALUE_ALIAS]).label(Metrics.valuesCount.name),
