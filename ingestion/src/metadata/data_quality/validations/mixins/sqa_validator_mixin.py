...
class SQAValidatorMixin:
    ...
    #     This method constructs queries for both top N dimensions and "Others" aggregation.
    #     For DIMENSIONAL queries, uses an intermediate CTE to materialize metrics and impact
    #     score once, avoiding expensive duplicate calculations in ORDER BY clause.
    #     For OTHERS queries, returns direct aggregation (no intermediate CTE needed).
    ...
    #         query_type: DIMENSIONAL (top N with sorting) or OTHERS (single aggregate row)
    ...
    #     if query_type == DataQualityQueryType.DIMENSIONAL:
    #         # OPTIMIZATION: Use intermediate CTE to materialize metrics and impact score
    #         # This prevents duplicate calculation of expensive expressions (percentile, impact)
    #         # in the ORDER BY clause. The CTE computes everything once, then we sort by
    #         # simple column references.
    #         query = query.group_by(dimension_expr)

    #         # Create CTE with all computed metrics
    #         metrics_cte = query.cte("dimension_metrics")

    #         # Final query: Simple column references only (no complex expressions)
    #         final_query = (
    #             select([metrics_cte])
    #             .order_by(
    #                 metrics_cte.c[DIMENSION_IMPACT_SCORE_KEY].desc(),
    #                 metrics_cte.c[DIMENSION_VALUE_KEY].asc()
    #             )
    #             .limit(DEFAULT_TOP_DIMENSIONS + 1)
    #         )
    #         return final_query
    #     else:
    #         # OTHERS query: No GROUP BY dimension (aggregates all filtered rows)
    #         # No ORDER BY needed (single row result)
    #         # Return direct query without intermediate CTE
    #         return query
