...
"""
Pipeline Profiler Workflow Definition
"""
...
class PipelineProfilerWorkflow(IngestionWorkflow):
    """
    Pipeline Profiler ingestion workflow implementation.
    """
    ...
    def _get_source(self) -> Source:
        # Source that we are ingesting, e.g., airflow, dagster, kafkaconnect
        source_type = self.config.source.type.lower()
        if not self.config.source.serviceName:
            raise WorkflowExecutionError(
                "ServiceName is required field for executing the Pipeline Profiler Workflow. "
    ...
    def _get_stage(self) -> Stage:
        stage_type = self.config.stage.type.lower()
        stage_class = import_stage_class(stage_type)
    ...
    def _get_bulk_sink(self) -> BulkSink:
        bulk_sink_type = self.config.bulkSink.type.lower()
        bulk_sink_class = import_bulk_sink_type(bulk_sink_type)
        ...
        logger.debug(f"BulkSink Type: {bulk_sink_type}, {bulk_sink_class} configured")
        return bulk_sink
