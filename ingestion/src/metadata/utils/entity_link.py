...
def get_decoded_column(entity_link: str) -> str:
    ...
    Examples:
        >>> get_decoded_column("<#E::om::table::om::rds.dev.dbt_jaffle.column_w_space::om::columns::om::first_name>")
        'first name'
        >>> get_decoded_column("<#E::om::table::om::rds.dev.dbt_jaffle.column_w_space::om::columns::om::随机的>")
        '随机的'
        >>> get_decoded_column("<#E::om::table::om::rds.dev.dbt_jaffle.table_w_space>")
        ''
    ...
    return unquote_plus(entity_link.split("::om::")[-1].replace(">", ""))
...
def get_entity_link(entity_type: Any, fqn: str, **kwargs) -> str:
    ...
    if not func:
        return f"<#E::om::{ENTITY_REFERENCE_TYPE_MAP[entity_type.__name__]}::om::{fqn}>"
...
def _(fqn: str, column_name: Optional[str] = None) -> str:
    ...
    if column_name:
        entity_link = f"<#E::om::{ENTITY_REFERENCE_TYPE_MAP[Table.__name__]}::om::{fqn}::om::columns::om::{column_name}>"
    else:
        entity_link = f"<#E::om::{ENTITY_REFERENCE_TYPE_MAP[Table.__name__]}::om::{fqn}>"
    return entity_link
