...
def build_query_filter(filters: list[tuple[Column, str, Any]], or_filter: bool = False) -> BinaryExpression | None:
    ...
    for filter_ in filters:
        ...
        try:
            ...
            logger.error(f"Error when looking for operator {operator} - {err}")  # noqa: G004
        else:
...
def get_integer_range_filter(partition_field, integer_range_start, integer_range_end) -> BinaryExpression | None:
...
def get_value_filter(partition_field, values) -> BinaryExpression | None:
...
def get_query_filter_for_runner(kwargs: dict) -> BinaryExpression | None:
...
def get_query_group_by_for_runner(kwargs: dict) -> BinaryExpression | None:
...
def handle_array(query: Query, column: Column, table: type | AliasedClass) -> Query:
