...
_pii_column_name_regexes: Mapping[PIITag, str | list[str]] = {
    ...
    # "timestamp" is excluded via the negative lookahead: event_timestamp /
    # created_timestamp / updated_timestamp are technical audit fields, not personal dates.
    PIITag.DATE_TIME: r"^(?!.*timestamp).*(date|time|dob|birthday|dod).*$",
