...
class CustomRecognizerFactory:
    ...
    def create_recognizer(
        ...
        if isinstance(config, PatternRecognizer):
            ...
            return CustomRecognizerFactory._create_deny_list_recognizer(
                config, recognizer_config, tag_name
            )
        elif isinstance(config, ContextRecognizer):
            return CustomRecognizerFactory._create_context_recognizer(
                config, recognizer_config, tag_name
            )
        elif isinstance(config, CustomRecognizer):
            return CustomRecognizerFactory._create_custom_recognizer(
                config, recognizer_config, tag_name
            )
        elif isinstance(config, PredefinedRecognizer):
            return CustomRecognizerFactory._create_predefined_recognizer(
                config, recognizer_config, tag_name
            )
        else:
    ...
    def _create_deny_list_recognizer(
        ...
        for value in config.denyList:
            ...
            if config.caseSensitive:
                pattern_regex = f"\\b{escaped_value}\\b"
            else:
    ...
    def create_recognizers_for_tag(tag: Tag) -> List[EntityRecognizer]:
        ...
        for recognizer_config in tag.recognizers:
            recognizer = CustomRecognizerFactory.create_recognizer(
                recognizer_config, tag.name
            )
            if recognizer:
...
class RecognizerRegistry:
    ...
    def register_tag_recognizers(self, tag: Tag) -> None:
        ...
        self.recognizers[tag_fqn] = CustomRecognizerFactory.create_recognizers_for_tag(
            tag
        )
