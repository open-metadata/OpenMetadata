...
class BigQueryProfilerSource(BaseProfilerSource):
    ...
    def _copy_service_config(self, config: OpenMetadataWorkflowConfig, database: DatabaseService) -> BigQueryConnection:
        ...
        if isinstance(config_copy.credentials.gcsConfig, GcsCredentialsValues):
            if isinstance(config_copy.credentials.gcsConfig.projectId, MultipleProjectId):
