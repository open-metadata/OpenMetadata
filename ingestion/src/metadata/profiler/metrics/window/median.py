...
class Median(StaticMetric):
    ...
    def fn(self):
        ...
        if is_quantifiable(self.col.type):
            ...
            return MedianFn(column(self.col.name), self.col.table.fullname if self.col.table is not None else None, 0.5)

        if is_concatenable(self.col.type):
            return MedianFn(LenFn(column(self.col.name)), self.col.table.fullname if self.col.table is not None else None, 0.5)
