...
def add_root_prefix(
    parameters: Dict[str, Any], root_attribute_name: str
) -> Dict[str, Any]:
    params = {}

    for key, value in parameters.items():
        if not key.startswith(root_attribute_name):
...
class RootModelFactory(factory.Factory):
    ...
    def _generate(cls, strategy, params):
        return super()._generate(
            strategy, add_root_prefix(params, cls._meta.root_attribute_name)
        )
...
class RootSubFactory(factory.SubFactory):
    ...
    def evaluate(self, instance, step, extra):
        return super().evaluate(
            instance, step, add_root_prefix(extra, self.root_attribute_name)
        )
