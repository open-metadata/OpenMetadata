...
from airflow_provider_openmetadata.lineage.config.providers import (
    provider_config_registry, InvalidAirflowProviderException,
)
...
def parse_airflow_config(airflow_service_name: str) -> AirflowLineageConfig:
    ...
    if auth_provider_type == AuthProvider.no_auth.value:
        ...
        load_security_config_fn = provider_config_registry.registry.get(auth_provider_type)
        if not load_security_config_fn:
            raise InvalidAirflowProviderException(f"Cannot find {auth_provider_type} in airflow providers registry.")
