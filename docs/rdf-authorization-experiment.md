...
| `openmetadata-integration-tests/…/it/tests/RdfAuthorizationAlignmentIT.java` | Real authorization alignment across six phases, and a fail-closed regression on live projections. |
...
14. **Review pass on coverage and wording (2026-09-18).** A code review found that the experiment claimed every unsupported fact rejects the build while several families had no test, and that two descriptions did not match the code.
    ...
    - **One source of truth for the mapping.** The integration test asserted against a hand-kept copy of the mapped vocabulary, which could drift from the builder. `SanitizedModelBuilder` now exposes `mappedPredicates()` and `approvedTypes()`, and the test reads those; the ontology prefix is the builder's constant in both test classes.
    - **The integration test moved to `org.openmetadata.it.tests`,** where [`.claude/rules/java.md`](../.claude/rules/java.md) requires integration tests to live and where every other test in that module already sits. It had been placed in the builder's package to reach package-private types; those experiment types, all of them test-jar classes, are now public instead. It declares `@Execution(ExecutionMode.SAME_THREAD)` rather than the conventional `CONCURRENT`, like the RDF-related `RdfRelationExclusionsIT`, because both of its tests await the same asynchronous projection of a shared graph. **Not verified by a run:** the module compiles, but the integration test has not been re-run since these changes.
