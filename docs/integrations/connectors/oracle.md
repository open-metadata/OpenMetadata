---
description: >-
  This guide will help you install and configure the Oracle connector and run
...
Using the OpenMetadata Oracle connector requires supporting services and software. Please ensure that your host system meets the requirements listed below. Then continue to follow the procedure for installing and configuring this connector.
...
Ensure that you have the Oracle client libraries. You can download the [Oracle Client Libraries](https://help.ubuntu.com/community/Oracle%20Instant%20Client) from here.

## **Procedure**

Here’s an overview of the steps in this procedure. Please follow the steps relevant to your use case.

1. [Prepare a Python virtual environment](oracle.md#1.-prepare-a-python-virtual-environment)
2. [Install the Python module for this connector](oracle.md#2.-install-the-python-module-for-this-connector)
3. [Create a configuration file using template JSON](oracle.md#3.-create-a-configuration-file-using-template-json)
4. [Configure service settings](oracle.md#4.-configure-service-settings)
5. [Enable/disable the data profiler](oracle.md#5.-enable-disable-the-data-profiler)
6. [Install the data profiler Python module (optional)](oracle.md#6.-install-the-data-profiler-python-module-optional)
7. [Configure data filters (optional)](oracle.md#7.-configure-data-filters-optional)
8. [Configure sample data (optional)](oracle.md#8.-configure-sample-data-optional)
9. [Configure DBT (optional)](oracle.md#9.-configure-dbt-optional)
10. [Confirm sink settings](oracle.md#10.-confirm-sink-settings)
11. [Confirm metadata\_server settings](oracle.md#11.-confirm-metadata\_server-settings)
12. [Run ingestion workflow](oracle.md#12.-run-ingestion-workflow)
...
Once the virtual environment is set up and activated as described in Step 1, run the following command to install the Python module for the Oracle connector.
...
Create a new file called `oracle.json` in the current directory. Note that the current directory should be the `openmetadata` directory you created in Step 1.

Copy and paste the configuration template below into the `oracle.json` file you created.
...
```javascript
{
  "source": {
    "type": "oracle",
    "config": {
      "host_port": "hostname.domain.com:1521",
      "username": "username",
...
In this step we will configure the Oracle service settings required for this connector. Please follow the instructions below to ensure that you’ve configured the connector to read from your Oracle service as desired.

#### **host\_port**

Edit the value for `source.config.host_port` in `oracle.json` for your Oracle deployment. Use the `host:port` format illustrated in the example below.

```javascript
"host_port": "hostname.domain.com:1521"
```

Please ensure that your Oracle deployment is reachable from the host you are using to run metadata ingestion.

#### **username**

Edit the value for `source.config.username` to identify your Oracle user.
...
Edit the value for `source.config.password` with the password for your Oracle user.
...
OpenMetadata uniquely identifies services by their `service_name`. Edit the value for `source.config.service_name` with a name that distinguishes this deployment from other services, including other Oracle services that you might be ingesting metadata from.

```javascript
"service_name": "local_oracle"
```

#### **o**racle\_service\_name

Oracle Service Name is the TNS alias used to remotely connect to your database. This Service name is recorded in tnsnames. Edit the value for `source.config.oracle_service_name` key as illustrated in the example below.
...
The syntax and semantics for `schema_filter_pattern` are the same as for [`table_filter_pattern`](oracle.md#table\_filter\_pattern-optional). Please check that section for details.
...
You need not make any changes to the fields defined for `sink` in the template code you copied into `oracle.json` in Step 4. This part of your configuration file should be as follows.
...
You need not make any changes to the fields defined for `metadata_server` in the template code you copied into `oracle.json` in Step 4. This part of your configuration file should be as follows.
...
Your `oracle.json` configuration file should now be fully configured and ready to use in an ingestion workflow.

To run an ingestion workflow, execute the following command from the `openmetadata` directory you created in Step 1.

```
metadata ingest -c ./oracle.json
```

## **Next Steps**

As the ingestion workflow runs, you may observe progress both from the command line and from the OpenMetadata user interface. To view the metadata ingested from Oracle, visit [http://localhost:8585/explore/tables](http://localhost:8585/explore/tables). Select the Oracle service to filter for the data you’ve ingested using the workflow you configured and ran following this guide. The image below provides an example.
...
When attempting to install the `openmetadata-ingestion[oracle]` Python package in Step 2, you might encounter the following error. The error might include a mention of a Rust compiler.
...
Then re-run the install command in [Step 2](oracle.md#2.-install-the-python-module-for-this-connector).
...
Max retries exceeded with url: /api/v1/services/databaseServices/name/local_oracle 
...
Then re-run the metadata ingestion workflow in [Step 12](oracle.md#12.-run-ingestion-workflow).
