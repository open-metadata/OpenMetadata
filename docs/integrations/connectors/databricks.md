...
Using the OpenMetadata Databricks connector requires supporting services and software. Please ensure your host system meets the requirements listed below. Then continue to follow the procedure for installing and configuring this connector.
...
1. [Prepare a Python virtual environment](databricks.md#1.-prepare-a-python-virtual-environment)
2. [Install the Python module for this connector](databricks.md#install-from-pypi-or-source)
3. [Create a configuration file using template JSON](databricks.md#3.-create-a-configuration-file-using-template-json)
4. [Configure service settings](databricks.md#4.-configure-service-settings)
5. [Enable/disable the data profiler](databricks.md#5.-enable-disable-the-data-profiler)
6. [Install the data profiler Python module (optional)](databricks.md#6.-install-the-data-profiler-python-module-optional)
7. [Configure data filters (optional)](databricks.md#7.-configure-data-filters-optional)
8. [Configure sample data (optional)](databricks.md#8.-configure-sample-data-optional)
9. [Configure DBT (optional)](databricks.md#9.-configure-dbt-optional)
10. [Confirm sink settings](databricks.md#10.-confirm-sink-settings)
11. [Confirm metadata\_server settings](databricks.md#11.-confirm-metadata\_server-settings)
12. [Run ingestion workflow](databricks.md#run-manually)
...
Once the virtual environment is set up and activated as described in Step 1, run the following command to install the Python module for the Databricks connector.
...
Create a new file called `databricks.json` in the current directory. Note that the current directory should be the `openmetadata` directory you created in Step 1.

Copy and paste the configuration template below into the `databricks.json` file you created.

{% hint style="info" %}
Note: The `source.config` field in the configuration JSON will include the majority of the settings for your connector. In the steps below we describe how to customize the key-value pairs in the `source.config` field to meet your needs.
{% endhint %}

{% code title="databricks.json" %}
```json
{
  "source": {
    "type": "databricks",
    "config": {
      ...
      "schema_filter_pattern": {
        "excludes": ["databricks.*", "information_schema.*", "performance_schema.*", "sys.*"]
      }
    }
  },
  "sink": {
...
In this step we will configure the Databricks service settings required for this connector. Please follow the instructions below to ensure that you've configured the connector to read from your Databricks service as desired.

#### host\_port

Edit the value for `source.config.host_port` in `databricks.json` for your Databricks deployment. Use the `host:port` format illustrated in the example below.

```json
"host_port": "hostname.domain.com:10000"
```

Please ensure that your Databricks deployment is reachable from the host you are using to run metadata ingestion.

#### token

Edit the value for `source.config.token` to identify your Databricks user. The value of `token` is used in place of a `username` and `password`.
...
Edit the value for `source.config.scheme` in `databricks.json` for your Databricks deployment. Use the `scheme` format illustrated in the example below.
...
OpenMetadata uniquely identifies services by their `service_name`. Edit the value for `source.config.service_name` with a name that distinguishes this deployment from other services, including other Databricks services that you might be ingesting metadata from.
...
Edit the value for `source.config.connect_args` as shown in the example below.
...
The syntax and semantics for `schema_filter_pattern` are the same as for [`table_filter_pattern`](databricks.md#table\_filter\_pattern-optional). Please check that section for details.
...
You need not make any changes to the fields defined for `sink` in the template code you copied into `databricks.json` in Step 4. This part of your configuration file should be as follows.
...
You need not make any changes to the fields defined for `metadata_server` in the template code you copied into `databricks.json` in Step 4. This part of your configuration file should be as follows.
...
Your `databricks.json` configuration file should now be fully configured and ready to use in an ingestion workflow.

To run an ingestion workflow, execute the following command from the `openmetadata` directory you created in Step 1.

```bash
metadata ingest -c ./databricks.json
```

## Next Steps

As the ingestion workflow runs, you may observe progress both from the command line and from the OpenMetadata user interface. To view the metadata ingested from Databricks, visit [http://localhost:8585/explore/tables](http://localhost:8585/explore/tables). Select the Databricks service to filter for the data you've ingested using the workflow you configured and ran following this guide. The image below provides an example.
...
When attempting to install the `openmetadata-ingestion[databricks]` Python package in Step 2, you might encounter the following error. The error might include a mention of a Rust compiler.
...
Then re-run the install command in [Step 2](databricks.md#install-from-pypi-or-source).
...
Max retries exceeded with url: /api/v1/services/databaseServices/name/local_databricks 
