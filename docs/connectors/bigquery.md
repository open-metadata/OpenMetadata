---
description: >-
  This guide will help you install and configure the BigQuery connector and run
...
The syntax and semantics for `schema_filter_pattern` are the same as for [`table_filter_pattern`](bigquery.md#table\_filter\_pattern-optional). Please check that section for details.
...
Then re-run the install command in [Step 2](bigquery.md#2.-install-the-python-module-for-this-connector).
...
Then re-run the metadata ingestion workflow in [Step 12](bigquery.md#12.-run-ingestion-workflow).
