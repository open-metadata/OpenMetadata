---
description: >-
  This guide will help you install and configure the MSSQL connector and run
...
1. [Prepare a Python virtual environment](mssql.md#1.-prepare-a-python-virtual-environment)
2. [Install the Python module for this connector](mssql.md#2.-install-the-python-module-for-this-connector)&#x20;
3. [Create a configuration file using template JSON](mssql.md#3.-create-a-configuration-file-using-template-json)&#x20;
4. [Configure service settings](mssql.md#4.-configure-service-settings)&#x20;
5. [Enable/disable the data profiler](mssql.md#5.-enable-disable-the-data-profiler)&#x20;
6. [Install the data profiler Python module (optional)](mssql.md#6.-install-the-data-profiler-python-module-optional)&#x20;
7. [Configure data filters (optional)](mssql.md#7.-configure-data-filters-optional)&#x20;
8. [Configure sample data (optional)](mssql.md#8.-configure-sample-data-optional)&#x20;
9. [Configure DBT (optional)](mssql.md#9.-configure-dbt-optional)&#x20;
10. [Confirm sink settings](mssql.md#10.-confirm-sink-settings)&#x20;
11. [Confirm metadata\_server settings](mssql.md#11.-confirm-metadata\_server-settings)&#x20;
12. [Run ingestion workflow](mssql.md#12.-run-ingestion-workflow)
...
Create a new file called `mssql.json` in the current directory. Note that the current directory should be the `openmetadata` directory you created in Step 1.
...
The syntax and semantics for `schema_filter_pattern` are the same as for [`table_filter_pattern`](mssql.md#table\_filter\_pattern-optional). Please check that section for details.
...
You need not make any changes to the fields defined for `sink` in the template code you copied into `mssql.json` in Step 4. This part of your configuration file should be as follows.
...
You need not make any changes to the fields defined for `metadata_server` in the template code you copied into `mssql.json` in Step 4. This part of your configuration file should be as follows.
...
Then re-run the install command in [Step 2](mssql.md#2.-install-the-python-module-for-this-connector).
...
Then re-run the metadata ingestion workflow in [Step 12](mssql.md#12.-run-ingestion-workflow).
