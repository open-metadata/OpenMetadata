...
Two other CI checks had separate causes. Sonar discarded 47 Airflow coverage paths after an obsolete `/github/workspace` rewrite, although the tests passed and reported 97% delete-module / 86% deploy-module coverage. Coverage filenames now resolve relative to the scanner's module directory. The Collate compatibility build was blocked by its dependency on `DataInsightsExtension` from [#33079](https://github.com/open-metadata/OpenMetadata/pull/33079). That PR has since merged, and the merge of main at `3db3e3d9f1` brings the dependency into this branch; a fresh compatibility run is pending.
...
### Fresh-runner and local Docker follow-up

The subsequent failures have separate, reproduced causes:

- [PostgreSQL E2E job 102757101837](https://github.com/open-metadata/OpenMetadata/actions/runs/34439596780/job/102757101837) could not resolve the component library when loading browser-helper tests. Shards install dependencies without lifecycle scripts, leaving the linked core library without its compiled `dist`. Planning now builds that library and runs the required browser-helper suite once in isolated Linux containers before shard fan-out. Planning overlaps the distribution build; fixture preparation and shards still require both jobs to succeed.
- [Visual job 102751748540](https://github.com/open-metadata/OpenMetadata/actions/runs/34439596548/job/102751748540) built the UI successfully but rejected the delete-modal screenshot. The old baseline captured a lingering menu and tooltip, and its glossary name came from a random fixture. A dedicated setup/teardown project now supplies a stable glossary and term. The tests require that exact page and a dismissed menu before capture. Only the two affected baselines changed; screenshot thresholds are unchanged.
- [Airflow job 102751695787](https://github.com/open-metadata/OpenMetadata/actions/runs/34439596806/job/102751695787) reported 11 lifecycle failures. The preceding test class deleted the shared Airflow home, leaving new scanner connections with an empty SQLite database. Running that class before deployment reproduced `no such table: dag_bundle` and the 60-second timeout. A session fixture now initializes the database before any test and cleans up only a home it owns. Separately, concurrent deployments forked while `filelock` was changing descriptor ownership. The scanner now starts with `spawn`; a real file-lock regression reproduces the previous startup blockage and verifies successful DAG registration after repair.

Commit `56a4fcc4da` was built with `mvn -DskipTests clean package -pl openmetadata-dist -am` and exercised against isolated local PostgreSQL, OpenSearch, OpenMetadata, and Airflow containers. Linux Playwright 1.57 ran all 15 visual scenarios 50 times: **750 passed in 8.7 minutes**, with three workers and zero retries. The 15 browser-helper scenarios also ran 50 times: **750 passed in 1.6 minutes**, with three workers and zero retries. The component-library preflight build took 7 seconds. The full Airflow API suite passed **41 tests in 63 seconds** on Airflow 3.3.1, Python 3.12, SQLite, and the failing CI's `filelock` 3.32.6; the original failing test order also passed separately. CI uses Python 3.10, which still needs its fresh runner validation.

[CodeQL check 102771779486](https://github.com/open-metadata/OpenMetadata/pull/33088/checks?check_run_id=102771779486) subsequently flagged the new component-library dependency install executing on the workflow runner. Dependency installation now runs in a restricted container with a bounded temporary package cache. Dependencies and build outputs live under `RUNNER_TEMP`; repository source stays read-only. The build and browser checks consume those outputs without network access. A clean Linux dependency install, library build, and all 15 browser-helper tests pass after the main merge. Runtime probes verify that runner credentials are absent, checkout writes are blocked, and the build/test containers have only loopback networking. No CodeQL alert is suppressed.

[Planner job 102774326621](https://github.com/open-metadata/OpenMetadata/actions/runs/34447070409/job/102774326621) then exposed missing Docker mount targets in a fresh checkout. Docker exited 125 before dependency installation because it could not create the core library's `node_modules` inside the read-only workspace. The previous local checkout already contained that directory. The workflow now creates empty `node_modules` and `dist` mount targets before applying the read-only mount. A checkout exported from Git reproduced the original failure; the corrected steps completed dependency installation, the library build, and all 15 browser-helper tests with zero retries. The checkout directories remain empty; generated files stay in the temporary runtime directory.

These local repetitions reuse a seeded server and are not 50 fresh environments or the complete 4,571-instance product suite. Moving the shared helper check removes duplicate executions across shards, but queue throughput and cold/warm GitHub acceptance remain unmeasured.
