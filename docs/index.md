...
| `bootstrap/MIGRATION_SYSTEM.md` | DB-migration architecture — baseline→native→extension execution order, the 2.0 upgrade gate, `SERVER_CHANGE_LOG` step tracking, file layout (no Flyway) | Before adding/debugging a migration under `bootstrap/sql/migrations/`, or reasoning about ordering / tracking tables / MySQL+Postgres dual paths | 2025-10-28 | CURRENT ⚠⁵ |
...
5. **MIGRATION_SYSTEM** — rewritten for the 2.0 baseline: `baseline/`+`native/` layout, the 2.0 upgrade gate, `SERVER_CHANGE_LOG` step columns, and `SqlStatementSplitter` in place of the Flyway parsers. The `extensions/` dir is still not materialized on disk (`extensionPath: ""`) though the path is supported.
