...
| `docs/rdf-fuseki-image-release.md` | Building, publishing and deploying the OpenMetadata Fuseki image (`docker/rdf-store/`) — what it adds over stock Fuseki, the readiness probe and its failures, cutover steps | Building or publishing the Fuseki image; debugging "RDF storage is not ready"; migrating off an upstream Jena | 2026-09-22 | CURRENT |
...
| `docs/adr/2026-09-14-authorized-sparql.md` | Proposed ADR for non-admin read-only SPARQL limited to caller-authorized facts: feasibility findings, options, and the semantics accepted so far | Designing or reviewing authorized SPARQL, #33224, or the ai-platform #1299 chat integration | 2026-09-17 | CURRENT (proposed, not accepted) |
| `docs/rdf-authorization-research.md` | Jena/Fuseki authorization facilities, a version matrix, alternatives compared from primary sources, and the latency decision gate | Choosing an authorized-SPARQL architecture or re-evaluating Fuseki ACLs, filtered views or query rewriting | 2026-09-17 | CURRENT |
| `docs/rdf-authorization-experiment.md` | Test-only sanitized-model experiment: permission map, fixtures, exact commands, live authorization alignment, and "Status for review" listing unsupported facts, contract questions and missing performance evidence | Extending or reviewing the experiment, mapping a new RDF fact family to OM permissions | 2026-09-17 | CURRENT (experiment, not production) |
| `docs/rdf-authorization-scope-proposal.md` | Non-deleted query scope and containment-reference categories for the sanitized model, with the decisions still open | Deciding deleted-entity scope, container identity-only exposure, or reference consistency rules | 2026-09-17 | CURRENT (partly implemented in the experiment) |
...
| `docs/plans/2026-09-14-asset-level-sparql-rbac.md` | Initial small-commit plan for asset-level SPARQL RBAC (#33224) | Historical context only — the ADR checklist and the experiment's "Status for review" are the current direction | 2026-09-14 | SUPERSEDED |
| `docs/plans/2026-09-17-entity-repository-epic.md` | The `EntityRepository` decomposition epic (#32946): why PR #33248 was closed, the measured baseline on `main`, the ~20-PR salvage inventory, the 13-pain list, and the wave order | Before touching `EntityRepository` or picking up any #32946 sub-issue; to find which salvage PR already covers a bug/perf fix from #33248 | 2026-09-17 | CURRENT (active plan) |
