...
public class OpenSearchClient implements SearchClient {
  ...
  private final OpenSearchClientConfig config;
  private Object client; // Will be the actual OpenSearch client once shaded deps are working
  private SearchIndexCrud indexCrud;
  private final ObjectMapper objectMapper;

  public OpenSearchClient(OpenSearchClientConfig config) {
    this.config = config;
  ...
  public void initialize() throws IOException {
    log.info("Initializing OpenSearch client with hosts: {}", config.getHosts());

    // TODO: Initialize actual OpenSearch client once shaded dependencies are resolved
    // this.client = new org.opensearch.client.opensearch.OpenSearchClient(...);
    // this.indexCrud = new OpenSearchIndexCrud(client, config.getClusterAlias());
