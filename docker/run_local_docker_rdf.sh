...
while getopts "m:d:s:x:r:f:h" opt
do
   case "$opt" in
      m ) mode="$OPTARG" ;;
      d ) database="$OPTARG" ;;
...
startFuseki="${startFuseki:=true}"

echo "Running local docker using mode [$mode] database [$database] and skipping maven build [$skipMaven] with cleanDB as [$cleanDbVolumes] and Fuseki [$startFuseki]"
...
docker compose -f docker/development/docker-compose-fuseki.yml down --remove-orphans
...
# Start Fuseki if requested
if [[ $startFuseki == "true" ]]; then
    echo "Starting Apache Jena Fuseki for RDF support"
...
# If RDF is enabled, trigger RDF indexing
if [[ $startFuseki == "true" ]]; then
    echo "✔running RDF reindexing"
    # Trigger RDF ReIndexing from UI  
    ...
    sleep 30 # Wait for RDF indexing to complete
fi
