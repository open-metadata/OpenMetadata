...
{% image src="/images/v1.0/deployment/upgrade/how-does-it-work/upgrade-simple.drawio.png" alt="simple architecture" /%}
...
You can find all the necessary information on how to run the backups [here](https://docs.open-metadata.org/v1.0/deployment/backup-restore-metadata).
...
{% image src="/images/v1.0/deployment/upgrade/how-does-it-work/upgrade-backup.drawio.png" alt="backup" /%}
...
- **Where**: We will restore the data to a clean database with the Flyway migrations at the same version as the backed up data.
  ...
  - We run the migrations ([docs](https://docs.open-metadata.org/v1.0/deployment/bare-metal#4.-prepare-the-openmetadata-database-and-indexes))
     or we start the OpenMetadata server, which will automatically run the migrations.
  - With the server stopped, we [restore](https://docs.open-metadata.org/v1.0/deployment/backup-restore-metadata#restore-metadata) the data.
...
{% image src="/images/v1.0/deployment/upgrade/how-does-it-work/upgrade-migrate.drawio.png" alt="backup" /%}
