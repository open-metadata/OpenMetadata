...
- **Project ID(Optional)**: The BigQuery Project ID is required only if the credentials path is being used instead of values. Now, you can pass multiple Project ID, click [here](/connectors/database/bigquery/cli#1-define-the-yaml-config) for more details.
