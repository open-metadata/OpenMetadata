...
class OrphanTestCaseCleanupTest {
  ...
  void performCleanup_deletesOnlyOrphans() {
    ...
    EntityPolicy<Table> tableRepo = mock(EntityPolicy.class);
    when(tableRepo.reads()).thenReturn(tableReader());
  ...
  private static EntityReader<Table> tableReader() {
    return EntityReadFixture.byName(
        (name, query) -> {
          if (DELETED_TABLE_FQN.equals(name)) {
            throw new EntityNotFoundException("Table instance for %s not found".formatted(name));
          }
          assertEquals(LIVE_TABLE_FQN, name);
          return new Table().withFullyQualifiedName(name);
        });
  }
}
