...
public class DataContractResourceODCSTest extends OpenMetadataApplicationTest {
  ...
  void testImportRetiredContract(TestInfo test) throws IOException {
    ...
    assertEquals(
        ...
        response.getContract().getStatus()); // retired maps to deprecated
  }
  ...
  void testImportInvalidVersion() throws IOException {
    ...
    HttpResponseException exception =
        assertThrows(
            HttpResponseException.class, () -> importODCSContract(table.getId(), odcsYaml, "yaml"));

    assertTrue(
        exception.getMessage().contains("Only ODCS versions v3.0.0, v3.0.1, v3.0.2 are supported"));
  }
  ...
  void testImportWithNonExistentEntity() throws IOException {
    ...
    HttpResponseException exception =
        assertThrows(
            HttpResponseException.class, () -> importODCSContract(fakeEntityId, odcsYaml, "yaml"));
  ...
  private String createTestDatabaseSchema() throws IOException {
    ...
    Response serviceResponse =
        SecurityUtil.addHeaders(serviceTarget, ADMIN_AUTH_HEADERS).post(Entity.json(createService));
    DatabaseService service =
        TestUtils.readResponse(
            serviceResponse, DatabaseService.class, Response.Status.CREATED.getStatusCode());
    ...
    Database database =
        TestUtils.readResponse(dbResponse, Database.class, Response.Status.CREATED.getStatusCode());
    ...
    DatabaseSchema schema =
        TestUtils.readResponse(
            schemaResponse, DatabaseSchema.class, Response.Status.CREATED.getStatusCode());
    ...
    Response response =
        SecurityUtil.addHeaders(target, ADMIN_AUTH_HEADERS)
            .post(
                Entity.entity(
                    odcsContent,
                    "yaml".equals(format) ? "application/yaml" : MediaType.APPLICATION_JSON));

    return TestUtils.readResponse(
        response, ODCSImportResponse.class, Response.Status.OK.getStatusCode());
  }
