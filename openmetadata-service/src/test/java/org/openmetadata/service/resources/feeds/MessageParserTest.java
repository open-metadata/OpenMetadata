...
class MessageParserTest {
  ...
  void parseMessage() {
    ...
    assertEqual(4, links.size());
    assertEqual(new EntityLink("table", "tableFQN", null, null, null), links.get(0));
    assertEqual(new EntityLink("table", "tableFQN", "description", null, null), links.get(1));
    assertEqual(new EntityLink("table", "tableFQN", "columns", "c1", null), links.get(2));
    assertEqual(new EntityLink("table", "tableFQN", "columns", "c1", "description"), links.get(3));
  }
  ...
  void parseMessageWithFallbackText() {
    ...
    assertEqual(2, links.size());
    EntityLink link = links.get(0);
    assertEqual("user1", link.getEntityFQN());
    assertEqual("user", link.getEntityType());
    assertNull(link.getFieldName());
    link = links.get(1);
    assertEqual("user2", link.getEntityFQN());
    assertEqual("user", link.getEntityType());
    assertNull(link.getFieldName());
  }
}
