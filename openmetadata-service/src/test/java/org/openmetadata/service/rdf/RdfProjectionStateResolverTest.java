...
class RdfProjectionStateResolverTest {
  ...
  void reportsRebuildingUntilTheFirstPostMigrationRun() {
    ...
    assertEquals(ProjectionState.REBUILDING, resolver.resolve());
  }

  @ParameterizedTest
  @MethodSource("projectionStates")
  void mapsLatestApplicationRunState(
      final AppRunRecord.Status runStatus, final ProjectionState expectedState) {
  ...
  void reportsDegradedForMalformedRunState() {
    ...
    assertEquals(ProjectionState.DEGRADED, resolver.resolve());
  }
  ...
  private static Stream<Arguments> projectionStates() {
    return Stream.of(
        Arguments.of(AppRunRecord.Status.SUCCESS, ProjectionState.READY),
        Arguments.of(AppRunRecord.Status.COMPLETED, ProjectionState.READY),
        Arguments.of(AppRunRecord.Status.RUNNING, ProjectionState.REBUILDING),
        Arguments.of(AppRunRecord.Status.PENDING, ProjectionState.REBUILDING),
        Arguments.of(AppRunRecord.Status.FAILED, ProjectionState.DEGRADED),
        Arguments.of(AppRunRecord.Status.ACTIVE_ERROR, ProjectionState.DEGRADED));
  }
}
