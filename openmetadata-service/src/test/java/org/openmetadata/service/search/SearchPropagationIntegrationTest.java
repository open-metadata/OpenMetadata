...
public class SearchPropagationIntegrationTest extends OpenMetadataApplicationTest {
  ...
  public static void setup(TestInfo test) throws IOException, HttpResponseException {
    ...
    testService = serviceResourceTest.createEntity(
        serviceResourceTest.createRequest(SERVICE_NAME),
        ADMIN_AUTH_HEADERS);
    ...
    LOG.info("Test setup complete - created service: {}, database: {}, schema: {}, tables: [{}, {}]",
        testService.getName(),
  ...
  void testOwnerPropagationFromDatabase() throws IOException, InterruptedException {
    ...
    String jsonPatch = JsonUtils.pojoToJson(List.of(
        Map.of(
            "op", "add",
            "path", "/owners",
    ...
    Database patchedDatabase = databaseResourceTest.patchEntity(
        testDatabase.getId(),
    ...
    WebTarget table1SearchTarget = getResource("search/query")
        .queryParam("q", "fullyQualifiedName:" + testTable1.getFullyQualifiedName())
        .queryParam("index", "table_search_index");
    
    String table1SearchResponse = TestUtils.get(table1SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table1SearchResponse.contains(testUser.getId().toString()),
        "Table 1 search result should contain owner ID that propagated from database");
    
    WebTarget table2SearchTarget = getResource("search/query")
        .queryParam("q", "fullyQualifiedName:" + testTable2.getFullyQualifiedName())
        .queryParam("index", "table_search_index");
    
    String table2SearchResponse = TestUtils.get(table2SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table2SearchResponse.contains(testUser.getId().toString()),
  ...
  void testDomainPropagationFromDatabase() throws IOException, InterruptedException {
    ...
    assertTrue(patchedDatabase.getDomains().stream()
    ...
    String table1SearchResponse = TestUtils.get(table1SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table1SearchResponse.contains(testDomain.getId().toString()),
    ...
    String table2SearchResponse = TestUtils.get(table2SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table2SearchResponse.contains(testDomain.getId().toString()),
  ...
  void testNoPropagationForDescriptionChange() throws IOException, InterruptedException {
    ...
    assertEquals("Updated description - should not trigger propagation", 
        patchedDatabase.getDescription());
    
    // Wait briefly for any potential propagation
    simulateWork(2000);
    
    // Fetch the database again to ensure it was updated
    Database updatedDatabase = databaseResourceTest.getEntity(
        testDatabase.getId(), "description", ADMIN_AUTH_HEADERS);
    assertEquals("Updated description - should not trigger propagation",
    ...
    DatabaseSchema updatedSchema = schemaResourceTest.getEntity(
        testSchema.getId(), "description", ADMIN_AUTH_HEADERS);
    assertNotEquals("Updated description - should not trigger propagation",
        updatedSchema.getDescription(),
        "Schema should not inherit description from database");
    
    Table updatedTable1 = tableResourceTest.getEntity(
        testTable1.getId(), "description", ADMIN_AUTH_HEADERS);
    assertNotEquals("Updated description - should not trigger propagation",
  ...
  void testNoPropagationForTagChanges() throws IOException, InterruptedException {
    LOG.info("Testing that tag changes to schema do NOT propagate to tables (tags don't propagate)");
    ...
    DatabaseSchema patchedSchema = schemaResourceTest.patchEntity(
        testSchema.getId(),
        JsonUtils.readTree(jsonPatch),
        ADMIN_AUTH_HEADERS);
    
    assertNotNull(patchedSchema.getTags());
    assertTrue(patchedSchema.getTags().stream()
    ...
    String table1SearchResponse = TestUtils.get(table1SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertFalse(table1SearchResponse.contains("PII.Sensitive"),
    ...
    String table2SearchResponse = TestUtils.get(table2SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertFalse(table2SearchResponse.contains("PII.Sensitive"),
  ...
  @Test
  @Order(5)
  void testMultipleFieldChangesWithConditionalPropagation() throws IOException, InterruptedException {
    LOG.info("Testing multiple field changes - only inheritable ones should propagate");
    
    // Record initial metrics
    Counter propagationSkipped = Metrics.globalRegistry
        .find("search.index.propagation.skipped")
        .tag("entity_type", "database")
        .counter();
    Counter propagationExecuted = Metrics.globalRegistry
    ...
    String jsonPatch1 = JsonUtils.pojoToJson(List.of(
        Map.of(
            "op", "add",
            "path", "/description",
    ...
    Database patchedDatabase1 = databaseResourceTest.patchEntity(
        testDatabase.getId(),
        JsonUtils.readTree(jsonPatch1),
        ADMIN_AUTH_HEADERS);
    
    assertEquals("Another description update - no propagation", 
        patchedDatabase1.getDescription());
    
    simulateWork(1500);
    
    // Verify propagation was skipped
    double afterDescSkipped = propagationSkipped != null ? propagationSkipped.count() : 0;
    assertTrue(afterDescSkipped > initialSkipped,
    ...
    String jsonPatch2 = JsonUtils.pojoToJson(List.of(
        Map.of(
            "op", "add",
            "path", "/displayName",
    ...
    Database patchedDatabase2 = databaseResourceTest.patchEntity(
        testDatabase.getId(),
        JsonUtils.readTree(jsonPatch2),
        ADMIN_AUTH_HEADERS);
    ...
    assertTrue(newExecuted > initialExecuted,
    ...
    String table1SearchResponse = TestUtils.get(table1SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table1SearchResponse.contains("Test Database Display Name"),
    ...
    String table2SearchResponse = TestUtils.get(table2SearchTarget, String.class, ADMIN_AUTH_HEADERS);
    assertTrue(table2SearchResponse.contains("Test Database Display Name"),
  ...
  void testTagPropagationFromTableToColumns() throws IOException, InterruptedException {
    ...
    CreateTable createTable = new CreateTable()
        ...
        .withColumns(List.of(
            ...
            new Column().withName("customer_name").withDataType(ColumnDataType.VARCHAR).withDataLength(100),
            new Column().withName("customer_email").withDataType(ColumnDataType.VARCHAR).withDataLength(255)));
    ...
    String jsonPatch = JsonUtils.pojoToJson(List.of(
        Map.of("op", "add", "path", "/tags", "value", List.of(piiTag))
    ));
    
    Table patchedTable = tableResourceTest.patchEntity(
        tableWithColumns.getId(),
        JsonUtils.readTree(jsonPatch),
        ADMIN_AUTH_HEADERS);
    
    assertNotNull(patchedTable.getTags());
    assertTrue(patchedTable.getTags().stream().anyMatch(t -> "PII.Sensitive".equals(t.getTagFQN())));
    
    // Wait for propagation
    simulateWork(3000);
    
    // Query search API for table to verify columns inherited the tag
    WebTarget tableSearchTarget = getResource("search/query")
    ...
    assertTrue(tableSearchResponse.contains("customer_name"), "Response should contain column names");
    assertTrue(tableSearchResponse.contains("customer_email"), "Response should contain column names");
  ...
  void testOwnerRemovalPropagation() throws IOException, InterruptedException {
    ...
    String addOwnerPatch = JsonUtils.pojoToJson(List.of(
        Map.of("op", "add", "path", "/owners", "value", List.of(ownerRef))
    ));
    
    Database dbWithOwner = databaseResourceTest.patchEntity(
        testDatabase.getId(),
        JsonUtils.readTree(addOwnerPatch),
        ADMIN_AUTH_HEADERS);
    ...
    String removeOwnerPatch = JsonUtils.pojoToJson(List.of(
        Map.of("op", "remove", "path", "/owners")
    ));
    
    Database dbWithoutOwner = databaseResourceTest.patchEntity(
        testDatabase.getId(),
        JsonUtils.readTree(removeOwnerPatch),
        ADMIN_AUTH_HEADERS);
    
    assertTrue(dbWithoutOwner.getOwners() == null || dbWithoutOwner.getOwners().isEmpty(),
    ...
    LOG.info("Owner removal propagation test completed - verified owner was removed from child entities");
  }
  ...
  void testPropagationMetricsAccuracy() {
    ...
    Counter searchSkipped = Metrics.globalRegistry
        .find("search.index.propagation.skipped")
        .tag("entity_type", "database")
        .counter();
    Counter searchExecuted = Metrics.globalRegistry
        .find("search.index.propagation.executed")
        .tag("entity_type", "database")
        .counter();
    ...
    if (searchSkipped != null) {
      ...
      assertTrue(searchSkipped.count() > 0, "Should have skipped search propagation for description change");
    } else {
      LOG.info("Search propagation skipped counter not found - may not be initialized in test environment");
    }
    ...
    if (searchExecuted != null) {
      ...
      assertTrue(searchExecuted.count() >= 2, "Should have executed search propagation for owner and displayName changes");
    } else {
      LOG.info("Search propagation executed counter not found - may not be initialized in test environment");
    }
    ...
    LOG.info("Search propagation metrics test completed - conditional propagation in search layer working as expected");
  }
}