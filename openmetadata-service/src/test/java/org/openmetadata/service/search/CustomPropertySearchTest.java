...
class CustomPropertySearchTest {
  ...
  void testMergeAddsNewAssetTypeFromDefaults() {
    ...
    AssetTypeConfiguration tableConfig = new AssetTypeConfiguration();
    tableConfig.setAssetType("table");
    tableConfig.setSearchFields(List.of(createFieldBoost("name", 10.0, "phrase")));

    AssetTypeConfiguration metricConfig = new AssetTypeConfiguration();
    metricConfig.setAssetType("metric");
    metricConfig.setSearchFields(
        List.of(
            createFieldBoost("displayName.keyword", 20.0, "exact"),
            createFieldBoost("name", 10.0, "phrase")));

    defaultSettings.setAssetTypeConfigurations(
        new ArrayList<>(List.of(tableConfig, metricConfig)));

    SearchSettings existingSettings = new SearchSettings();
    GlobalSettings existingGlobal = new GlobalSettings();
    existingGlobal.setMaxResultHits(8000);
    existingSettings.setGlobalSettings(existingGlobal);

    AssetTypeConfiguration existingTableConfig = new AssetTypeConfiguration();
    existingTableConfig.setAssetType("table");
    existingTableConfig.setSearchFields(
        List.of(createFieldBoost("name", 15.0, "phrase")));

    existingSettings.setAssetTypeConfigurations(
        new ArrayList<>(List.of(existingTableConfig)));

    SearchSettings merged =
        searchSettingsHandler.mergeSearchSettings(defaultSettings, existingSettings);

    boolean metricExists =
        merged.getAssetTypeConfigurations().stream()
            .anyMatch(config -> "metric".equals(config.getAssetType()));
    assertTrue(metricExists, "Metric asset type from defaults should be added to merged settings");

    AssetTypeConfiguration mergedMetric =
        merged.getAssetTypeConfigurations().stream()
            .filter(config -> "metric".equals(config.getAssetType()))
            .findFirst()
            .orElse(null);
    assertNotNull(mergedMetric);
    assertEquals(2, mergedMetric.getSearchFields().size());

    AssetTypeConfiguration mergedTable =
        merged.getAssetTypeConfigurations().stream()
            .filter(config -> "table".equals(config.getAssetType()))
            .findFirst()
            .orElse(null);
    ...
    assertEquals(
        15.0,
        mergedTable.getSearchFields().get(0).getBoost(),
  ...
  void testMergeDoesNotDuplicateExistingAssetType() {
    ...
    AssetTypeConfiguration defaultMetric = new AssetTypeConfiguration();
    defaultMetric.setAssetType("metric");
    defaultMetric.setSearchFields(
        List.of(createFieldBoost("name", 10.0, "phrase")));
    defaultSettings.setAssetTypeConfigurations(new ArrayList<>(List.of(defaultMetric)));
    ...
    AssetTypeConfiguration existingMetric = new AssetTypeConfiguration();
    existingMetric.setAssetType("metric");
    existingMetric.setSearchFields(
        List.of(createFieldBoost("name", 20.0, "phrase")));
    existingSettings.setAssetTypeConfigurations(new ArrayList<>(List.of(existingMetric)));
    ...
    assertEquals(
        20.0,
        mergedMetric.getSearchFields().get(0).getBoost(),
