 ...
 * registered for its type. A named channel that is not registered on this server is never replaced
 * by the type's channel, because a destination meant for one channel must not be sent through
 * another.
 *
...
public record ChannelResolution(String channelId, Optional<Channel> channel) {
  ...
  public static ChannelResolution of(SubscriptionDestination destination) {
    String named = destination.getChannel();
    String channelId = named == null ? destination.getType().value() : named;
    ChannelResolution resolution = new ChannelResolution(channelId, Channels.of(channelId));
    logOnce(destination, resolution, named == null ? "its type" : "its channel field");
    return resolution;
  }
