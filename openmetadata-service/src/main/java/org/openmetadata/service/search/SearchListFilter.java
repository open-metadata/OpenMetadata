...
public class SearchListFilter extends Filter<SearchListFilter> {
    ...
    private String getTestCaseCondition() {
        ...
        if (testPlatform!= null) {
            conditions.add(
                    String.format(
                            "{\"term\": {\"testPlatforms\": \"%s\"}}",
                            testPlatform));
        }

        if (startTimestamp != null && endTimestamp != null) {
            conditions.add(getTimestampFilter("testCaseResult.timestamp", "gte", Long.parseLong(startTimestamp)));
            conditions.add(getTimestampFilter("testCaseResult.timestamp", "lte", Long.parseLong(endTimestamp)));
        }
