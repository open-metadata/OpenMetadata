...
public class EsUtils {
  ...
  /**
   * Builds and applies search source filters for Elasticsearch queries with SQL injection protection.
   *
   ...
   * @param searchSourceBuilder Elasticsearch search source builder to apply filters to
   *
  ...
  public static void buildSearchSourceFilter(
    ...
    if (!nullOrEmpty(sanitizedFilter) && !sanitizedFilter.equals("{}")) {
      try {
        // Parse the sanitized filter string into Elasticsearch QueryBuilder
        // Note: SearchSourceBuilder.fromXContent() was the vulnerable method in the original code
        XContentParser filterParser =
            XContentType.JSON
                .xContent()
                .createParser(
                    esXContentRegistry, LoggingDeprecationHandler.INSTANCE, sanitizedFilter);
