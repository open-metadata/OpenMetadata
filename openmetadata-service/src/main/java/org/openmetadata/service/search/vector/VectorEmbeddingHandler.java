...
public class VectorEmbeddingHandler implements EntityLifecycleEventHandler {
  ...
  private void updateEmbedding(EntityInterface entity) {
    ...
    if (!Entity.getSearchRepository().shouldIndexEntity(entity)) {
      // A memory flipped to PRIVATE/SHARED must drop its chunk docs so it no longer matches
      // KNN/vector queries — mirrors the entity-doc delete in updateEntityIndex.
      deleteChunks(entity);
      return;
    }
    try {
