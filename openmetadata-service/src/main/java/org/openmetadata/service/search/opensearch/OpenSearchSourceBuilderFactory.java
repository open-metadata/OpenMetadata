...
import static os.org.opensearch.index.query.QueryBuilders.*;
...
public class OpenSearchSourceBuilderFactory
  ...
  public FunctionScoreQueryBuilder getFunctionScoreQueryBuilder(QueryBuilder query) {
  ...
  public SearchSourceBuilder searchBuilder(
    ...
    if (query instanceof QueryStringQueryBuilder) {
      FunctionScoreQueryBuilder functionScoreQueryBuilder =
          boostScore((QueryStringQueryBuilder) query);
  ...
  private SearchSourceBuilder buildTopicSearchBuilder(String query, int from, int size) {
        ...
        (QueryStringQueryBuilder)
            buildSearchQueryBuilder(
                ...
                Map.of(
                    ...
                    "fullyQualifiedName", 10.0f,
                    "fullyQualifiedName.ngram", 1.0f,
                    "messageSchema.schemaFields.description", 1.0f,
                    "messageSchema.schemaFields.children.name", 1.0f));
    HighlightBuilder hb =
        buildHighlights(
            List.of(
    ...
    SearchSourceBuilder searchSourceBuilder = searchBuilder(queryBuilder, hb, from, size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("messageSchema.schemaFields.name")
  ...
  private SearchSourceBuilder buildTableSearchBuilder(String query, int from, int size) {
    ...
    searchSourceBuilder
        ...
        .aggregation(AggregationBuilders.terms("columns.name").field("columns.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }
