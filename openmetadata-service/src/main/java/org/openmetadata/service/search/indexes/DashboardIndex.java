...
public class DashboardIndex implements SearchIndex {
  ...
  public Map<String, Object> buildESDoc() {
    ...
    doc.put(
        "totalVotes",
        CommonUtil.nullOrEmpty(dashboard.getVotes())
            ? 0
            : dashboard.getVotes().getUpVotes() - dashboard.getVotes().getDownVotes());
    ...
    doc.put("ownerKeyword", getOwnerDisplayNameOrNoOwner(dashboard.getOwner()));
    doc.put("service", getEntityWithDisplayName(dashboard.getService()));
    doc.put("domain", getEntityWithDisplayName(dashboard.getDomain()));
    doc.put("descriptionStatus", getDescriptionStatus(dashboard.getDescription()));
    return doc;
  }
