...
public class PaginatedEntityTimeSeriesSource implements Source<ResultList<? extends EntityTimeSeriesInterface>> {
  ...
  public PaginatedEntityTimeSeriesSource(String entityType, int batchSize, List<String> fields) {
    ...
    @Override
    public ResultList<? extends EntityTimeSeriesInterface> readNext(Map<String, Object> contextData) throws SearchIndexException {
    ...
    private ResultList<? extends EntityTimeSeriesInterface> read(String cursor) throws SearchIndexException {
        LOG.debug("[PaginatedEntityTimeSeriesSource] Fetching a Batch of Size: {} ", batchSize);
        EntityTimeSeriesRepository<? extends EntityTimeSeriesInterface> repository = getEntityTimeSeriesRepository();
        ResultList<? extends EntityTimeSeriesInterface> result;
        ListFilter filter = getFilter();
        try {
            result = repository
                    .listWithOffset(cursor, filter, batchSize, true);
