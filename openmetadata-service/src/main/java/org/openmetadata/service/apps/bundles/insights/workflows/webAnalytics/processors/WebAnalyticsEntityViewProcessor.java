...
public class WebAnalyticsEntityViewProcessor implements Processor<Map<String, WebAnalyticEntityViewReportData>, ResultList<WebAnalyticEventData>> {
    ...
    public Map<String, WebAnalyticEntityViewReportData> process(ResultList<WebAnalyticEventData> input, Map<String, Object> contextData) throws SearchIndexException {
        try {
            ...
            IndexingError error = new IndexingError()
                    ...
                    .withSuccessCount(0)
                    .withMessage(
                            "WebAnalytics Entity View Processor Encounter Failure.")
                    .withStackTrace(ExceptionUtils.exceptionStackTraceAsString(e));
            LOG.debug("[WebAnalyticsEntityViewProcessor] Failed. Details: {}", JsonUtils.pojoToJson(error));
            updateStats(0, input.getData().size());
            throw new SearchIndexException(error);
        }
        return (HashMap<String, WebAnalyticEntityViewReportData>) contextData.get(ENTITY_VIEW_REPORT_DATA_KEY);
    }

    private void processEvent(WebAnalyticEventData event, Map<String, Object> contextData) {
        Map<String, WebAnalyticEntityViewReportData> entityViewData = (HashMap<String, WebAnalyticEntityViewReportData>) contextData.get(ENTITY_VIEW_REPORT_DATA_KEY);
        ...
        if (oProcessedUrl.isPresent()) {
            ...
            if (!entityViewData.containsKey(entityFqn)) {
                ...
                try {
                    ...
                    EntityInterface entity = Entity.getEntityByName(entityType, entityFqn, "*", Include.NON_DELETED, true);
                    WebAnalyticEntityViewReportData webAnalyticEntityViewReportData = new WebAnalyticEntityViewReportData()
                    ...
                    if (oEntityOwner.isPresent()) {
                        ...
                        // Skip the Event if Entity Owner is the same as the User from the Event since we are not counting
                ...
                WebAnalyticEntityViewReportData webAnalyticEntityViewReportData = entityViewData.get(entityFqn);
    ...
    private Optional<String> getEntityTier(List<String> entityTags) {
        ...
        List<String> tierTags = entityTags.stream()
                .filter(tag -> tag.startsWith("Tier"))
                .toList();

        // We can directly get the first element if the list is not empty since there can only be ONE Tier tag.
        if (!tierTags.isEmpty()) {
