...
public class EntityJsonUpdateJob extends AbstractNativeApplication implements Job {
  ...
  private void processTask(UpdateTask task) {
    switch (task.getType()) {
      ...
      case TAG_RENAMED:
        processTagRename(
            task.getOldFQN(), task.getNewFQN(), task.getNewName(), task.getTagSource());
        break;
      case OWNER_DELETED:
      ...
      case OWNER_RENAMED:
        processOwnerRename(
            task.getEntityId(), task.getEntityType(), task.getNewName(), task.getNewFQN());
        break;
      case DOMAIN_DELETED:
  ...
  private void processTagDeletion(String tagFQN, TagSource source) {
    ...
    for (String targetFQNHash : targetFQNHashes) {
      try {
        ...
        LOG.error(
            "Failed to remove tag {} from entity {}: {}", tagFQN, targetFQNHash, e.getMessage());
      }
    }
  }
  ...
  private void processTagRename(String oldFQN, String newFQN, String newName, TagSource source) {
    ...
    for (String targetFQNHash : targetFQNHashes) {
      try {
        ...
        LOG.error(
            "Failed to rename tag {} in entity {}: {}", oldFQN, targetFQNHash, e.getMessage());
      }
    }
  }
  ...
  private void processOwnerDeletion(UUID ownerId, String ownerType) {
    ...
    List<CollectionDAO.EntityRelationshipRecord> relationships =
        collectionDAO
            .relationshipDAO()
            .findTo(ownerId, ownerType, org.openmetadata.schema.type.Relationship.OWNS.ordinal());
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to remove owner {} from entity {}: {}", ownerId, rel.getId(), e.getMessage());
      }
    }
  }
  ...
  private void processOwnerRename(UUID ownerId, String ownerType, String newName, String newFQN) {
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to update owner {} in entity {}: {}", ownerId, rel.getId(), e.getMessage());
      }
    }
  }
  ...
  private void processDomainDeletion(UUID domainId) {
    ...
    List<CollectionDAO.EntityRelationshipRecord> relationships =
        collectionDAO
            .relationshipDAO()
            .findTo(
                domainId, Entity.DOMAIN, org.openmetadata.schema.type.Relationship.HAS.ordinal());
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to remove domain {} from entity {}: {}", domainId, rel.getId(), e.getMessage());
      }
    }
  }
  ...
  private void processDomainRename(UUID domainId, String newName, String newFQN) {
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to update domain {} in entity {}: {}", domainId, rel.getId(), e.getMessage());
      }
    }
  }
  ...
  private void processDataProductDeletion(UUID dataProductId) {
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to remove data product {} from entity {}: {}",
            dataProductId,
            rel.getId(),
            e.getMessage());
      }
  ...
  private void processDataProductRename(UUID dataProductId, String newName, String newFQN) {
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      try {
        ...
        LOG.error(
            "Failed to update data product {} in entity {}: {}",
            dataProductId,
            rel.getId(),
            e.getMessage());
      }
  ...
  private void updateTagInEntityJson(
      String targetFQNHash, String oldFQN, String newFQN, String newName) {
    // TODO: Need reverse lookup from FQN hash to entity type and ID
    LOG.debug("Need to implement reverse lookup for targetFQNHash: {}", targetFQNHash);
  }
  ...
  private void removeOwnerFromEntityJson(UUID entityId, String entityType, UUID ownerId) {
    try {
      ...
      LOG.error(
          "Error marking owner {} as deleted in {} {}: {}",
          ownerId,
          entityType,
          entityId,
          e.getMessage());
  ...
  private void updateOwnerInEntityJson(
      UUID entityId, String entityType, UUID ownerId, String newName, String newFQN) {
    try {
      // Get the entity repository to properly update the JSON
      EntityRepository<?> repository = Entity.getEntityRepository(entityType);
      if (repository == null) {
      ...
      LOG.error(
          "Error updating owner {} in {} {}: {}", ownerId, entityType, entityId, e.getMessage());
    }
  }
  ...
  private void updateDomainInEntityJson(
      UUID entityId, String entityType, UUID domainId, String newName, String newFQN) {
    try {
      // Get the entity repository to properly update the JSON
      EntityRepository<?> repository = Entity.getEntityRepository(entityType);
      if (repository == null) {
      ...
      LOG.error(
          "Error updating domain {} in {} {}: {}", domainId, entityType, entityId, e.getMessage());
    }
  }

  private void removeDataProductFromEntityJson(
      UUID entityId, String entityType, UUID dataProductId) {
    try {
      // Get the entity repository to properly update the JSON
      EntityRepository<?> repository = Entity.getEntityRepository(entityType);
      if (repository == null) {
      ...
      LOG.error(
          "Error removing data product {} from {} {}: {}",
          dataProductId,
          entityType,
          entityId,
          e.getMessage());
  ...
  private void updateDataProductInEntityJson(
      UUID entityId, String entityType, UUID dataProductId, String newName, String newFQN) {
    try {
      // Get the entity repository to properly update the JSON
      EntityRepository<?> repository = Entity.getEntityRepository(entityType);
      if (repository == null) {
      ...
      if (json != null) {
        ...
        if (dataProducts != null) {
          ...
          for (JsonNode dataProduct : dataProducts) {
            if (dataProduct.has("id")
                && dataProduct.get("id").asText().equals(dataProductId.toString())) {
      ...
      LOG.error(
          "Error updating data product {} in {} {}: {}",
          dataProductId,
          entityType,
          entityId,
          e.getMessage());
  ...
  public static void queueTagRename(
      String oldFQN, String newFQN, String newName, TagSource source) {
    try {
      updateQueue.offer(
          UpdateTask.builder()
              .type(UpdateTask.TaskType.TAG_RENAMED)
  ...
  public static void queueOwnerRename(
      UUID ownerId, String ownerType, String newName, String newFQN) {
    try {
      updateQueue.offer(
          UpdateTask.builder()
              .type(UpdateTask.TaskType.OWNER_RENAMED)
  ...
  public static void queueDomainDeletion(UUID domainId) {
    try {
      updateQueue.offer(
          UpdateTask.builder().type(UpdateTask.TaskType.DOMAIN_DELETED).entityId(domainId).build());
