    ...
    public Map<UUID, WebAnalyticUserActivityReportData> process(Map<UUID, WebAnalyticsWorkflow.UserActivityData> input, Map<String, Object> contextData) throws SearchIndexException {
        ...
        try {
            ...
            IndexingError error = new IndexingError()
                    ...
                    .withSuccessCount(0)
                    .withMessage(
                            "Web Analytics User Activity Aggregator Encounter Failure.")
                    .withStackTrace(ExceptionUtils.exceptionStackTraceAsString(e));
            LOG.debug("[WebAnalyticsUserActivityAggregator] Failed. Details: {}", JsonUtils.pojoToJson(error));
            updateStats(0, input.size());
            throw new SearchIndexException(error);
        }
        return (HashMap<UUID, WebAnalyticUserActivityReportData>) contextData.get(USER_ACTIVITY_REPORT_DATA_KEY);
    }

    private void processEvent(WebAnalyticsWorkflow.UserActivityData userActivityData, Map<String, Object> contextData) {
        Map<UUID, WebAnalyticUserActivityReportData> userActivityReportData = (HashMap<UUID, WebAnalyticUserActivityReportData>) contextData.get(USER_ACTIVITY_REPORT_DATA_KEY);
        int totalSessionDurationSeconds = 0;

        for (List<Long> timestampList : userActivityData.sessions().values()) {
            totalSessionDurationSeconds += (Collections.max(timestampList) - Collections.min(timestampList)) / 1000;
        }
