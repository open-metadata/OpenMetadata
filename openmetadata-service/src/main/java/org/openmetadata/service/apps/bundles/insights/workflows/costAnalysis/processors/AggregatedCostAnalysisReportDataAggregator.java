    ...
    public List<AggregatedCostAnalysisReportData> process(Map<String, Map<String, Map<String, CostAnalysisWorkflow.AggregatedCostAnalysisData>>> input, Map<String, Object> contextData) throws SearchIndexException {
        ...
        try {
            for (Map.Entry<String, Map<String, Map<String, CostAnalysisWorkflow.AggregatedCostAnalysisData>>> entry : input.entrySet()) {
                ...
                for (Map.Entry<String, Map<String, CostAnalysisWorkflow.AggregatedCostAnalysisData>> serviceTypeEntry : entityTypeEntryValue.entrySet()) {
                    ...
                    for (Map.Entry<String, CostAnalysisWorkflow.AggregatedCostAnalysisData>  serviceNameEntry : serviceTypeEntryValue.entrySet()) {
                        ...
                        DataAssetMetrics unusedDataAssets = aggregatedData.unusedDataAssets()
                                .withTotalCount((double) aggregatedData.unusedDataAssets().getCount().getThreeDays())
                                .withTotalSize((double) aggregatedData.unusedDataAssets().getSize().getThreeDays());
                        DataAssetMetrics frequentlyUsedDataAssets = aggregatedData.frequentlyUsedDataAssets()
                                .withTotalCount((double) aggregatedData.frequentlyUsedDataAssets().getCount().getThreeDays())
                                .withTotalSize((double) aggregatedData.frequentlyUsedDataAssets().getSize().getThreeDays());
            ...
            IndexingError error = new IndexingError()
                    ...
                    .withSuccessCount(0)
                    .withMessage(
                            "Aggregated Cost Analysis Aggregator Encounter Failure.")
                    .withStackTrace(ExceptionUtils.exceptionStackTraceAsString(e));
            LOG.debug("[AggregatedCostAnalysisAggregator] Failed. Details: {}", JsonUtils.pojoToJson(error));
