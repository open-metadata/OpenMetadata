    ...
    public Map<UUID, WebAnalyticsWorkflow.UserActivityData> process(ResultList<WebAnalyticEventData> input, Map<String, Object> contextData) throws SearchIndexException {
        ...
        try {
            ...
            IndexingError error = new IndexingError()
                    ...
                    .withSuccessCount(0)
                    .withMessage(
                            "WebAnalytics User Activity Processor Encounter Failure.")
                    .withStackTrace(ExceptionUtils.exceptionStackTraceAsString(e));
            LOG.debug("[WebAnalyticsUserActivityProcessor] Failed. Details: {}", JsonUtils.pojoToJson(error));
            updateStats(0, input.getData().size());
            throw new SearchIndexException(error);
        }
        return (HashMap<UUID, WebAnalyticsWorkflow.UserActivityData>) contextData.get(USER_ACTIVITY_DATA_KEY);
    }

    private void processEvent(WebAnalyticEventData event, Map<String, Object> contextData) {
        Map<UUID, WebAnalyticsWorkflow.UserActivityData> userActivityDataMap = (HashMap<UUID, WebAnalyticsWorkflow.UserActivityData>) contextData.get(USER_ACTIVITY_DATA_KEY);
        ...
        if (!userActivityDataMap.containsKey(userId)) {
            // Fetch user Info
            var userDetails = Entity.getEntityRepository(Entity.USER).get(null, userId, new EntityUtil.Fields(Set.of("teams")));

            Map<UUID, List<Long>> sessions = new HashMap<>();
            sessions.put(sessionId, List.of(timestamp));

            WebAnalyticsWorkflow.UserActivityData userActivityData = new WebAnalyticsWorkflow.UserActivityData(
                    userDetails.getName(),
                    userId,
                    null,
                    sessions,
...
//                                    .withTeam(userDetails.)
            ...
            WebAnalyticsWorkflow.UserActivityData newUserActivityData = new WebAnalyticsWorkflow.UserActivityData(
