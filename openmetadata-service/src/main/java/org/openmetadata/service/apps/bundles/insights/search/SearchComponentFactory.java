...
public final class SearchComponentFactory {
  ...
  public Processor createEntityTimeSeriesProcessor(int totalRecords) {
    return switch (searchRepository.getSearchType()) {
      case OPENSEARCH -> new OpenSearchEntityTimeSeriesProcessor(totalRecords);
      default -> new ElasticSearchEntityTimeSeriesProcessor(totalRecords);
    };
  }

  public DataInsightsSearchInterface createSearchInterface() {
    return switch (searchRepository.getSearchType()) {
      case OPENSEARCH -> new OpenSearchDataInsightsClient(
          searchRepository.getSearchClient().getHighLevelClient(),
          searchRepository.getClusterAlias());
      default -> new ElasticSearchDataInsightsClient(
