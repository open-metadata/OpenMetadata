...
public class WebAnalyticsWorkflow {
    ...
    public WebAnalyticsWorkflow(Long timestamp, int batchSize, Optional<DataInsightsApp.Backfill> backfill) {
        if (backfill.isPresent()) {
            Long oldestPossibleTimestamp = TimestampUtils.getStartOfDayTimestamp(TimestampUtils.subtractDays(timestamp, webAnalyticEventsRetentionDays));
            ...
            this.startTimestamp = TimestampUtils.getStartOfDayTimestamp(Collections.max(
                    List.of(TimestampUtils.getTimestampFromDateString(backfill.get().startDate()), oldestPossibleTimestamp)));

            if (oldestPossibleTimestamp.equals(endTimestamp)) {
                LOG.warn("Backfill won't happen because the set date is before the limit of {}", oldestPossibleTimestamp);
            }
        } else {
    ...
    private void initialize() {
        ...
        while (pointerTimestamp > startTimestamp) {
            sources.add(new PaginatedWebAnalyticEventDataSource(batchSize, TimestampUtils.getStartOfDayTimestamp(pointerTimestamp), pointerTimestamp));
    ...
    public void process() throws SearchIndexException {
        ...
        for (PaginatedWebAnalyticEventDataSource source : sources) {
            ...
            // Process each Report.
            Optional<String> processEntityViewDataError = processEntityViewData(entityViewReportData, contextData);

            processEntityViewDataError.ifPresent(LOG::debug);

            Optional<String> processUserActivityError = processUserActivityData(userActivityData, userActivityReportData, contextData);
    ...
    // TODO: How to better divide the two flows while keeping stats consistent and avoiding breaking one flow if
    ...
    private Optional<String> processEntityViewData(Map<String, WebAnalyticEntityViewReportData> entityViewReportData, Map<String, Object> contextData) throws SearchIndexException {
        ...
        contextData.put(REPORT_DATA_TYPE_KEY, ReportData.ReportDataType.WEB_ANALYTIC_ENTITY_VIEW_REPORT_DATA);
        CreateReportDataProcessor createReportDataProcessor = new CreateReportDataProcessor(entityViewReportData.values().size(), "EntityViewReportDataProcessor");

        Optional<List<ReportData>> entityViewReportDataList = Optional.empty();

        // Process EntityView ReportData
        try {
            entityViewReportDataList = Optional.of(createReportDataProcessor.process(
                    entityViewReportData.values().stream().toList(),
                    contextData));
        } catch (SearchIndexException ex) {
        ...
        if (entityViewReportDataList.isPresent()) {
            ReportDataSink reportDataSink = new ReportDataSink(entityViewReportDataList.get().size(), "EntityViewReportDataSink");
    ...
    private Optional<String> processUserActivityData(
            ...
            Map<String, Object> contextData) throws SearchIndexException {
        Optional<String> error = Optional.empty();

        contextData.put(REPORT_DATA_TYPE_KEY, ReportData.ReportDataType.WEB_ANALYTIC_USER_ACTIVITY_REPORT_DATA);
        ...
        CreateReportDataProcessor createReportdataProcessor = new CreateReportDataProcessor(userActivityReportData.values().size(), "UserActivityReportDataProcessor");
        Optional<List<ReportData>> userActivityReportDataList = Optional.empty();

        // Process UserActivity ReportData
        try {
            userActivityReportDataList = Optional.of(createReportdataProcessor.process(
                    userActivityReportData.values().stream().toList(),
                    contextData));
        ...
        if (userActivityReportDataList.isPresent()) {
            ReportDataSink reportDataSink = new ReportDataSink(userActivityReportDataList.get().size(), "UserActivityReportDataSink");
            try {
    ...
    private void pruneWebAnalyticEvents() {
        for (WebAnalyticEventType eventType : WebAnalyticEventType.values()) {
            ((WebAnalyticEventRepository) Entity.getEntityRepository(Entity.WEB_ANALYTIC_EVENT)).deleteWebAnalyticEventData(eventType, TimestampUtils.subtractDays(endTimestamp, webAnalyticEventsRetentionDays));
        }
    }
    ...
    private void updateWorkflowStats(String stepName, StepStats newStepStats) {
        ...
        int currentSuccess = workflowStats.getWorkflowStepStats().values().stream()
                .mapToInt(StepStats::getSuccessRecords)
                .sum();
        int currentFailed = workflowStats.getWorkflowStepStats().values().stream()
