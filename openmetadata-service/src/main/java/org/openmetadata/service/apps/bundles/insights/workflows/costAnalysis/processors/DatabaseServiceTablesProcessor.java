        ...
        try {
            for (EntityInterface entity: input.getData()) {
                ...
                if (oTableLifeCycleData.isPresent()) {
                    Optional<AccessDetails> oAccessed = Optional.ofNullable(oTableLifeCycleData.get().getAccessed());
                ...
                Table tableProfileData = ((TableRepository) Entity.getEntityRepository(Entity.TABLE)).getLatestTableProfile(table.getFullyQualifiedName(), true);
            ...
            IndexingError error = new IndexingError()
                    ...
                    .withSuccessCount(0)
                    .withMessage(
                            "Database Service Tables Processor Encounter Failure.")
                    .withStackTrace(ExceptionUtils.exceptionStackTraceAsString(e));
            LOG.debug("[DatabaseServiceTAblesProcessor] Failed. Details: {}", JsonUtils.pojoToJson(error));
