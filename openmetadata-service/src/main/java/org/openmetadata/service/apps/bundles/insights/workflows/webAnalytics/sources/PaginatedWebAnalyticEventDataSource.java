...
public class PaginatedWebAnalyticEventDataSource
    ...
    @Getter private final WebAnalyticEventRepository repository = (WebAnalyticEventRepository) Entity.getEntityRepository(entityType);
    ...
    private ResultList<WebAnalyticEventData> read(String cursor)
            throws SearchIndexException {
        LOG.debug("[PaginatedEntityTimeSeriesSource] Fetching a Batch of Size: {} ", batchSize);
        ResultList<WebAnalyticEventData> result;
        try {
            result = repository.listWebAnalyticEventDataWithOffset(cursor, eventType, batchSize, startTs, endTs, false, true);
