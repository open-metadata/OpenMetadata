...
public class DataAssetsWorkflow {
    ...
    private final Set<String> entityTypes = Set.of(
            "table",
            "storedProcedure",
            "databaseSchema",
            "database",
    ...
    public DataAssetsWorkflow(Long timestamp, int batchSize, Optional<DataInsightsApp.Backfill> backfill, CollectionDAO collectionDAO, SearchRepository searchRepository) {
        if (backfill.isPresent()) {
            Long oldestPossibleTimestamp = TimestampUtils.getStartOfDayTimestamp(TimestampUtils.subtractDays(timestamp, retentionDays));
    ...
    private void initialize() {
        ...
        entityTypes.forEach(
                entityType -> {
                    ...
                    PaginatedEntitiesSource source = new PaginatedEntitiesSource(entityType, batchSize, fields).withName(String.format("PaginatedEntitiesSource-%s", entityType));
                    sources.add(source);
                });
