...
public class CostAnalysisWorkflow {
    ...
    @Getter private AggregatedCostAnalysisReportDataProcessor aggregatedCostAnalysisReportDataProcessor;
...
//                    List.of(TimestampUtils.getTimestampFromDateString(backfill.get().endDate()), oldestPossibleTimestamp)));
//
...
//        }
    ...
    private void initialize() throws SearchIndexException {
        PaginatedEntitiesSource databaseServices = new PaginatedEntitiesSource(Entity.DATABASE_SERVICE, batchSize, List.of("*"));
        int total = 0;

        while (!databaseServices.isDone()) {
            ResultList<DatabaseService> resultList = filterDatabaseServices(databaseServices.readNext(null));
            if (!resultList.getData().isEmpty()) {
                for (DatabaseService databaseService : resultList.getData()) {
                    ...
                    sources.add(new PaginatedEntitiesSource(Entity.TABLE, batchSize, List.of("*"), filter).withName(String.format("PaginatedEntitiesSource-%s", databaseService.getFullyQualifiedName())));
    ...
    public void process() throws SearchIndexException {
        ...
        for (PaginatedEntitiesSource source : sources) {
            ...
            Map<String, Map<String, Map<String, AggregatedCostAnalysisData>>> aggregatedCostAnalysisDataMap = new HashMap<>();
            ...
            while (!source.isDone()) {
                try {
                    ...
                    List<CostAnalysisTableData> costAnalysisTableData = databaseServiceTablesProcessor.process(resultList, contextData);
                    rawCostAnalysisReportDataList.addAll(rawCostAnalysisReportDataProcessor.process(costAnalysisTableData, contextData));
            ...
            Optional<String> processRawCostAnalysisError = processRawCostAnalysisReportData(rawCostAnalysisReportDataList, contextData);

            processRawCostAnalysisError.ifPresent(LOG::debug);

            Optional<String> processAggregatedCostAnalysisError = processAggregatedCostAnalysisReportData(aggregatedCostAnalysisDataMap, contextData);
    ...
    private Optional<String> processRawCostAnalysisReportData(List<RawCostAnalysisReportData> rawCostAnalysisReportDataList, Map<String, Object> contextData) throws SearchIndexException {
        ...
        CreateReportDataProcessor createReportdataProcessor = new CreateReportDataProcessor(rawCostAnalysisReportDataList.size(), "RawCostAnalysisReportDataProcessor");

        Optional<List<ReportData>> rawCostAnalysisReportData = Optional.empty();

        try {
            rawCostAnalysisReportData = Optional.of(createReportdataProcessor.process(
                    rawCostAnalysisReportDataList,
                    contextData));
        } catch (SearchIndexException ex) {
        ...
        if (rawCostAnalysisReportData.isPresent()) {
            ReportDataSink reportDataSink = new ReportDataSink(rawCostAnalysisReportData.get().size(), "RawCostAnalysisReportDataSink", true);
            try {
    ...
    private Optional<String> processAggregatedCostAnalysisReportData(Map<String, Map<String, Map<String, AggregatedCostAnalysisData>>> aggregatedCostAnalysisDataMap, Map<String, Object> contextData) throws SearchIndexException {
        ...
        contextData.put(REPORT_DATA_TYPE_KEY, ReportData.ReportDataType.AGGREGATED_COST_ANALYSIS_REPORT_DATA);
        ...
        Optional<List<AggregatedCostAnalysisReportData>> aggregatedCostAnalysisReportDataList = Optional.empty();
        ...
        if (aggregatedCostAnalysisReportDataList.isPresent()) {
            CreateReportDataProcessor createReportdataProcessor = new CreateReportDataProcessor(aggregatedCostAnalysisReportDataList.get().size(), "AggregatedCostAnalysisReportDataProcessor");
            Optional<List<ReportData>> aggregatedCostAnalysisReportData = Optional.empty();

            try {
                aggregatedCostAnalysisReportData = Optional.of(createReportdataProcessor.process(
                        aggregatedCostAnalysisReportDataList.get(),
                        contextData));
            } catch (SearchIndexException ex) {
            ...
            if (aggregatedCostAnalysisReportData.isPresent()) {
                ReportDataSink reportDataSink = new ReportDataSink(aggregatedCostAnalysisReportData.get().size(), "AggregatedCostAnalysisReportDataSink");
                try {
