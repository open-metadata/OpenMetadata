...
public class HybridStorageMigrationUtil {
  ...
  public void populateTagUsageFQNColumns() {
    ...
    try {
      ...
      List<Map<String, Object>> tagUsageRows = handle
          .createQuery("SELECT DISTINCT targetFQNHash, tagFQN, source FROM tag_usage WHERE targetFQN IS NULL")
          .mapToMap()
          .list();
      ...
      for (Map<String, Object> row : tagUsageRows) {
        ...
        if (targetFQN != null) {
          ...
          handle.createUpdate("UPDATE tag_usage SET targetFQN = :targetFQN WHERE targetFQNHash = :targetFQNHash AND tagFQN = :tagFQN AND source = :source")
  ...
  private int updateEntitiesOfType(String entityType) {
    try {
      ...
      do {
        ...
        ResultList<? extends EntityInterface> entities = repository.listAfter(
            null, // uriInfo
        ...
        for (EntityInterface entity : entities.getData()) {
          try {
            ...
            LOG.error("Error updating entity {} {}: {}", entityType, entity.getId(), e.getMessage());
          }
        }
  ...
  private void updateEntityJson(EntityInterface entity, EntityDAO dao, EntityRepository<?> repository) {
    try {
      ...
      if (repository.getAllowedFields().contains(FIELD_DATA_PRODUCTS)) {
        ...
        if (!entityNode.has(FIELD_DATA_PRODUCTS) && dataProducts != null && !dataProducts.isEmpty()) {
  ...
  private String findEntityFQNByHash(String fqnHash) {
    ...
    for (String entityType : Entity.getEntityList()) {
      try {
        ...
        String query = String.format("SELECT fqnHash, fullyQualifiedName FROM %s WHERE fqnHash = :fqnHash", tableName);
