...
/**
 * v203 (2.0.3) copies of the DataConsumerPolicy task-rule backfills. Installs already upgraded to
 * 2.0.0/2.0.1/2.0.2 never re-run v200 (which dropped {@code DataConsumerPolicy-CreateTask-Rule} via
 * a stale L1 name cache, #32668), so these copies repair them here. They intentionally duplicate the
 * v200 helpers rather than importing them: a version's data migration only runs once, so keeping the
 * logic self-contained per version means fixing or evolving one release train cannot silently change
 * what another already recorded.
 *
