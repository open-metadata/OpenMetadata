...
public abstract class IncidentSeverityClassifierInterface {
    ...
    private static IncidentSeverityClassifierInterface getClassifierClass(String severityClassifierClassString) {
        ...
        try {
            ...
            incidentSeverityClassifier = (IncidentSeverityClassifierInterface) severityClassifierConstructor.newInstance();
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InstantiationException |
                 InvocationTargetException e) {
            LOG.error("Error occurred while initializing the incident severity classifier. Default to LogisticRegressionClassifier", e);
            // If we encounter an error while initializing the incident severity classifier, we default to the logistic regression classifier
