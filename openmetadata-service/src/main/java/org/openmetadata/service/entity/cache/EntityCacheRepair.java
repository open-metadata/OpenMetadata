...
/**
 * Deferred re-eviction of the in-JVM entity caches ({@link EntityLocalCache#byId()} and
 * {@link EntityLocalCache#byName()}). Backstops the nanosecond race where a cache loader's
...
public final class EntityCacheRepair {
  ...
  private final Set<Object> pendingRepairs = ConcurrentHashMap.newKeySet();

  EntityCacheRepair(final EntityLocalCache local) {
    this(local, EntityCacheRepair::createExecutor);
  }

  EntityCacheRepair(
      final EntityLocalCache local, final Supplier<ScheduledExecutorService> executors) {
    this.local = local;
  ...
  void scheduleRepair(String entityType, UUID id, String fqn, String originalFqn) {
    ScheduledExecutorService current = executor;
    if (entityType != null && !current.isShutdown()) {
      RepairRequest request = new RepairRequest(entityType, id, fqn, originalFqn);
      if (pendingRepairs.add(request.coalesceKey())) {
        submit(current, request);
      }
    }
  }

  private void submit(ScheduledExecutorService current, RepairRequest request) {
    try {
      current.schedule(() -> repair(request), REPAIR_DELAY_MS, TimeUnit.MILLISECONDS);
    } catch (RejectedExecutionException e) {
      pendingRepairs.remove(request.coalesceKey());
      LOG.debug(
  ...
  private void repair(RepairRequest request) {
    ...
    try {
      if (request.id() != null) {
        local.byId().invalidate(new ImmutablePair<>(request.entityType(), request.id()));
      }
      if (request.fqn() != null) {
        local.byName().invalidate(EntityCacheKeys.name(request.entityType(), request.fqn()));
      }
      if (request.originalFqn() != null && !request.originalFqn().equals(request.fqn())) {
        local
            .byName()
            .invalidate(EntityCacheKeys.name(request.entityType(), request.originalFqn()));
      }
