...
public class WorkflowHandler {
  ...
  public void deploy(Workflow workflow) {
    ...
    if (workflow.getWorkflowDefinition().getTrigger() != null
      ...
      //          LOG.info(
      //              "Removing old trigger deployment: {} (version: {})", pd.getKey(),
      // pd.getVersion());
      ...
      //      } catch (Exception e) {
      //        LOG.warn(
      //            "Error removing old deployments for workflow {}: {}", workflowName,
      // e.getMessage());
      //      }
    }
  ...
  //  public void cleanupAllOldDeployments(int historyBatchSize, int runtimeBatchSize) {
  //    RepositoryService repositoryService = processEngine.getRepositoryService();
  //    RuntimeService runtimeService = processEngine.getRuntimeService();
  //    HistoryService historyService = processEngine.getHistoryService();
  //
  //    LOG.info("Starting global cleanup of old deployments");
  //
  //    // 1) Load all process definitions in the engine
  //    List<ProcessDefinition> allDefinitions =
  //        repositoryService.createProcessDefinitionQuery().list();
  //
  //    if (allDefinitions.isEmpty()) {
  //      LOG.info("No process definitions found, nothing to clean up");
  //      return;
  //    }
  //
  //    // 2) Group by key, and identify latest version per key
  //    Map<String, List<ProcessDefinition>> byKey =
  //        allDefinitions.stream().collect(Collectors.groupingBy(ProcessDefinition::getKey));
  //
  //    List<ProcessDefinition> definitionsToCleanup = new ArrayList<>();
  //
  //    for (Map.Entry<String, List<ProcessDefinition>> entry : byKey.entrySet()) {
  //      String key = entry.getKey();
  //      List<ProcessDefinition> defsForKey = entry.getValue();
  //
  ...
  //              .orElse(null);
  //
  //      // Mark all non-latest as "old"
  //      for (ProcessDefinition pd : defsForKey) {
  ...
  //      if (latest != null) {
  //        LOG.info(
  //            "Keeping latest definition for key '{}' (id={}, version={})",
  //            key,
  //            latest.getId(),
  //            latest.getVersion());
  //      }
  //    }
  //
  //    if (definitionsToCleanup.isEmpty()) {
  //      LOG.info("No old deployments found to clean up; every key only has a single version");
  //      return;
  //    }
  //
  //    List<String> defIdsToCleanup =
  //
  // definitionsToCleanup.stream().map(ProcessDefinition::getId).collect(Collectors.toList());
  //
  //    LOG.info(
  //        "Cleaning up {} old process definitions across {} keys",
  //        defIdsToCleanup.size(),
  //        byKey.size());
  //
  //    // 3) Delete any remaining runtime instances of old versions
  //    int runtimeDeleted =
  //        deleteRuntimeInstancesInBatches(runtimeService, defIdsToCleanup, runtimeBatchSize);
  //    LOG.info("Deleted {} runtime instances for old deployments", runtimeDeleted);
  //
  //    // 4) Delete historic instances of old versions
  //    int historicDeleted =
  //        deleteHistoricInstancesInBatches(historyService, defIdsToCleanup, historyBatchSize);
  //    LOG.info("Deleted {} historic instances for old deployments", historicDeleted);
  //
  //    // 5) Finally delete deployments for old definitions
  //    for (ProcessDefinition pd : definitionsToCleanup) {
  //      LOG.info(
  //          "Deleting old deployment {} (key={}, version={})",
  //          pd.getDeploymentId(),
  //          pd.getKey(),
  //          pd.getVersion());
  ...
  //    }
  //
  //    LOG.info("Global cleanup of old deployments finished");
  //  }

  /**
   * Batched deletion of runtime instances for ALL versions of a given key.
   * (Used at the very start for periodicBatchEntity to kill any running instances.)
   */
  private void cleanupRuntimeInstancesByKey(
      RuntimeService runtimeService, String processDefinitionKey, int batchSize, String label) {
    ...
    while (true) {
      ...
      for (ProcessInstance pi : batch) {
        runtimeService.deleteProcessInstance(
            pi.getId(), "Terminated for redeployment of periodicBatchEntity workflow");
        totalDeleted++;
      }
    }
    ...
    if (totalDeleted > 0) {
      LOG.info(
          "Terminated {} old running {} instances of {} before redeployment",
          totalDeleted,
          label,
  ...
  //  /**
  //   * Batched runtime deletion by process definition IDs (used when cleaning
  //   * specific old versions of trigger/main workflows).
  //   */
  //  private int deleteRuntimeInstancesInBatches(
  //      RuntimeService runtimeService, List<String> processDefinitionIds, int batchSize) {
  //
  //    int totalDeleted = 0;
  //
  ...
  //
  //  /**
  //   * Batched historic deletion using bulkDeleteHistoricProcessInstances.
  //   */
  //  private int deleteHistoricInstancesInBatches(
  //      HistoryService historyService, List<String> processDefinitionIds, int batchSize) {
  //
  //    int totalDeleted = 0;
  //
  //    for (String defId : processDefinitionIds) {
  ...
  //                .processDefinitionId(defId)
  //                .finished(); // only finished instances
  //
  //        List<HistoricProcessInstance> batch = query.listPage(0, batchSize);
  //        if (batch.isEmpty()) {
  //          break;
  //        }
  //
  //        List<String> ids =
  //            batch.stream().map(HistoricProcessInstance::getId).collect(Collectors.toList());
  //
  //        LOG.info(
  //            "Deleting batch of {} historic process instances for defId={} (total so far: {})",
  //            ids.size(),
  //            defId,
  //            totalDeleted);
  //
  ...
  /**
   * Cancel old timer jobs for the trigger workflow:
   * - Base trigger key
   * - Any entity-specific variants starting with triggerKey + "-"
   */
  private void cancelOldTimerJobs(ManagementService managementService, String triggerWorkflowKey) {

    // Base trigger timer jobs (all versions)
    List<Job> triggerTimerJobs =
        managementService.createTimerJobQuery().processDefinitionKey(triggerWorkflowKey).list();

    for (Job timerJob : triggerTimerJobs) {
      LOG.info("Cancelling old timer job {} for workflow {}", timerJob.getId(), triggerWorkflowKey);
      managementService.deleteJob(timerJob.getId());
    }

    // Entity-specific trigger timer jobs (e.g. SetTierForMLModelTrigger-dashboard, -table, etc.)
    List<Job> allTimerJobs = managementService.createTimerJobQuery().list();
    for (Job timerJob : allTimerJobs) {
      String processDefId = timerJob.getProcessDefinitionId();
      if (processDefId != null) {
