...
/**
 * UserApprovalTaskV2 - Uses the new Task entity system instead of Thread/Feed entities.
 *
 * <p>This is a drop-in replacement for UserApprovalTask that creates Task entities using
 * CreateTaskImpl instead of Thread entities using CreateApprovalTaskImpl.
 *
 * <p>Key differences from UserApprovalTask:
 * - Uses CreateTaskImpl instead of CreateApprovalTaskImpl
 * - Creates Task entity (new system) instead of Thread entity (legacy)
 * - Supports taskType and taskCategory configuration
 * - Integrates with TaskRepository and TaskWorkflowHandler
 */
public class UserApprovalTaskV2 implements NodeInterface {
  private final SubProcess subProcess;
  private final BoundaryEvent runtimeExceptionBoundaryEvent;
  private final List<Message> messages = new ArrayList<>();

  public UserApprovalTaskV2(
      UserApprovalTaskDefinition nodeDefinition, WorkflowConfiguration config) {
  ...
  private UserTask getUserTask(
    ...
    // Use CreateTaskImpl (new Task entity) instead of CreateApprovalTaskImpl (Thread entity)
