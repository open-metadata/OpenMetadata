...
public class FilterEntityImpl implements JavaDelegate {
  ...
  public void execute(DelegateExecution execution) {
    ...
    // Duplicate-instance supersede is intentionally NOT done here. Killing the prior run at trigger
    // time — before the workflow evaluates checkChangeDescription/checkEntityAttributes — lets a
    // no-op event that passes the entity filter but creates no task orphan a valid pending approval
    // (its Flowable process is gone while the reused approval Thread stays Open). A genuine
    // supersede already happens at task-creation time in CreateApprovalTaskImpl, which reuses the
    // entity's open approval Thread and terminates the prior run via terminateTaskProcessInstance.
    String workflowKey =
