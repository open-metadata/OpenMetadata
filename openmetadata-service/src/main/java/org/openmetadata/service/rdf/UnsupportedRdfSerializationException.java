...
/**
 * Raised when a graph cannot be expressed in the serialization the caller asked for. Extends {@link
 * IllegalArgumentException} because the requested format is caller input, so it maps onto the same
 * 400 the RDF services already return for an unusable format rather than surfacing as a 500.
 */
