...
public final class RdfProjectionStateResolver {
  ...
  private ProjectionState applyRuntimeHealth(final ProjectionState persistedState) {
    return persistedState == ProjectionState.READY && isDegraded.getAsBoolean()
        ? ProjectionState.DEGRADED
        : persistedState;
  }

  private static ProjectionState parse(final String runJson) {
    try {
      final AppRunRecord run = JsonUtils.readValue(runJson, AppRunRecord.class);
      return fromStatus(run == null ? null : run.getStatus());
    } catch (JsonParsingException exception) {
      LOG.warn("RDF projection run state is malformed", exception);
      return ProjectionState.DEGRADED;
    }
  }

  static ProjectionState fromStatus(final AppRunRecord.Status status) {
    return switch (status) {
      case SUCCESS, COMPLETED -> ProjectionState.READY;
      case FAILED, ACTIVE_ERROR, STOPPED -> ProjectionState.DEGRADED;
      case null -> ProjectionState.REBUILDING;
      case STARTED, RUNNING, ACTIVE, STOP_IN_PROGRESS, PENDING -> ProjectionState.REBUILDING;
    };
  }
}
