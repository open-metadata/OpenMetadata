...
public class ODCSMapper {
  ...
  public DataContract mapToDataContract(ODCSContract odcs, EntityReference targetEntity) {
    ...
    LOG.info("Generated contract name: {} for entity: {}", contractName, targetEntity.getFullyQualifiedName());
  ...
  private Column mapPropertyToColumn(ODCSProperty property) {
    ...
    // Set constraint - OpenMetadata supports single constraint, so prioritize PRIMARY_KEY > UNIQUE > NOT_NULL
    ...
    for (ODCSQuality quality : odcsQuality) {
      switch (quality.getType())  {
        case "text" -> {
          ...
          expectation.setName(quality.getDescription() != null ? quality.getDescription() : "Quality Rule");
          expectation.setDescription(quality.getRule());
          result.expectations.add(expectation);
        }
        case "sql" -> {
          SemanticsRule semantic = new SemanticsRule();
          semantic.setName(quality.getDescription() != null ? quality.getDescription() : "SQL Rule");
        ...
        case "library" -> {
          ...
          libExpectation.setName(quality.getDescription() != null ? quality.getDescription() : quality.getLibrary());
          libExpectation.setDescription("Library: " + quality.getLibrary() + " - Rule: " + quality.getRule());
          result.expectations.add(libExpectation);
        }
        case "custom" -> {
          QualityExpectation customExpectation = new QualityExpectation();
          customExpectation.setName(quality.getDescription() != null ? quality.getDescription() : "Custom Rule");
          customExpectation.setDescription("Custom Rule: " + quality.getRule());
          result.expectations.add(customExpectation);
        }
        default ->
          LOG.warn("Unknown Quality Rules");
      }
    }
  ...
  private Sla mapSlaProperties(List<ODCSSla> slaProperties) {
    ...
    for (ODCSSla prop : slaProperties) {
      ...
      switch (property) {
        ...
        default ->
          LOG.warn("Unknown SLA property: {}", property);
      }
    }
  ...
  private TeamMapping mapTeamMembers(List<ODCSTeam> team) {
    ...
    for (ODCSTeam member : team) {
      ...
      try {
        ...
        User user = Entity.getEntityByName(
            Entity.USER, member.getUsername(), "id", Include.NON_DELETED);
        ...
        String error = String.format("User '%s' not found in OpenMetadata. Team member skipped.", member.getUsername());
        mappingErrors.add(error);
        LOG.warn(error);
        continue;
      } catch (Exception e) {
        // Other error - add to errors and skip
        String error = String.format("Error looking up user '%s': %s", member.getUsername(), e.getMessage());
        mappingErrors.add(error);
        LOG.error(error);
        continue;
      }
