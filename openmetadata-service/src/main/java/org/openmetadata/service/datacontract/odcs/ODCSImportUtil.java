...
public class ODCSImportUtil {
   ...
   */
  public static ODCSImportResponse importContract(
      String odcsContent, String format, UUID entityId) {
    ...
    try {
      ...
      if (!contract.getApiVersion().matches("v3\\.0\\.[0-2]")) {
        throw new DataContractImportException(
            "Only ODCS versions v3.0.0, v3.0.1, v3.0.2 are supported. Found: "
      ...
      throw new DataContractImportException(
          "Invalid " + format.toUpperCase() + " content: " + e.getMessage(), e);
    }
  }
  ...
  private static void buildImportReport(ODCSContract odcsContract, ODCSImportReport report) {
    ...
    if (odcsContract.getRoles() != null && !odcsContract.getRoles().isEmpty()) {
      ...
      report.addWarning(
          "Access roles stored as custom property - OpenMetadata manages permissions separately");
    }
