...
public class TagRepository extends EntityRepository<Tag> {
  ...
  @Override
  public Tag setInheritedFields(Tag tag, Fields fields) {
  ...
  private List<EntityReference> getChildren(Tag entity) {
    List<CollectionDAO.EntityRelationshipRecord> ids = findTo(entity.getId(), TAG, Relationship.CONTAINS, TAG);
  ...
  public class TagUpdater extends EntityUpdater {
    ...
    private void invalidateTags(UUID tagId) {
      ...
      List<EntityRelationshipRecord> tagRecords = findTo(tagId, TAG, Relationship.CONTAINS, TAG);
