...
public class EntityJsonUpdateHandler {
  ...
  public void handleTagRename(String oldTagFQN, String newTagFQN, int source) {
    ...
    List<String> affectedEntities =
        daoCollection.tagUsageDAO().getTargetFQNHashForTag(oldTagFQNHash);
  ...
  public void handleOwnerRename(
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      updateEntityJsonRenameOwner(
          rel.getId(), rel.getType(), ownerId, oldName, newName, oldFQN, newFQN);
    }
  }
   ...
   */
  public void handleDomainRename(
      UUID domainId, String oldName, String newName, String oldFQN, String newFQN) {
    ...
    for (CollectionDAO.EntityRelationshipRecord rel : relationships) {
      updateEntityJsonRenameDomain(
          rel.getId(), rel.getType(), domainId, oldName, newName, oldFQN, newFQN);
    }
  }
  ...
  private void updateEntityJsonRemoveTag(String targetFQNHash, String tagFQN) {
    try {
      ...
      LOG.error(
          "Error removing tag {} from entity {}: {}", tagFQN, targetFQNHash, e.getMessage(), e);
    }
  }
  ...
  private void updateEntityJsonRenameTag(String targetFQNHash, String oldTagFQN, String newTagFQN) {
    try {
      LOG.debug(
          "Would rename tag {} to {} in entity with hash {}", oldTagFQN, newTagFQN, targetFQNHash);
      ...
      LOG.error(
          "Error renaming tag {} to {} in entity {}: {}",
          oldTagFQN,
          newTagFQN,
          targetFQNHash,
          e.getMessage(),
  ...
  private void updateEntityJsonRemoveOwner(UUID entityId, String entityType, UUID ownerId) {
    try {
      ...
      LOG.error(
          "Error removing owner {} from entity {} JSON: {}", ownerId, entityId, e.getMessage(), e);
    }
  }

  private void updateEntityJsonRenameOwner(
      UUID entityId,
      String entityType,
      UUID ownerId,
      String oldName,
    ...
    try {
      ...
      LOG.error(
          "Error renaming owner {} in entity {} JSON: {}", ownerId, entityId, e.getMessage(), e);
    }
  }
  ...
  private void updateEntityJsonRemoveDomain(UUID entityId, String entityType, UUID domainId) {
    try {
      ...
      LOG.error(
          "Error removing domain {} from entity {} JSON: {}",
          domainId,
          entityId,
          e.getMessage(),
          e);
  ...
  private void updateEntityJsonRenameDomain(
      UUID entityId,
      String entityType,
      UUID domainId,
      String oldName,
    ...
    try {
      ...
      LOG.error(
          "Error renaming domain {} in entity {} JSON: {}", domainId, entityId, e.getMessage(), e);
    }
  }
}
