...
public interface DeletionLockDAO {
  ...
  DeletionLock findByEntity(@BindUUID("entityId") UUID entityId, @Bind("entityType") String entityType);
  
  @SqlQuery(
      "SELECT * FROM entity_deletion_lock WHERE entityFqn LIKE :fqnPrefix || '%'")
  ...
  @SqlQuery(
      "SELECT * FROM entity_deletion_lock WHERE lockedAt < :staleTime")
