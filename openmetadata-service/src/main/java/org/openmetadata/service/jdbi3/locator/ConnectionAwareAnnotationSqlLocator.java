...
public class ConnectionAwareAnnotationSqlLocator implements SqlLocator {
  ...
  private Optional<String> getAnnotationValue(Method method) {
    return JdbiOptionals.findFirstPresent(
        () ->
            getSqlForConnection(
                method.getAnnotationsByType(ConnectionAwareSqlUpdate.class),
                ConnectionAwareSqlUpdate::connectionType,
                ConnectionAwareSqlUpdate::value),
        () ->
            getSqlForConnection(
                method.getAnnotationsByType(ConnectionAwareSqlQuery.class),
                ConnectionAwareSqlQuery::connectionType,
                ConnectionAwareSqlQuery::value),
        () ->
            getSqlForConnection(
                method.getAnnotationsByType(ConnectionAwareSqlBatch.class),
                ConnectionAwareSqlBatch::connectionType,
                ConnectionAwareSqlBatch::value),
        () -> SqlAnnotations.getAnnotationValue(method));
  }

  private <A> Optional<String> getSqlForConnection(
      A[] annotations,
      Function<A, ConnectionType> connectionTypeExtractor,
      Function<A, String> sqlExtractor) {
    if (annotations == null || annotations.length == 0) {
      return Optional.empty();
    }
    return Arrays.stream(annotations)
        .filter(annotation -> connectionTypeExtractor.apply(annotation).equals(connectionType))
        .map(sqlExtractor)
        .findFirst();
  }
}
