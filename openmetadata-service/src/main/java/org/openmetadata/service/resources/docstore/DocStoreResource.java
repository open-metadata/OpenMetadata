...
public class DocStoreResource extends EntityResource<Document, DocumentRepository> {
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllDocumentVersionsByTimestamp",
      summary = "List all document versions within a time range",
      description =
          "Get a paginated list of all document entity versions within a given time range "
  ...
  public ResultList<Document> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(description = "End timestamp in milliseconds since epoch", required = true)
          ...
          long endTs,
      @Parameter(
              description = "Limit the number of documents returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
          @Min(value = 1, message = "must be greater than or equal to 1")
          @Max(value = 1000000, message = "must be less than or equal to 1000000")
          @QueryParam("limit")
          int limitParam,
      @Parameter(description = "Returns list of document versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(description = "Returns list of document versions after this cursor")
          @QueryParam("after")
          String after) {
