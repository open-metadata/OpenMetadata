...
@Tag(
  name = "Containers",
  description = "A Container is an abstraction for any path(including the top level eg. bucket in S3) storing " +
...
public class ContainerResource extends EntityResource<Container, ContainerRepository> {
  ...
  @Operation(
    ...
    description = "Get a list of containers, optionally filtered by `service` it belongs to. Use `fields` " +
  ...
  public ResultList<Container> list(
    ...
    @Parameter(description = "Limit the number containers returned. (1 to 1000000, default = 10)") @DefaultValue(
      "10"
  ...
  @Operation(
    ...
    responses = {
      ...
      @ApiResponse(responseCode = "404", description = "Container for instance {id} and version {version} is not found")
    }
  )
