...
public class DomainResource extends EntityResource<Domain, DomainRepository> {
  ...
  public ResultList<Domain> list(
    ...
    @Parameter(description = "Returns list of Domain after this cursor", schema = @Schema(type = "string")) @QueryParam(
      "after"
    ) String after
  ) {
  ...
  private Domain getDomain(CreateDomain create, String user) {
    ...
    return repository
      ...
      .withParent(Entity.getEntityReference(getEntityReference(Entity.DOMAIN, create.getParent()), Include.NON_DELETED))
