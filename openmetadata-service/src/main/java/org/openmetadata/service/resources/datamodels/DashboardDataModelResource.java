...
public class DashboardDataModelResource
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllDashboardDataModelVersionsByTimestamp",
      summary = "List all dashboard data model versions within a time range",
      description =
          "Get a paginated list of all dashboard data model entity versions within a given time range "
              + "specified by `startTs` and `endTs` in milliseconds since epoch. "
              + "This endpoint requires admin privileges.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of dashboard data model versions",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = DashboardDataModelList.class)))
      })
  public ResultList<DashboardDataModel> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(
              description =
                  "Limit the number of dashboard data models returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
          @Min(value = 1, message = "must be greater than or equal to 1")
          @Max(value = 1000000, message = "must be less than or equal to 1000000")
          @QueryParam("limit")
          int limitParam,
      @Parameter(description = "Returns list of dashboard data model versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(description = "Returns list of dashboard data model versions after this cursor")
          @QueryParam("after")
          String after) {
