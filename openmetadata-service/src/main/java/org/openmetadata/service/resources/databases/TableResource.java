...
public class TableResource extends EntityResource<Table, TableRepository> {
  ...
  @Operation(
      operationId = "listFilteredTableVersions",
      summary = "List table versions with filters",
      description =
          "Get a filtered list of versions of a table identified by `Id`. "
              + "Supports filtering by limit, timestamp range, and field changes.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of filtered table versions",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = EntityHistory.class)))
  ...
  public EntityHistory listVersionsFiltered(
      ...
      @Parameter(
              description =
                  "Field name to filter versions by changes (e.g., 'tier', 'tags', 'columns', 'description')",
              schema = @Schema(type = "string"))
