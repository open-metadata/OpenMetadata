...
public class AppMarketPlaceResource
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllAppMarketPlaceDefinitionVersionsByTimestamp",
      summary = "List all app marketplace definition versions within a time range",
      description =
          "Get a paginated list of all app marketplace definition entity versions within a given time range "
              + "specified by `startTs` and `endTs` in milliseconds since epoch. "
              + "This endpoint requires admin privileges.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of app marketplace definition versions",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = AppMarketPlaceDefinitionList.class)))
      })
  public ResultList<AppMarketPlaceDefinition> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(
              description =
                  "Limit the number of app marketplace definitions returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
      ...
      @Parameter(
              description =
                  "Returns list of app marketplace definition versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(
              description = "Returns list of app marketplace definition versions after this cursor")
          @QueryParam("after")
          String after) {
