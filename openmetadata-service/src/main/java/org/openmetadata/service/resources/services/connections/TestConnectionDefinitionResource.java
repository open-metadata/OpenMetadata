...
public class TestConnectionDefinitionResource
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllTestConnectionDefinitionVersionsByTimestamp",
      summary = "List all test connection definition versions within a time range",
      description =
          "Get a paginated list of all test connection definition entity versions within a given time range "
              + "specified by `startTs` and `endTs` in milliseconds since epoch. "
              + "This endpoint requires admin privileges.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of test connection definition versions",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = TestConnectionDefinitionList.class)))
      })
  public ResultList<TestConnectionDefinition> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(
              description =
                  "Limit the number of test connection definitions returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
      ...
      @Parameter(
              description =
                  "Returns list of test connection definition versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(
              description = "Returns list of test connection definition versions after this cursor")
          @QueryParam("after")
          String after) {
