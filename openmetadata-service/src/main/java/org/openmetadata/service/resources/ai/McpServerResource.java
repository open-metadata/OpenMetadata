...
@Tag(
    name = "MCP Servers",
    description =
        "`MCP Servers` are Model Context Protocol servers that expose tools, resources, and prompts for AI applications. "
            + "They enable AI governance including risk assessment, compliance tracking, and shadow AI detection.")
...
public class McpServerResource extends EntityResource<McpServer, McpServerRepository> {
  ...
  public McpServer addHref(UriInfo uriInfo, McpServer mcpServer) {
    super.addHref(uriInfo, mcpServer);
    Entity.withHref(uriInfo, mcpServer.getUsedByApplications());
    return mcpServer;
  }

  public McpServerResource(Authorizer authorizer, Limits limits) {
    super(Entity.MCP_SERVER, authorizer, limits);
  }
  ...
  @Operation(
      ...
      description =
          "Get a list of MCP servers. Use `fields` parameter to get only necessary fields. "
              + "Use cursor-based pagination to limit the number of entries in the list using `limit` and `before` or `after` query params.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            ...
            content =
                @Content(
                    ...
                    schema = @Schema(implementation = McpServerList.class)))
      })
  public ResultList<McpServer> list(
      ...
      @Parameter(
          ...
          @QueryParam("fields")
          String fieldsParam,
      @Parameter(description = "Limit the number of servers returned. (1 to 1000000, default = 10)")
          @DefaultValue("10")
      ...
      responses = {
        @ApiResponse(
            ...
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = McpServer.class))),
        @ApiResponse(
            responseCode = "404",
  ...
  public Response create(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      @Valid CreateMcpServer create) {
    McpServer mcpServer =
        mapper.createToEntity(create, securityContext.getUserPrincipal().getName());
    return create(uriInfo, securityContext, mcpServer);
  }
  ...
  public Response createOrUpdate(
    ...
    McpServer mcpServer =
        mapper.createToEntity(create, securityContext.getUserPrincipal().getName());
    return createOrUpdate(uriInfo, securityContext, mcpServer);
  }
  ...
  public Response restoreMcpServer(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
