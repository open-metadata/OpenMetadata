...
public class DataInsightChartResource extends EntityResource<DataInsightChart, DataInsightChartRepository> {
  ...
  public EntityHistory listVersions(
    @Context UriInfo uriInfo,
    @Context SecurityContext securityContext,
    @Parameter(description = "Id of the data insight chart", schema = @Schema(type = "UUID")) @PathParam("id") UUID id
  ) {
  ...
  public Response listDataInsightChartResult(
    ...
    ) @NonNull @QueryParam("dataInsightChartName") DataInsightChartResult.DataInsightChartType dataInsightChartName,
    @Parameter(description = "Query filter for the aggregation") @QueryParam("queryFilter") String queryFilter,
    @Parameter(description = "Limit the number of results returned.") @DefaultValue("10") @QueryParam(
      "size"
    ) Integer size,
    @Parameter(description = "Offset the results returned. (default = 0)") @DefaultValue("0") @QueryParam(
      "from"
    ) Integer from,
    @Parameter(
      description = "Specify the elasticsearch index to fetch data from",
      schema = @Schema(implementation = DataReportIndex.class)
    ) @NonNull @QueryParam("dataReportIndex") String dataReportIndex,
    @Parameter(
    ...
    @Parameter(description = "Filter after the given start timestamp", schema = @Schema(type = "number")) @QueryParam(
      "startTs"
    ) Long startTs,
    @Parameter(description = "Filter before the given end timestamp", schema = @Schema(type = "number")) @QueryParam(
      "endTs"
    ) Long endTs
  )
