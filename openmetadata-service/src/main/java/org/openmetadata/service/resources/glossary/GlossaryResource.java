...
public class GlossaryResource extends EntityResource<Glossary, GlossaryRepository> {
  ...
  @Operation(
    ...
    description = "Get a list of glossaries. Use `fields` parameter to get only necessary fields. " +
  ...
  public ResultList<Glossary> list(
    ...
    @Parameter(description = "Limit the number glossaries returned. (1 to 1000000, default = 10)") @DefaultValue(
      "10"
  ...
  public Glossary getByName(
    @Context UriInfo uriInfo,
    @Parameter(description = "Name of the glossary", schema = @Schema(type = "string")) @PathParam("name") String name,
    @Context SecurityContext securityContext,
    @Parameter(
      description = "Fields requested in the returned resource",
      schema = @Schema(type = "string", example = FIELDS)
  ...
  @Operation(
    ...
    responses = {
      ...
      @ApiResponse(responseCode = "404", description = "Glossary for instance {id} and version {version} is not found")
    }
  )
