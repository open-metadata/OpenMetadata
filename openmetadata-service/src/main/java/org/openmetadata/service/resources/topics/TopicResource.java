...
public class TopicResource extends EntityResource<Topic, TopicRepository> {
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllTopicVersionsByTimestamp",
      summary = "List all topic versions within a time range",
      description =
          "Get a paginated list of all topic entity versions within a given time range "
  ...
  public ResultList<Topic> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(description = "End timestamp in milliseconds since epoch", required = true)
          @QueryParam("endTs")
          long endTs,
      @Parameter(description = "Limit the number of topics returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
          @Min(value = 1, message = "must be greater than or equal to 1")
          @Max(value = 1000000, message = "must be less than or equal to 1000000")
          @QueryParam("limit")
          int limitParam,
      @Parameter(description = "Returns list of topic versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(description = "Returns list of topic versions after this cursor")
          @QueryParam("after")
          String after) {
  ...
  @Operation(
      ...
      description =
          "Create or update multiple topics in a single operation. "
              + "Returns a BulkOperationResult with success/failure details for each topic.",
      responses = {
        @ApiResponse(
            responseCode = "200",
