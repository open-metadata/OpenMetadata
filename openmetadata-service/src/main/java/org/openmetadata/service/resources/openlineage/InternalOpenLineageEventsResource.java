...
public class InternalOpenLineageEventsResource {
  ...
  @Operation(
      ...
      description =
          "Get OpenLineage Events based on query parameters such as runId, eventType, or unprocessed.",
      responses = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(
      ...
      responses = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "OpenLineageEvent {lineageEventId} is not found"),
        @ApiResponse(responseCode = "400", description = "Bad request")
      })
  public Response deleteLineageEvent(@PathParam("id") UUID id) {
    OpenLineageWrappedEvent event =
        dao.queryEvents(String.valueOf(id), null, null).getData().stream().findFirst().orElse(null);
