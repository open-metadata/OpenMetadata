...
public class WorkflowDefinitionResource
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllWorkflowDefinitionVersionsByTimestamp",
      summary = "List all workflow definition versions within a time range",
      description =
          "Get a paginated list of all workflow definition entity versions within a given time range "
      ...
      responses = {
        @ApiResponse(
            ...
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = WorkflowDefinitionList.class)))
      })
  public ResultList<WorkflowDefinition> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(
              description =
                  "Limit the number of workflow definitions returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
          @Min(value = 1, message = "must be greater than or equal to 1")
          @Max(value = 1000000, message = "must be less than or equal to 1000000")
          @QueryParam("limit")
          int limitParam,
      @Parameter(description = "Returns list of workflow definition versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(description = "Returns list of workflow definition versions after this cursor")
          @QueryParam("after")
          String after) {
