...
@Tag(
  name = "Dashboards",
  description = "Dashboards are computed from data and visually present data, metrics, and " +
...
public class DashboardResource extends EntityResource<Dashboard, DashboardRepository> {
  ...
  public ResultList<Dashboard> list(
    ...
    @Parameter(description = "Limit the number dashboards returned. (1 to 1000000, default = 10)") @DefaultValue(
      "10"
  ...
  public Dashboard getByName(
    @Context UriInfo uriInfo,
    @Parameter(description = "Fully qualified name of the dashboard", schema = @Schema(type = "string")) @PathParam(
      "fqn"
    ) String fqn,
    @Context SecurityContext securityContext,
    @Parameter(
  ...
  @Operation(
    ...
    responses = {
      ...
      @ApiResponse(responseCode = "404", description = "Dashboard for instance {id} and version {version} is not found")
    }
  )
