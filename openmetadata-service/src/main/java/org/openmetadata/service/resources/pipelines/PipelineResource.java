...
public class PipelineResource extends EntityResource<Pipeline, PipelineRepository> {
  ...
  @Operation(
      operationId = "listFilteredPipelineVersions",
      summary = "List pipeline versions with filters",
      description =
          "Get a filtered list of versions of a pipeline identified by `Id`. "
              + "Supports filtering by limit, timestamp range, and field changes.",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of filtered pipeline versions",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = EntityHistory.class)))
