...
public class PipelineResource extends EntityResource<Pipeline, PipelineRepository> {
  ...
  public ResultList<Pipeline> list(
    ...
    @Parameter(description = "Limit the number pipelines returned. (1 to 1000000, default = 10)") @DefaultValue(
      "10"
  ...
  public Pipeline getByName(
    @Context UriInfo uriInfo,
    @Parameter(description = "Fully qualified name of the pipeline", schema = @Schema(type = "string")) @PathParam(
      "fqn"
    ) String fqn,
    @Context SecurityContext securityContext,
    @Parameter(
  ...
  @Operation(
    ...
    responses = {
      ...
      @ApiResponse(responseCode = "404", description = "Pipeline for instance {id} and version {version} is not found")
    }
  )
  ...
  @Operation(
    ...
    description = "Get a list of pipeline status." +
    "parameter to get only necessary fields. Use cursor-based pagination to limit the number " +
    "entries in the list using `limit` and `before` or `after` query params.",
    responses = {
      @ApiResponse(
  ...
  public Pipeline deletePipelineStatus(
    ...
    @Parameter(description = "Timestamp of the pipeline status", schema = @Schema(type = "long")) @PathParam(
      "timestamp"
    ) Long timestamp
  ) {
    OperationContext operationContext = new OperationContext(entityType, MetadataOperation.EDIT_STATUS);
