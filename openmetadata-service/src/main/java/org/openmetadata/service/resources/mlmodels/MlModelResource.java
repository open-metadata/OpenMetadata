...
public class MlModelResource extends EntityResource<MlModel, MlModelRepository> {
  ...
  @Operation(
    ...
    description = "Get a list of mlmodels, optionally filtered by `service` it belongs to. Use `fields` " +
  ...
  public ResultList<MlModel> list(
    ...
    @Parameter(description = "Limit the number models returned. (1 to 1000000, default = 10)") @DefaultValue("10") @Min(
      0
    ...
    @Parameter(description = "Returns list of models after this cursor", schema = @Schema(type = "string")) @QueryParam(
      "after"
    ) String after,
    @Parameter(
  ...
  public MlModel getByName(
    @Context UriInfo uriInfo,
    @Parameter(description = "Fully qualified name of ML Model", schema = @Schema(type = "string")) @PathParam(
      "fqn"
    ) String fqn,
    @Context SecurityContext securityContext,
    @Parameter(
  ...
  @Operation(
    ...
    responses = {
      ...
      @ApiResponse(responseCode = "404", description = "ML Model for instance {id} and version {version} is not found")
    }
  )
