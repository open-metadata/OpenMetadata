...
public class TestDefinitionResource
  ...
  @Path("/versions")
  @Operation(
      operationId = "listAllTestDefinitionVersionsByTimestamp",
      summary = "List all test definition versions within a time range",
      description =
          "Get a paginated list of all test definition entity versions within a given time range "
      ...
      responses = {
        @ApiResponse(
            ...
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = TestDefinitionList.class)))
      })
  public ResultList<TestDefinition> listAllVersionsByTimestamp(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      ...
      @Parameter(
              description =
                  "Limit the number of test definitions returned (1 to 1000000, default = 10)")
          @DefaultValue("10")
          @Min(value = 1, message = "must be greater than or equal to 1")
          @Max(value = 1000000, message = "must be less than or equal to 1000000")
          @QueryParam("limit")
          int limitParam,
      @Parameter(description = "Returns list of test definition versions before this cursor")
          @QueryParam("before")
          String before,
      @Parameter(description = "Returns list of test definition versions after this cursor")
          @QueryParam("after")
          String after) {
