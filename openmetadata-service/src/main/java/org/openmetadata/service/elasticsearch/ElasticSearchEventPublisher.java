...
public class ElasticSearchEventPublisher extends AbstractEventPublisher {
  ...
  private void updateTag(ChangeEvent event) throws IOException {
    ...
    switch (event.getEventType()) {
      ...
      case ENTITY_DELETED:
        ...
        String[] indexes =
            new String[] {
              indexResolver.indexInfo(IndexType.TABLE_SEARCH_INDEX).getIndexName(),
              indexResolver.indexInfo(IndexType.TOPIC_SEARCH_INDEX).getIndexName(),
              indexResolver.indexInfo(IndexType.DASHBOARD_SEARCH_INDEX).getIndexName(),
              indexResolver.indexInfo(IndexType.PIPELINE_SEARCH_INDEX).getIndexName(),
              indexResolver.indexInfo(IndexType.GLOSSARY_SEARCH_INDEX).getIndexName(),
              indexResolver.indexInfo(IndexType.MLMODEL_SEARCH_INDEX).getIndexName()
            };
  ...
  private UpdateRequest updateRequests(String entityType, String entityId, Script script) {
    IndexTypeInfo typeInfo = ElasticSearchIndexDefinition.getIndexMappingByEntityType(entityType, indexResolver);
    UpdateRequest updateRequest = new UpdateRequest(typeInfo.getIndexInfo().getIndexName(), entityId).script(script);
    return updateRequest;
  }
