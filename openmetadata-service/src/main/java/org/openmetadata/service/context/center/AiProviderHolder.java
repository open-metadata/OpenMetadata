  ...
  private static AiProvider resolveConfigured(final String className) {
    ...
    try {
      ...
      LOG.warn(
          ...
          className);
      resolved = new LlmAiProvider();
    } catch (ClassCastException notAProvider) {
      LOG.error(
          "Configured aiProviderClass '{}' does not implement AiProvider; using the built-in LLM provider",
          className,
          notAProvider);
      resolved = new LlmAiProvider();
