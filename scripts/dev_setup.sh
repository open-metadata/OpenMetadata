...
ensure_python() {
  ...
  if [ "$DO_TOOLS" -eq 0 ] || [ "$CHECK_ONLY" -eq 1 ]; then
    warn "No bootstrap-compatible Python found (need one of: $PYTHON_BOOTSTRAP_SUPPORTED)${found:+; system python3 is $(python_mm "$found")}."
    return 1
  fi
...
run_prerequisite_check() {
  ...
  if [ "$bash_major" -lt 4 ] 2>/dev/null; then
    warn "Skipping 'make prerequisites': it needs bash >= 4 and only bash $bash_major is available (brew install bash)."
    return 0
  fi
  if "$bash_bin" "$REPO_ROOT/scripts/check_prerequisites.sh"; then
    ok "All prerequisites verified"
  else
    warn "'make prerequisites' reported problems — see the output above."
  fi
}
...
write_env_file() {
  {
    ...
    [ -n "$NODE_BIN_DIR" ] && echo "export PATH=\"$NODE_BIN_DIR:\$PATH\""
    if [ "$OS" = macos ] && have brew; then
      local nprefix
      nprefix="$(brew --prefix "node@$NODE_VERSION" 2>/dev/null || true)"
      [ -n "$nprefix" ] && [ -d "$nprefix/bin" ] && echo "export PATH=\"$nprefix/bin:\$PATH\""
    fi
    echo "[ -f \"$REPO_ROOT/$VENV_DIR/bin/activate\" ] && . \"$REPO_ROOT/$VENV_DIR/bin/activate\""
  } > "$ENV_FILE"
  ok "Wrote $ENV_FILE"
}
...
configure_mise() {
  ...
  # otherwise undo .dev-env.local.sh. Keep the override local to this checkout.
  {
    printf '[env]\n'
    printf 'JAVA_HOME = "%s"\n' "$JAVA_HOME"
    printf '_.path = ["%s/bin", "%s"]\n' "$JAVA_HOME" "$NODE_BIN_DIR"
