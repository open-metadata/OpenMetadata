...
schema_directory='openmetadata-core/src/main/resources/json/schema/'
...
getTypes(){
    ...
    for file_with_dir in $(find $schema_directory  -name "*.json" | sed -e 's/openmetadata-core\/src\/main\/resources\/json\/schema\///g')
    do
        joblist=$(jobs | wc -l)
        while (( ${joblist} >= 30 ))
            do
