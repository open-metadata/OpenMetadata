...
/**
 * Helper that registers the per-entity custom property describe blocks for
 * the entities passed in. Single source of truth for the per-entity test body.
 *
 * Used by:
 *   - e2e/Pages/TableCustomProperty.spec.ts                       (PR: [Table])
 *   - e2e/Pages/DashboardAdvanceSearchCustomProperty.spec.ts      (PR: [Dashboard])
 *   - e2e/stress/CustomPropertiesAllEntities.spec.ts              (Stress: 17 others)
 *
 * The Dashboard branch contains the 7 advanced-search describes + the 2
 * search-config tests — those fire only when Dashboard is in the entity list,
 * so they run on PR via the Dashboard spec but NOT in stress (Dashboard is
 * excluded there to avoid redundancy with PR).
 */
...
import { CustomPropertiesPageObject } from '../e2e/PageObject/Explore/CustomPropertiesPageObject';
import { RightPanelPageObject } from '../e2e/PageObject/Explore/RightPanelPageObject';
...
export const ALL_ENTITIES: CRUDEntity[] = [
  // Part-1 entities
...
export const registerCustomPropertiesEntityTests = (entities: CRUDEntity[]) => {
  entities.forEach(({ key, makeInstance }) => {
    ...
    test.describe
      ...
      if (makeInstance !== null) {
        test(`Set & Update all CP types on ${entity.name}`, async ({
          page,
        }) => {
      ...
      if (key === 'entity_table') {
        ...
        test('Enum: Set Value, Verify, Remove Value', async ({ page }) => {
          test.slow();
          const propertyName =
            mainEntity.customPropertyValue[CustomPropertyTypeByName.ENUM]
              .property.name;
      ...
      if (key === 'entity_container') {
        test('should show No Data placeholder when hyperlink has no value', async ({
          ...
          const propertyName =
            mainEntity.customPropertyValue[
              CustomPropertyTypeByName.HYPERLINK_CP
            ].property.name;
      ...
      if (key === 'entity_dashboard') {
        ...
        test.describe('Dashboard CP Advanced Search - Table CP', () => {
          ...
          test('Table CP - Name column with all operators', async ({
            page,
          }) => {
            test.slow();
          ...
          test('Table CP - Role column with all operators', async ({
            page,
          }) => {
            test.slow();
          ...
          test('Table CP - Sr No column with all operators', async ({
            page,
          }) => {
            test.slow();
        ...
        test('Create custom property and configure search for Dashboard', async ({
          ...
          await test.step('Create and assign custom property to Dashboard', async () => {
            ...
            await addCustomPropertiesForEntity({
              ...
              customPropertyData:
                CUSTOM_PROPERTIES_ENTITIES['entity_dashboard'],
              customType: 'String',
            });
