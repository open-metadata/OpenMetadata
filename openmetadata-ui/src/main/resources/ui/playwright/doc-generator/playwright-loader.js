...
`;
  ...
  try {
    ...
    console.log(
      `   Running 'npx playwright test --list --reporter=json' using temp config in ${projectRoot}...`
    );

    const output = execSync(
      `npx playwright test --list --reporter=json --config=${tempConfigName}`,
      {
        cwd: projectRoot,
        encoding: 'utf-8',
  ...
  try {
    ...
    function visit(node) {
      if (ts.isCallExpression(node)) {
        ...
        // Identify test(...) calls
        const isTest =
          (ts.isIdentifier(expr) && expr.text === 'test') ||
          (ts.isPropertyAccessExpression(expr) &&
            expr.expression.getText() === 'test' &&
            ['skip', 'fixme', 'only'].includes(expr.name.getText()));
...
function flattenTests(suite, testDir) {
  ...
  if (suite.specs) {
    suite.specs.forEach((spec) => {
      ...
      const description =
        extractJSDoc(absolutePath, specLine) || extractBehavior(specTitle);
...
function loadTestsFromPlaywright(testDir) {
  ...
  function processFileSuite(fileSuite) {
    ...
    rootTests.forEach(
      (t) => t.tags && t.tags.forEach((tag) => fileTags.add(tag))
    );
