...
ALL_ENTITIES.forEach(({ key, makeInstance }) => {
  ...
  test.describe
    ...
    if (key === 'entity_table') {
      ...
      test('entityReference with quoted FQN renders clean link text and href (Issue #25765)', async ({
        page,
      }) => {
        test.slow();
        const { apiContext, afterAction } = await getApiContext(page);
        const propertyName =
          mainEntity.customPropertyValue[
            CustomPropertyTypeByName.ENTITY_REFERENCE
          ].property.name;
        const testUser = users[1];
        const plainName = testUser.responseData.name;
        ...
        const userElement = page.getByTestId(plainName);
        const isUserVisible = await userElement.isVisible();
        if (!isUserVisible) {
          await page.getByTestId('custom_properties').click();
        }

        const link = page.getByTestId(propertyName).getByRole('link').first();
