...
test.describe('Soft Delete User', () => {
  test('Create and soft delete 30 users', async ({ browser, adminPage }) => {
    ...
    try {
      ...
      const deletedUserResponsePromise = adminPage.waitForResponse(
        (response) => {
          ...
          return hasValidParams && method === 'GET';
        }
      );
      ...
      if (await nextButton.isVisible()) {
        ...
        throw new Error('Next button is not visible. Test failed.');
      }
    } finally {
      // Cleanup: Permanently delete all created users
      for (const testUser of users) {
