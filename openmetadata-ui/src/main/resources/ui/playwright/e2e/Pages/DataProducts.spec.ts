...
test.describe('Data Products', () => {
  ...
  test('Empty State - No Data Products', async ({ page }) => {
    ...
    await test.step('Verify empty state is shown', async () => {
      ...
      await expect(page.getByTestId('no-data-placeholder').getByRole('button', { name: 'Add Data Product' })).toBeVisible();
    });

    await test.step('Click add button from empty state', async () => {
      await page.getByTestId('no-data-placeholder').getByRole('button', { name: 'Add Data Product' }).click();
