  ...
  async ({ page }) => {
    ...
    await page.route(
      ...
      async (route) => {
        ...
        const originalNodeCount = data.nodes
        ...
        // Diagnostic: confirms the route handler runs and the slice is
        // applied. Surfaces in Playwright output as one line per intercepted
        // view (service / domain / dataProduct) so it's obvious when the
        // upstream payload already fits under the cap vs when we sliced.
