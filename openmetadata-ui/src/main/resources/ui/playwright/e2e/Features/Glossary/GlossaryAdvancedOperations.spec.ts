...
test.describe('Glossary Advanced Operations', () => {
  ...
  // GlossaryTerm Domain Entity Rules tests - must run serially as they modify shared backend state
  test.describe.serial('GlossaryTerm Domain Entity Rules', () => {
    // T-Domain-01: Verify glossary term allows multiple domains when entity rules are disabled
    test('should allow multiple domain selection when entity rules are disabled', async ({
      page,
    }) => {
      test.slow(true);
      const { apiContext, afterAction } = await getApiContext(page);
      ...
      try {
        ...
        // Disable entity rules (allow multiple domain selection)
        ...
        await expect(
          page
            ...
            .filter({ hasText: domain1.data.displayName })
        ).toBeVisible();
        await expect(
          page
            .getByTestId('domain-link')
            .filter({ hasText: domain2.data.displayName })
        ).toBeVisible();
      } finally {
    ...
    // T-Domain-02: Verify entity rules restrict to single domain selection for glossary term
    test('should enforce single domain selection when entity rules are enabled', async ({
      page,
    }) => {
      test.slow(true);
      const { apiContext, afterAction } = await getApiContext(page);
      ...
      try {
        ...
        // Enable entity rules (single domain selection)
        await apiContext.put(`/api/v1/system/settings`, {
          data: {
            config_type: 'entityRulesSettings',
            config_value: {
        ...
        // Disable entity rules to not affect other tests
        await apiContext.put(`/api/v1/system/settings`, {
          data: {
            config_type: 'entityRulesSettings',
            config_value: {
