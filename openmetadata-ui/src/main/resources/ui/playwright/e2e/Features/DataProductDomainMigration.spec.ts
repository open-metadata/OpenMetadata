...
test.describe('Data Product Domain Migration', () => {
  ...
  test('Changing data product domain via UI fires a single minimal PATCH', async ({
    page,
  }) => {
    test.slow();
    ...
    const uiDomain = new Domain({
      name: `ui_target_domain_${shortId}`,
      displayName: `UI Target Domain ${shortId}`,
      description: 'UI target domain for single-patch regression test',
      domainType: 'Aggregate',
      fullyQualifiedName: `ui_target_domain_${shortId}`,
    });
    const uiSourceDomain = new Domain({
      name: `ui_source_domain_${shortId}`,
      displayName: `UI Source Domain ${shortId}`,
      description: 'UI source domain for single-patch regression test',
      domainType: 'Aggregate',
      fullyQualifiedName: `ui_source_domain_${shortId}`,
    });
    const uiDataProduct = new DataProduct(
      [uiSourceDomain],
      `dp_ui_patch_${shortId}`
    );

    const { apiContext } = await getApiContext(page);
    await uiSourceDomain.create(apiContext);
    await uiDomain.create(apiContext);
    await uiDataProduct.create(apiContext);

    try {
      await redirectToHomePage(page);
      await sidebarClick(page, SidebarItem.DATA_PRODUCT);
      await selectDataProduct(page, uiDataProduct.data);
      ...
      page.on('request', (req) => {
        if (
          ...
          req
            .url()
            .includes(`/api/v1/dataProducts/${uiDataProduct.responseData.id}`)
        ) {
      ...
      const searchDomain = page.waitForResponse(
        `/api/v1/search/query?q=*${encodeURIComponent(uiDomain.data.name)}*`
      );
      await page
        .getByTestId('selectable-list')
        .getByTestId('searchbar')
        .fill(uiDomain.data.name);
      await searchDomain;
      await page
        .getByRole('listitem', { name: uiDomain.data.displayName })
        .click();
      ...
      const finishedPatch = await patchResponse;

      expect(finishedPatch.ok()).toBeTruthy();
      expect(patchRequests).toHaveLength(1);

      const body = JSON.parse(patchRequests[0]);
      const domainOps = body.filter(
        (op: { path: string }) => op.path === '/domains'
      );

      expect(domainOps).toHaveLength(1);

      const forbiddenPaths = body.filter((op: { path: string }) =>
        ['/impersonatedBy', '/changeDescription', '/version'].includes(op.path)
      );

      expect(forbiddenPaths).toHaveLength(0);

      for (const entry of domainOps[0].value) {
      ...
      try {
        await uiDataProduct.delete(apiContext);
      } catch {
        // Ignore
      }
      try {
        await uiDomain.delete(apiContext);
      } catch {
        // Ignore
      }
      try {
        await uiSourceDomain.delete(apiContext);
      } catch {
        // Ignore
      }
      await afterAction();
