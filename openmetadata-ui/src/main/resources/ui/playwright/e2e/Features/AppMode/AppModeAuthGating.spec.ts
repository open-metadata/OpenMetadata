...
/**
 * Ported verbatim (import paths only) from `collate-ui`'s
 * `AppMode/AppModeAuthGating.spec.ts` — the one spec in this suite with no
 * Collate/AI-shell dependency at all. It never renders the switcher or
 * touches `useAppRoutesRegistry`; both the `/preferences/appMode` resource
 * and the `appConfiguration` system setting are core OM (see
 * `generated/api/teams/preferences/appModePreference.ts` and
 * `generated/api/configuration/appConfiguration.ts`), so this is pure
 * API-level authz coverage for `UserResource#authorizeSelfOrAdmin` and
 * `SystemResource#createOrUpdateSetting`'s unconditional
 * `authorizer.authorizeAdmin`. No UI interaction beyond the one-time login
 * each fixture user does to mint its own `APIRequestContext`.
 */
...
test.describe('AppMode — v2 authz gating', () => {
  ...
  test('Admin can PUT /system/settings with configType appConfiguration', async ({
    ...
    try {
      const response = await adminApiContext.put('/api/v1/system/settings', {
        data: {
          config_type: 'appConfiguration',
          config_value: { defaultAppMode: 'ai' },
        },
