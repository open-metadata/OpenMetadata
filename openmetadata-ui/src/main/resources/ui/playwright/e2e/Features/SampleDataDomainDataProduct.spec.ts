...
test.describe('Sample Data Domain and Data Product Validation', () => {
  ...
  test(
    'Verify TestDomain exists from sample data ingestion',
    { tag: '@quarantine' },
    async ({ page }) => {
  ...
  test(
    'Verify TestDataProduct exists under TestDomain',
    { tag: '@quarantine' },
    async ({ page }) => {
      await sidebarClick(page, SidebarItem.DOMAIN);
      await waitForAllLoadersToDisappear(page);
  ...
  test(
    'Verify TestDataProduct shows correct details and domain association',
    { tag: '@quarantine' },
    async ({ page }) => {
      await sidebarClick(page, SidebarItem.DOMAIN);
      await waitForAllLoadersToDisappear(page);
