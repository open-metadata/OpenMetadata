...
test.describe('Column Level Data Quality Test Cases', { tag: DOMAIN_TAGS.OBSERVABILITY }, () => {
  ...
  test('Column Values To Be Between', async ({ page }) => {
    ...
    await test.step('Create', async () => {
      const testDefinitionResponse = page.waitForResponse(
        '/api/v1/dataQuality/testDefinitions?limit=*&entityType=COLUMN&testPlatform=OpenMetadata&supportedDataType=INT'
      );
      await page.click('[id="root\\/column"]');
      await page.waitForSelector(`[data-id="column"]`, { state: 'visible' });
  ...
  test('Column Values To Not Match Regex', async ({ page }) => {
    ...
    await test.step('Create', async () => {
      ...
      await page.fill('#testCaseFormV1_params_forbiddenRegex', testCase.regex);
    ...
    await test.step('Edit', async () => {
      ...
      await page.locator('#tableTestForm_params_forbiddenRegex').fill('[^0-9]');
