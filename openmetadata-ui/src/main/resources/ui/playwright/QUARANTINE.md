...
|---|---|---|
| `e2e/Pages/DataContracts.spec.ts` | Create Data Contract and validate for Table | A BE bug, not a test flake: the contract's quality/test-suite run finished without writing a result, so `qualityValidation` never populated and the aggregate `contractExecutionStatus` hung on `Running`, timing out the poll. The BE now reaps contract validation to a terminal state when the quality run fails to trigger or returns no result, so `waitForDataContractExecution` reaches a terminal status. Tag removed. |
