...
/**
 * Builds the custom properties the IntakeForm specs need on `dataProduct`,
 * `domain` and `glossaryTerm`.
 *
 * They live here, alongside the other fixtures, rather than being created by the
 * specs themselves: `PUT /api/v1/metadata/types/{id}` is read-modify-write on a
 * single global row per entity type, so a spec creating properties while another
 * worker writes the same row silently loses one side's property. Creating them
 * in entity-data.setup.ts keeps every write sequential and pre-test.
 *
 * The configs deliberately differ from `getCustomPropertyCreationData` — the
 * intake-form tests assert on ISO date formats, glossaryTerm/user scoped
 * references and their own enum option sets — which is exactly why they need
 * dedicated fixtures instead of reusing the shared ones.
 */
export const getIntakeFormCustomPropertyCreationData = (
  types: CPASTestData['types'],
  entityType: string
): Record<string, CustomPropertyDetails> => {
  const namePrefix = `cp-intake-${uuid()}-`;
  const typeIdMapping = types.reduce((acc, type) => {
    acc[type.name] = type.id;
  ...
  const build = (
    ...
    config?: NonNullable<
      CustomPropertyDetails['customPropertyConfig']
    >['config']
  ): CustomPropertyDetails => ({
    name: `${namePrefix}${key}`,
    description: `IntakeForm ${key} custom property for ${entityType}`,
    propertyType: {
      name: propertyTypeName,
      type: 'type',
      id: typeIdMapping[propertyTypeName],
    },
    ...(config === undefined ? {} : { customPropertyConfig: { config } }),
  });

  // Three distinct string properties per entity type: the form-designer tests
  // build multi-field intake forms and need to tell the fields apart.
  const designerStrings = {
    designerSteward: build('designerSteward', 'string'),
    designerAudience: build('designerAudience', 'string'),
    designerSource: build('designerSource', 'string'),
  };

  const shared = {
    ...designerStrings,
    string: build('string', 'string'),
    hyperlink: build('hyperlink', 'hyperlink-cp'),
    markdown: build('markdown', 'markdown'),
    sqlQuery: build('sqlQuery', 'sqlQuery'),
    table: build('table', 'table-cp', { columns: ['Name', 'Value'] }),
  };

  if (entityType === 'glossaryTerm') {
    return {
      ...shared,
      enum: build('enum', 'enum', {
  ...
  const withDomainExtras = {
    ...
    reference: build('reference', 'entityReference', ['glossaryTerm']),
  };

  if (entityType === 'domain') {
    return {
      ...withDomainExtras,
      // Consumed by IntakeFormCustomPropertyFields.spec.ts, which asserts on
      // these specific column names and on a required timeInterval field.
      fieldsTable: build('fieldsTable', 'table-cp', {
        columns: ['id', 'value'],
      }),
      timeInterval: build('timeInterval', 'timeInterval'),
    };
  }

  return {
    ...withDomainExtras,
    date: build('date', 'date-cp', 'yyyy-MM-dd'),
    dateTime: build('dateTime', 'dateTime-cp', 'yyyy-MM-dd HH:mm:ss'),
    time: build('time', 'time-cp', 'HH:mm:ss'),
    duration: build('duration', 'duration'),
    email: build('email', 'email'),
    enumMulti: build('enumMulti', 'enum', {
      values: ['Tag1', 'Tag2', 'Tag3'],
      multiSelect: true,
    }),
    integer: build('integer', 'integer'),
    number: build('number', 'number'),
    referenceList: build('referenceList', 'entityReferenceList', [
      'glossaryTerm',
    ]),
    stewardRef: build('stewardRef', 'entityReference', ['user']),
    timeInterval: build('timeInterval', 'timeInterval'),
