...
// Cross-worker mutex for tests that mutate the tenant-wide
// `appConfiguration.defaultAppMode` setting (or any other single-row
// system setting). Playwright fans different spec files across workers,
// and within-file `describe.serial` cannot stop a sibling file's worker
// from flipping a shared server-side setting in the small window between
// our PUT and a reader's fetch — the boot resolver locks a resolved mode
// into the session tuple, so a race there silently corrupts the assertion.
//
// Uses a `wx` (exclusive create) open on a well-known tmp file — atomic on
// POSIX and Windows — with stale-lock detection based on the writer PID.
// Consumers wrap their critical section in `withAppConfigLock(async () => { ... })`.
 ...
 * Releases the lock in `finally`, even when `critical` throws.
 */
