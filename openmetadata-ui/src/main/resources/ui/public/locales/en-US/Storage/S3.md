...
By default, the S3 connector will ingest only top-level containers (Buckets). If you want to extract any information from within and their data models, you can follow the <a href="https://docs.open-metadata.org/connectors/storage" target="_blank">docs</a>.
...
**Note:** CloudWatch permissions cannot be scoped to specific S3 buckets due to AWS limitations. These APIs only support `Resource: "*"`. However, you can restrict the permissions to only the AWS/S3 namespace using IAM conditions:
...
If you don't need bucket-level metrics (size and object count), you can disable CloudWatch metrics fetching entirely using the **Fetch CloudWatch Metrics** option. This allows you to use the connector without any CloudWatch permissions.

You can find further information on the Kafka connector in the <a href="https://docs.open-metadata.org/connectors/storage/s3" target="_blank">docs</a>.
...
The Console Endpoint URL is used to generate clickable links in the OpenMetadata UI that direct users to the appropriate web console for viewing buckets and objects.
...
$$section
### Fetch CloudWatch Metrics $(id="fetchCloudWatchMetrics")

When enabled (default), the connector will fetch bucket-level metrics from CloudWatch, including:
- **Total number of objects** in each bucket
- **Total size in bytes** of each bucket

This requires additional IAM permissions (`cloudwatch:GetMetricData` and `cloudwatch:ListMetrics`).

**Set to `false` if:**
- You want to minimize the required IAM permissions
- CloudWatch metrics are not available for your buckets
- You are using an S3-compatible storage that doesn't support CloudWatch

When disabled, the connector will still function normally but will report `0` for bucket size and object count metrics.
$$
