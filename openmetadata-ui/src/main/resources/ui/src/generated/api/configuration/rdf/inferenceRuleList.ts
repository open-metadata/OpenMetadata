 ...
 */
/**
 * Inference rules with durable materialization state.
 */
export interface InferenceRuleList {
    rules: InferenceRuleStatus[];
}

/**
 * Durable definition and materialization state for an RDF inference rule.
 */
export interface InferenceRuleStatus {
    /**
     * Whether source RDF changed after the last successful materialization.
     */
    dirty: boolean;
    /**
     * Named graph containing triples materialized by this rule.
     */
    graphUri: string;
    /**
     * Most recent materialization error, if any.
     */
    lastError?: string;
    /**
     * Timestamp of the most recent successful materialization.
     */
    lastMaterializedAt?: number;
    rule:                InferenceRule;
    /**
     * Whether the rule is part of the shipped starter pack.
     */
    systemRule: boolean;
    /**
     * Number of triples in the rule's materialized graph.
     */
    tripleCount: number;
}
