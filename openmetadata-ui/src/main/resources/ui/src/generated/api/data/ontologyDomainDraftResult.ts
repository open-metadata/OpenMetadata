 ...
 */
/**
 * A generated Draft payload that must be persisted through the normal change-set API.
 */
export interface OntologyDomainDraftResult {
    draft:       CreateOntologyChangeSetRequest;
    generatedAt: number;
    modelId:     string;
}

/**
 * Create or update an ontology authoring session.
 */
export interface CreateOntologyChangeSetRequest {
    description: string;
    /**
     * Immutable source evidence and run attribution for an automatically discovered draft.
     */
    discoveryContext?: OntologyDiscoveryContext;
    displayName:       string;
    /**
     * Fully qualified names of affected ontology models.
     */
    glossaries:  string[];
    name:        string;
    operations?: OntologyChangeOperation[];
