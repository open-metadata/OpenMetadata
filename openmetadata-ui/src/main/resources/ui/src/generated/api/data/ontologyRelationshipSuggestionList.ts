 ...
 */
/**
 * Typed relationship proposals returned by the configured AI provider.
 */
export interface OntologyRelationshipSuggestionList {
    generatedAt: number;
    modelId:     string;
    suggestions: OntologyRelationshipSuggestion[];
}

/**
 * A validated relationship proposal represented as a Draft-ready operation.
 */
export interface OntologyRelationshipSuggestion {
    confidence: number;
    id:         string;
...
/**
 * A governed OWL axiom whose relational representation remains authoritative when RDF is
 * disabled.
 */
