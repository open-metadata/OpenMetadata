...
export interface AuthenticationConfiguration {
    /**
     * Callback (redirect_uri) values registered with the identity provider for the other
     * hostnames this deployment answers on — a load-balanced second site, or a
     * disaster-recovery host. At login the entry whose scheme, host and port match the incoming
     * request is used as the redirect_uri; entries whose path differs from 'callbackUrl' are
     * ignored. When nothing matches, 'callbackUrl' is used, so a single-hostname deployment is
     * unaffected. Every entry must also be registered with the identity provider.
     */
