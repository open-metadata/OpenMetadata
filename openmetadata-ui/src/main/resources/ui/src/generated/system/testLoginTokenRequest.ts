 ...
 */
/**
 * Admin-only request to validate a browser-obtained OIDC id_token against a candidate
 * (unsaved) security configuration. Used to confirm a real login would resolve the expected
 * identity before saving the configuration.
 */
export interface TestLoginTokenRequest {
    /**
     * OIDC id_token obtained by the admin's browser from the identity provider during the test
     * login.
     */
    idToken: string;
    /**
