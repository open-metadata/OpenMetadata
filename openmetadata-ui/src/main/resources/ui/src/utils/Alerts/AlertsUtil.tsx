              ...
              <Row align="middle" gutter={[8, 8]}>
                ...
                <Form.Item
                  ...
                  shouldUpdate={(prevValues, currentValues) => {
                    const prevType =
                      prevValues?.destinations?.[fieldName]?.config?.authType
                        ?.type;
                    const currentType =
                      currentValues?.destinations?.[fieldName]?.config?.authType
                        ?.type;
                        ...
                        <>
                          ...
                          <Col span={24}>
                            <Form.Item
                              ...
                              labelCol={{ span: 24 }}
                              name={[fieldName, 'config', 'authType', 'scope']}>
                              <Input
...
const getEmailDestinationConfigField = (
  type: SubscriptionType | SubscriptionCategory,
  fieldName: number
) => {
  return (
...
const getTeamOrUserDestinationConfigField = (
  type: SubscriptionType | SubscriptionCategory,
  fieldName: number
) => {
  return (
...
const getHiddenSwitchDestinationConfigField = (
  type: SubscriptionType | SubscriptionCategory,
  fieldName: number
) => {
  return (
...
type DestinationConfigFieldRenderer = (
  type: SubscriptionType | SubscriptionCategory,
  fieldName: number
) => JSX.Element | null;

const DESTINATION_CONFIG_FIELD_TYPE_GROUPS: [
  Array<SubscriptionType | SubscriptionCategory>,
  DestinationConfigFieldRenderer
][] = [
  [
    [
    ...
    getWebhookLikeDestinationConfigField,
  ],
  [[SubscriptionType.Email], getEmailDestinationConfigField],
  [
    [SubscriptionCategory.Teams, SubscriptionCategory.Users],
    getTeamOrUserDestinationConfigField,
  ],
  [
    [
    ...
    getHiddenSwitchDestinationConfigField,
  ],
];
...
export const getDestinationConfigField = (
  ...
  const group = DESTINATION_CONFIG_FIELD_TYPE_GROUPS.find(([types]) =>
    types.includes(type)
  );

  return group ? group[1](type, fieldName) : null;
};
...
export const getFieldByArgumentType = (
  ...
  const fieldFactories: Record<string, () => JSX.Element> = {
    fqnList: () => (
      <FQNListSelect
        api={getEntityByFQN}
        className="w-full"
    ...
    // For updateByUserList, we need to show bot users as well
    updateByUserList: () => (
      <AsyncSelect
    ...
    // For userList, which is an argument for `conversation` filters we need to show only non-bot users
    userList: () => (
      <AsyncSelect
        api={getUserOptions}
        className="w-full"
  ...
  const field = fieldFactories[argument]?.() ?? <></>;
