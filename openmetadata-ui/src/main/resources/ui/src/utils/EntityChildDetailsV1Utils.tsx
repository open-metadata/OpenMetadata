...
const APIEndpointSchemaV1: React.FC<{
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'dataType',
      width: 150,
      render: (dataType: string, record: Record<string, string>) => (
        <Typography.Text>
...
const DatabaseSchemasV1: React.FC<{
  ...
  const databaseSchemas = (entityInfo.databaseSchemas || []) as Array<Record<string, unknown>>;
    ...
    <div className="schema-field-cards-container">
      <Row>
        {databaseSchemas.map((schema) => {
          return (
            <Col key={schema.id as string} span={24}>
              <FieldCard
                dataType={(schema.type as string) || 'Database Schema'}
                description={(schema.description as string) || ''}
                fieldName={(schema.displayName as string) || (schema.name as string)}
                tags={(schema.tags as TagLabel[]) || []}
              />
            </Col>
          );
        })}
