...
class SwTokenStorage implements StateStore {
  ...
  async get(key: string): Promise<string | null> {
    return await sendMessageToServiceWorker({ type: 'get', key });
  }

  async remove(key: string): Promise<string | null> {
    return await sendMessageToServiceWorker({ type: 'remove', key });
  }

  async getAllKeys(): Promise<string[]> {
    return await sendMessageToServiceWorker({ type: 'getAllKeys' });
  }
}
