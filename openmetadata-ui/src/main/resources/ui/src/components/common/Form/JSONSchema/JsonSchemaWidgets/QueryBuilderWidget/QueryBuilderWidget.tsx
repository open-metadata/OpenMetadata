...
import {
  ...
  getDefaultEntityTypeFilterQuery,
...
const QueryBuilderWidget: FC<WidgetProps> = ({
  ...
  const handleChange = (nTree: ImmutableTree, nConfig: Config) => {
    ...
    if (outputType === SearchOutputType.ElasticSearch) {
      ...
      const qFilter: { query: any } = {
        query:
          data ||
          (entityType !== EntityType.ALL
            ? getDefaultEntityTypeFilterQuery(entityType)
            : ''),
      };

      if (data && entityType !== EntityType.ALL) {
        const boolQuery = (qFilter.query as QueryFieldInterface)
          ?.bool as EsBoolQuery;

        if (Array.isArray(boolQuery?.must)) {
          boolQuery.must.push(getDefaultEntityTypeFilterQuery(entityType));
        }
      }
  ...
  const loadDefaultValueInTree = useCallback(() => {
    ...
    } else if (outputType === SearchOutputType.ElasticSearch) {
      onChange(
        JSON.stringify({
          query: getDefaultEntityTypeFilterQuery(entityType),
        })
      );
    }
    setInitDone(true);
  }, [config, value, outputType, entityType]);
  ...
  useEffect(() => {
    if (isSearchIndexUpdatedInContext) {
      loadDefaultValueInTree();
    }
  }, [isSearchIndexUpdatedInContext]);
