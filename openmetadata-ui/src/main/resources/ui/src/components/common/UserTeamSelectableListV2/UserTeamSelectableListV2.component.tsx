...
import '../UserTeamSelectableList/user-team-selectable-list.less';
import { UserTeamSelectableListV2Props } from './UserTeamSelectableListV2.interface';
...
export const UserTeamSelectableListV2 = ({
  hasPermission,
  owner,
  onUpdate = noop,
  children,
  ...
  const { selectedUsers, selectedTeams } = useMemo(() => {
    return {
      selectedUsers:
        owner?.filter((item) => item.type === EntityType.USER) ?? [],
      selectedTeams:
        owner?.filter((item) => item.type === EntityType.TEAM) ?? [],
    };
  }, [owner]);
