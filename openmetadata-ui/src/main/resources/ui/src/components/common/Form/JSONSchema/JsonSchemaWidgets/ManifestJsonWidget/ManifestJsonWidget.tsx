...
const EXPECTED_STRING_ARRAY = 'expected-string-array';
const EXPECTED_OBJECT_ARRAY = 'expected-object-array';
const PARTITION_COLUMN_REQUIRED = 'partition-column-required';
const ENTRY_REQUIRED_FIELD = 'entry-required-field';
  ...
  switch (expected) {
    ...
    case 'string[]':
      if (!Array.isArray(value)) {
        return { kind: EXPECTED_STRING_ARRAY };
      }

      return value.every((item) => typeof item === 'string')
        ? null
        : { kind: EXPECTED_STRING_ARRAY };
    case 'object[]':
      if (!Array.isArray(value)) {
        return { kind: EXPECTED_OBJECT_ARRAY };
      }
...
const validatePartitionColumns = (
  ...
  for (let colIndex = 0; colIndex < columns.length; colIndex += 1) {
    ...
    if (typeof columnRecord.name !== 'string' || !columnRecord.name.trim()) {
      return {
        code: PARTITION_COLUMN_REQUIRED,
        entryIndex: entryIndex + 1,
        colIndex,
        field: 'name',
      };
...
const formatTypeMismatch = (mismatch: TypeMismatch, t: TFunction): string => {
  switch (mismatch.kind) {
    ...
    case EXPECTED_STRING_ARRAY:
      return t('message.expected-an-array-of-strings');
    case EXPECTED_OBJECT_ARRAY:
...
export const formatValidationError = (
  ...
  switch (error.code) {
    ...
    case PARTITION_COLUMN_REQUIRED:
