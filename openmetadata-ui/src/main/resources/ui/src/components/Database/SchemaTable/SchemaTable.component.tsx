...
const SchemaTable = () => {
  ...
  const searchTableColumns = useCallback(
    async (searchQuery: string, page = 1,  columnSortBy?: 'name' | 'ordinalPosition',
  ...
  const fetchTableColumns = useCallback(
    async (page = 1, columnSortBy?: 'name' | 'ordinalPosition',
      columnSortOrder?: 'asc' | 'desc') => {
      if (!tableFqn) {
        return;
      }
  ...
  }, [tableFqn, pageSize, currentPage, searchText, fetchTableColumns, sortBy, sortOrder]);
