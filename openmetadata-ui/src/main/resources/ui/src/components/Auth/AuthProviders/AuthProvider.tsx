...
export const AuthProvider = ({
  ...
  const handledVerifiedUser = () => {
    if (!applicationRoutesClass.isProtectedRoute(location.pathname)) {
      ...
      //
      // At post-login redirect time `hydrateAndResolveAppMode` may not
      // have finished its persona-doc fetch yet, so the useAppMode store
      // alone only reflects the sessionStorage tuple (empty on a fresh
      // login). `resolveInitialAppMode` consults the same
      // synchronously-available signals as the boot resolver — session
      // tuple → fresh cross-tab hint → user's stored preference — so a
      // user whose "remember" checkbox is on AI or whose sibling tab is
      // in AI lands on `/` from the start instead of being bounced
      // through `/my-data` and then flipped to AI a tick later. Persona
      // (async) stays with `hydrateAndResolveAppMode`.
