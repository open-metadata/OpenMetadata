...
interface IconDrawContext {
  ...
  circ: (x: number, y: number, r: number, fill?: boolean) => void;
  ell: (
    x: number,
    y: number,
    rx: number,
...
type IconDrawer = (d: IconDrawContext) => void;

const dashboardOrPipeline: IconDrawer = ({ L, R, B, C, line, rr }) => {
  [
    [L + 2, 28],
    [C - 7, 46],
    [R - 18, 36],
...
const ICON_DRAWERS: Partial<Record<NodeType, IconDrawer>> = {
  table: ({ L, R, T, B, C, line, rr }) => {
    rr(L, T, R - L, B - T, 8);
    line(L, T + 20, R, T + 20);
    line(C, T + 20, C, B);
    line(L, T + 47, R, T + 47);
  },
  column: ({ T, B, C, line, rr }) => {
    rr(C - 16, T, 32, B - T, 7);
    line(C - 16, T + 20, C + 16, T + 20);
    line(C - 16, T + 37, C + 16, T + 37);
    line(C - 16, T + 54, C + 16, T + 54);
  },
  database: ({ T, B, C, line, ell }) => {
    const rx = 30;
    const ry = 11;
    const ty = T + 10;
    const by = B - 10;
  ...
  dashboard: dashboardOrPipeline,
  pipeline: dashboardOrPipeline,
  user: ({ ctx, B, C, T, circ }) => {
    circ(C, T + 15, 13);
    ctx.beginPath();
  ...
  product: ({ L, R, T, B, C, line, rr }) => {
    rr(L, T + 6, R - L, B - T - 12, 6);
    line(L, T + 27, R, T + 27);
    line(C, T + 6, C, T + 27);
  },
  tag: ({ ctx, L, R, T, B, C, circ }) => {
    ctx.beginPath();
    ctx.moveTo(L + 22, T);
    ctx.lineTo(R, T);
    ctx.lineTo(R, B);
  ...
  searchIndex: ({ R, B, C, line, circ }) => {
    circ(C - 6, C - 8, 20);
    line(C + 8, C + 6, R - 6, B - 2);
  },
  storedProcedure: ({ C, line, circ }) => {
    circ(C, C, 22);
    for (let i = 0; i < 8; i++) {
  ...
  dataContract: ({ ctx, L, R, T, B, C, line, rr, circ }) => {
...
const drawIcon = (type: NodeType, ctx: CanvasRenderingContext2D): void => {
  ...
  if (drawer) {
    drawer({ ctx, L, R, T, B, C, line, rr, circ, ell });
