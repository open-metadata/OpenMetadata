...
const ExploreQuickFilters: FC<ExploreQuickFiltersProps> = ({
  ...
  // Every dropdown shares the options slot because only one can be open. Keep
  // an ownership token so a slower request from the previous dropdown cannot
  // overwrite the options (or loading state) of the currently active one.
  ...
  const fetchDefaultOptions = async (
    ...
    if (staticOptions) {
      if (latestOptionsRequest.current === requestId) {
  ...
  const getInitialOptions = async (
    ...
    const requestId = ++latestOptionsRequest.current;
    const staticOptions = getStaticOptions(key);
    if (staticOptions) {
