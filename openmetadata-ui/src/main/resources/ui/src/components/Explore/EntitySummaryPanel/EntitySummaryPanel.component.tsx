...
const DETAILS_ENTITYTYPE = 'details.entityType';
...
export default function EntitySummaryPanel({
  ...
  const entityType = useMemo(
    () => get(entityDetails, DETAILS_ENTITYTYPE) as EntityType | undefined,
    [entityDetails]
  );
  ...
  const fetchResourcePermission = async (id: string) => {
    try {
      ...
      let type = get(entityDetails, DETAILS_ENTITYTYPE) as
        | ResourceEntity
        | undefined;
  ...
  const summaryComponentV1 = useMemo(() => {
    ...
    const type = (get(entityDetails, DETAILS_ENTITYTYPE) ??
  ...
  return (
    <div
      className={classNames('entity-summary-panel-container', {
        ...
        'glossary-term-assets-tab': panelPath === GLOSSARY_TERM_ASSETS_TAB,
        'ontology-explorer': panelPath === ONTOLOGY_EXPLORER,
      })}
