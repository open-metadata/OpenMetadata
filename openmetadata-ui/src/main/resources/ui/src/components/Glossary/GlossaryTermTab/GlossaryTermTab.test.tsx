...
describe('Test GlossaryTermTab component', () => {
  ...
  it('should contain expand-terms button when glossary data is not empty', () => {
    const { container } = render(<GlossaryTermTab {...mockProps2} />, {
      wrapper: MemoryRouter,
    });

    expect(getByTestId(container, 'expand-terms')).toBeInTheDocument();
  });

  it('should contain all necessary fields value in table when glossary data is not empty', () => {
    const { container } = render(<GlossaryTermTab {...mockProps2} />, {
      wrapper: MemoryRouter,
    });
