...
import { LineagePlatformView } from '../../../context/LineageV1/LineageProviderV1.interface';
...
import './LineageLayersV1.less';

interface LineageLayersV1Props {
...
const LayerButton = memo<LayerButtonProps>(
...
const LineageLayersV1 = ({ entityType, entity }: LineageLayersV1Props) => {
  const { activeLayer = [], isEditMode } = useLineageUI();
  const { platformView, isPlatformLineage } = useLineageView();
  const { onUpdateLayerView, onPlatformViewChange } = useLineageActions();
...
export default memo(LineageLayersV1);
