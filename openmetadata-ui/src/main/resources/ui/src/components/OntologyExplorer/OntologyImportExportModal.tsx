...
interface OntologyDerivedState {
  isRdfFormat: boolean;
...
// Extracted so the boolean/ternary short-circuits live in their own
// complexity scope instead of OntologyImportExportModal's render body.
const computeOntologyDerivedState = (
  format: ExportFormatKey,
  preview: string,
  isPreviewLoading: boolean,
  importValidation: OntologyImportResult | undefined,
  importTarget: Glossary | undefined
): OntologyDerivedState => {
  const isRdfFormat = format !== 'csv';
  const isPreviewReady = Boolean(isRdfFormat && preview && !isPreviewLoading);
...
const OntologyImportExportModal = ({
  ...
  useEffect(() => {
    ...
    exportGlossaryAsOntology({
      format: format as OntologyExportFormat,
  ...
  const renderExportPreview = () => {
    ...
    return isRdfFormat && preview ? highlightRdf(preview) : '—';
  };

  const renderExportPreviewStatus = () => {
    if (shaclResult) {
      const shaclStatusClassName = shaclResult.conforms
  ...
  const renderExportPanel = () => (
    <div className="tw:flex tw:min-h-0 tw:flex-1 tw:gap-5 tw:overflow-y-auto tw:p-5">
      ...
      <div className="tw:w-[300px] tw:shrink-0">
        ...
        {renderExportPreviewStatus()}
      </div>
    </div>
  );

  const renderImportTargetSelector = () => {
    if (activeGlossary) {
      return null;
    }
  ...
  const renderImportFileDetails = () => {
    if (!importFileName) {
      return t('message.ontology-accepted-extensions');
    }
  ...
  const renderImportSummary = () => {
    ...
    const showImportNote = hasReviewers || importSkippedCount > 0;
  ...
  const renderExportFooterActions = () => (
    <>
      <Button
        color="primary"
  ...
  const renderImportFooterActions = () => (
    <>
      {hasImportedTerms ? (
        <Button
          color="primary"
  ...
  return (
    <ModalOverlay isDismissable isOpen onOpenChange={handleModalOpenChange}>
      <Modal>
        <Dialog
          showCloseButton
          ...
          <div
            ...
            {activeTab === 'export' ? renderExportPanel() : renderImportPanel()}

            <div className="tw:flex tw:items-center tw:gap-2.5 tw:border-t tw:border-secondary tw:px-5 tw:py-3.5">
              {activeTab === 'export'
                ? renderExportFooterActions()
                : renderImportFooterActions()}
            </div>
          </div>
        </Dialog>
      </Modal>
