...
interface OntologyQuerySidebarProps {
  readonly t: (key: string) => string;
  readonly querySuggestions: OntologyQuerySuggestion[];
  readonly activeQueryId?: string;
  readonly onSuggestionRun: (suggestion: OntologyQuerySuggestion) => void;
  readonly onNewQuery: () => void;
  readonly isLoading: boolean;
  readonly queryTemplates: SavedSparqlQuery[];
  readonly isAdminUser?: boolean;
  readonly onTemplateRun: (queryTemplate: SavedSparqlQuery) => void;
  readonly onTemplateEdit: (queryTemplate: SavedSparqlQuery) => void;
  readonly onDeleteTemplate: (id: string) => void;
  readonly savedQueries: SavedSparqlQuery[];
  readonly onSavedQueryRun: (savedQuery: SavedSparqlQuery) => void;
  readonly onDeleteSavedQuery: (id: string) => void;
}

// Rendered separately so its per-section empty/loading ternaries don't add to
// the parent console's cyclomatic complexity.
const OntologyQuerySidebar = ({
  t,
  querySuggestions,
  activeQueryId,
  onSuggestionRun,
  ...
  onSavedQueryRun,
  onDeleteSavedQuery,
}: OntologyQuerySidebarProps) => {
  let installationQueriesContent;
  if (isLoading) {
    installationQueriesContent = (
      <p className="tw:m-0 tw:px-1 tw:font-body tw:text-xs tw:text-quaternary">
        {t('label.loading')}
      </p>
    );
  } else if (queryTemplates.length === 0) {
    installationQueriesContent = (
      <p className="tw:m-0 tw:px-1 tw:font-body tw:text-xs tw:text-quaternary">
    ...
    installationQueriesContent = (
      <div
        ...
        {queryTemplates.map((queryTemplate) => {
          const isActive = `template-${queryTemplate.id}` === activeQueryId;
            ...
            <div
              ...
              <Button
                ...
                className={classNames(
                  ...
                  isActive ? 'tw:text-brand-secondary' : 'tw:text-secondary'
                )}
                color="tertiary"
                data-testid={`ontology-query-template-${queryTemplate.id}`}
                title={queryTemplate.name}
                onClick={() => onTemplateRun(queryTemplate)}>
              ...
              {isAdminUser ? (
                <>
                  <ButtonUtility
                    ...
                    onClick={() => onTemplateEdit(queryTemplate)}
                  />
                  <ButtonUtility
                    className={classNames(
                    ...
                    onClick={() => void onDeleteTemplate(queryTemplate.id)}
                  />
                </>
              ) : null}
            </div>
        ...
        {querySuggestions.map((suggestion) => {
          ...
          return (
            <Button
              ...
              onClick={() => onSuggestionRun(suggestion)}>
              <span className="tw:block tw:truncate">{suggestion.label}</span>
            </Button>
          );
        })}
            ...
            return (
              <div
                ...
                <Button
                  ...
                  onClick={() => onSavedQueryRun(savedQuery)}>
                  <span className="tw:block tw:truncate">
                    {savedQuery.name}
                  </span>
                </Button>
...
interface SaveQueryModalProps {
  ...
  readonly onCommitSave: () => void;
}

// Rendered separately so the title/label ternaries don't add to the parent
// console's cyclomatic complexity.
const SaveQueryModal = ({
  ...
  let title = t('label.save-query');
  if (saveTarget === 'template') {
    title = isUpdatingTemplate
      ? t('label.update-sample-query')
      : t('label.save-as-sample-query');
  }
  const commitLabel = isUpdatingTemplate ? t('label.update') : t('label.save');
    ...
    <>
      <div
        ...
        <OntologyQuerySidebar
          ...
          onSavedQueryRun={handleSavedQueryRun}
          onSuggestionRun={handleSuggestionRun}
