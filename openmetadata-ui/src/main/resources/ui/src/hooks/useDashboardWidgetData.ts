...
import { MAX_DASHBOARD_WIDGET_CACHE_ENTRIES } from '../constants/DashboardCache.constants';
...
const dashboardWidgetCache = makeLruCache<unknown>(
  MAX_DASHBOARD_WIDGET_CACHE_ENTRIES
);
const dashboardWidgetRequests = new Map<string, Promise<unknown>>();
let dashboardWidgetCacheEpoch = 0;

// Landing-page widgets call this hook so cached data can render immediately
// while a background refresh coalesces duplicate requests for the same key.
