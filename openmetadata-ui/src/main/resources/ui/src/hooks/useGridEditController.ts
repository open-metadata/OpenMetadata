...
const ARIA_COLINDEX = 'aria-colindex';
  ...
  */
  ...
  const getCellIndices = useCallback(
    (target: EventTarget | null): { row: number; col: number } | null => {
      ...
      const rowAttr = target.parentElement?.getAttribute(ARIA_ROWINDEX);
      const colAttr = target.getAttribute(ARIA_COLINDEX);
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        if (!isInInput && !isInCustomEditor) {
          ...
          if (isInColumnHeader) {
            ...
            const rowAttr = target.getAttribute(ARIA_ROWINDEX);
            if (rowAttr) {
