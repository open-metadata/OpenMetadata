...
// Preference keys that are synced to the backend (`User.preferences`) in
// addition to being persisted locally. Everything else in UserPreferences
// stays purely on-device. Keep this to a single entry for now — see the
// task brief for the rollout plan of additional keys.
...
async function flushPendingPatch(
  ...
  const ops: Operation[] = Array.from(attempted.entries()).map(
    ([key, value]) => {
      ...
      return { op, path: `/preferences/${key}`, value } as Operation;
    }
  );

  try {
    const updated = await updateUserDetail(userId, ops);
 ...
 * `currentUser` is set. Reconciles the freshly-fetched server preferences
 * with whatever is already in the local persisted store:
 ...
 */
export function hydrateBackendSyncedPreferences(user: {
  id: string;
  name: string;
