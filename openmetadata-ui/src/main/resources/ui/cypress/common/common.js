...
export const elasticSearchReIndexData = () => {
  cy.get('[data-testid="appbar-item-settings"]').should('be.visible').click();

  interceptURL(
    'GET',
    '/api/v1/search/reindex/stream/status',
    'streamJobStatus'
  );

  // Click on elasticSearch Menu Item
  cy.get('[data-menu-id*="elasticsearch"]')
    .should('exist')
    .scrollIntoView()
    .should('be.visible')
    .click();

  verifyResponseStatusCode('@streamJobStatus', 200);

  cy.get('[data-testid="elastic-search-re-index-all"]')
    .should('exist')
    .scrollIntoView()
    .should('be.visible')
    .click();

  interceptURL('POST', '/api/v1/search/reindex', 'reIndexData');
  cy.get('.ant-modal-footer > .ant-btn-primary ')
    .should('exist')
    .scrollIntoView()
    .should('be.visible')
  ...
  verifyResponseStatusCode('@reIndexData', 201);

  cy.wait(5000);

  interceptURL('GET', '/api/v1/search/reindex/latest', 'latestBatchJobData');

  cy.get(':nth-child(1) > [data-testid="elastic-search-re-fetch-data"]')
    .should('exist')
    .scrollIntoView()
    .should('be.visible')
    .click();

  verifyResponseStatusCode('@latestBatchJobData', 200);

  cy.get(
    '[style="flex-wrap: wrap; gap: 0px;"] > :nth-child(3) > .flex > .ant-space > :nth-child(2) > span'
  )
    .scrollIntoView()
    .should('be.visible')
    .contains('Completed');
