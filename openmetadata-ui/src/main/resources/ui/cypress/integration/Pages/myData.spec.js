...
describe('MyData page should work', () => {
  ...
  it('MyData Page should render properly with all required comonents', () => {
    ...
    Object.values(MYDATA_SUMMERY_OPTIONS).forEach((value) => {
      cy.get(
  ...
  it('onClick of table should redirect to tables tab in explore page', () => {
    visitEntityTab(MYDATA_SUMMERY_OPTIONS.tables);
  });

  it('onClick of topics should redirect to topics tab in explore page', () => {
    visitEntityTab(MYDATA_SUMMERY_OPTIONS.topics);
  });

  it('onClick of dashboards should redirect to dashboards tab in explore page', () => {
    visitEntityTab(MYDATA_SUMMERY_OPTIONS.dashboards);
  });

  it('onClick of pipelines should redirect to pipelines tab in explore page', () => {
    visitEntityTab(MYDATA_SUMMERY_OPTIONS.pipelines);
  });

  it('Recently views should work properly', () => {
    // checking for table entity
    checkRecentlyViewElement(0, MYDATA_SUMMERY_OPTIONS.tables);

    // checking for topic entity
    checkRecentlyViewElement(0, MYDATA_SUMMERY_OPTIONS.topics);

    // checking for dashboard entity
    checkRecentlyViewElement(0, MYDATA_SUMMERY_OPTIONS.dashboards);

    // checking for pipeline entity
    checkRecentlyViewElement(0, MYDATA_SUMMERY_OPTIONS.pipelines);
  });
});
