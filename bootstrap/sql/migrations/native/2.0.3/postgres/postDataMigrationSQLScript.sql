...
--
-- Carried on the 2.0 train as 2.0.3 rather than as the 1.13.6 slot main uses. MigrationWorkflow
-- selects versions by exact set membership against SERVER_CHANGE_LOG and drops an already-recorded
-- version unless it is the recorded maximum, so on a 2.0.x deployment a 1.13.6 directory is either
-- already recorded (upgraded from the 1.13 train) or no longer the maximum, and in both cases these
-- statements would never be offered. 2.0.3 is unrecorded everywhere on this train, so it is applied
-- exactly once on the next upgrade.
--
...
SET json = jsonb_set(json, '{chartDetails,metrics,0,filter}', to_jsonb('{"query":{"bool":{"must":[{"exists":{"field":"entityType"}}],"must_not":[{"term":{"entityType.keyword":"tag"}},{"term":{"entityType.keyword":"glossaryTerm"}},{"term":{"entityType.keyword":"dataProduct"}}]}}}'::text))
WHERE name IN (
    'total_data_assets',
  ...
  AND jsonb_typeof(json -> 'chartDetails' -> 'metrics') = 'array'
  AND jsonb_array_length(json -> 'chartDetails' -> 'metrics') = 1
  AND COALESCE(json -> 'chartDetails' -> 'metrics' -> 0 ->> 'filter', '') <> '{"query":{"bool":{"must":[{"exists":{"field":"entityType"}}],"must_not":[{"term":{"entityType.keyword":"tag"}},{"term":{"entityType.keyword":"glossaryTerm"}},{"term":{"entityType.keyword":"dataProduct"}}]}}}';

-- Drop the same governance types from the breakdown that the filter above drops from the card.
UPDATE di_chart_entity
SET json = jsonb_set(json, '{chartDetails,excludeGroups}', '["tag","glossaryTerm","dataProduct"]'::jsonb)
WHERE name IN (
    'total_data_assets',
    'total_data_assets_by_tier',
    'total_data_assets_summary_card',
  ...
  AND json -> 'chartDetails' ->> 'groupBy' = 'entityType.keyword'
  AND COALESCE(json -> 'chartDetails' -> 'excludeGroups', 'null'::jsonb) <> '["tag","glossaryTerm","dataProduct"]'::jsonb;
