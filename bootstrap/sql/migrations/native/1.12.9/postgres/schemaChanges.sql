...
-- Session-level (not LOCAL) because schemaChanges runs in autocommit mode —
-- each statement is its own transaction, so SET LOCAL would reset between
-- statements. RESET at the bottom of the file restores defaults before the
