...
-- MySQL has no reliable `ADD COLUMN IF NOT EXISTS`, so each column is guarded via
-- information_schema: these columns previously shipped under 2.1.0 on pre-release
-- branches, and a database that already applied them must not fail this migration.
SET @ddl = (
  SELECT IF(
    EXISTS (
      SELECT 1
