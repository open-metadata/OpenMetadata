...
TABLE_INITIALIZER_MAIN_CLASS=org.openmetadata.apis.util.TablesInitializer
...
if [ -d "${LIBS_DIR}" ]; then
  ...
  CLASSPATH=`mvn -pl openmetadata-apis -q exec:exec -Dexec.executable=echo -Dexec.args="%classpath"`
fi
