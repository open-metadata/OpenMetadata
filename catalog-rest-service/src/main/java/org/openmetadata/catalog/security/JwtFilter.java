...
public class JwtFilter implements ContainerRequestFilter {
  ...
  public void filter(ContainerRequestContext requestContext) {
    ...
    if (providerType.equals("saml") || providerType.equals("basic")) {
    ...
    if (uriInfo.getPath().contains("logout") && (providerType.equals("saml") || providerType.equals("basic"))) {
      // This ensures that on logout we are invalidation the token generated by the OMD
      Date logoutTime = Date.from(LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant());
      LogoutRequest logoutRequest =
          new LogoutRequest().withUsername(userName).withToken(tokenFromHeader).withLogoutTime(logoutTime);
      JwtTokenCacheManager.getInstance().markLogoutEventForToken(logoutRequest);
      return;
    }
