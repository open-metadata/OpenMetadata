...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    const iconElement = Icon && <Icon aria-hidden="true" className="tw:mr-2 tw:size-5 tw:shrink-0 tw:text-fg-quaternary tw:transition-inherit-all" />;

    const badgeElement =
        badge && (typeof badge === "string" || typeof badge === "number") ? (
            <Badge className="tw:ml-3" color="gray" type="pill-color" size="sm">
                {badge}
            </Badge>
        ) : (
            badge
    ...
    const externalIcon = isExternal && <Share04 className="tw:size-4 tw:stroke-[2.5px] tw:text-fg-quaternary" />;

    if (type === "collapsible") {
        return (
            <summary className={cx("tw:px-3 tw:py-2", styles.root, current && styles.rootSelected)} onClick={onClick}>
                {iconElement}
                ...
                <ChevronDown aria-hidden="true" className="tw:ml-3 tw:size-4 tw:shrink-0 tw:stroke-[2.5px] tw:text-fg-quaternary tw:in-open:-scale-y-100" />
            </summary>
        );
    }
    ...
    if (type === "collapsible-child") {
        return (
            <AriaLink
                ...
                className={cx("tw:py-2 tw:pr-3 tw:pl-10", styles.root, current && styles.rootSelected)}
                onClick={onClick}
    ...
    return (
        <AriaLink
            ...
            target={isExternal ? "_blank" : "_self"}
            rel="noopener noreferrer"
            className={cx("tw:px-3 tw:py-2", styles.root, current && styles.rootSelected)}
            onClick={onClick}
            aria-current={current ? "page" : undefined}
        >
            {iconElement}
