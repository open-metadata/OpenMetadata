...
export const NavAccountMenu = ({
    ...
    return (
        <AriaDialog
            {...dialogProps}
            ref={dialogRef}
            className={cx("tw:w-66 tw:rounded-xl tw:bg-secondary_alt tw:shadow-lg tw:ring tw:ring-secondary_alt tw:outline-hidden", className)}
        >
                    ...
                    <div className="tw:flex tw:flex-col tw:gap-0.5 tw:px-1.5">
                        {accounts.map((account) => (
                            ...
                            >
                                ...
                                <RadioButtonBase isSelected={account.id === selectedAccountId} className="tw:absolute tw:top-2 tw:right-2" />
                            </button>
                        ))}
                    </div>
                </div>
...
const NavAccountCardMenuItem = ({
    ...
    return (
        <button {...buttonProps} className={cx("tw:group/item tw:w-full tw:cursor-pointer tw:px-1.5 tw:focus:outline-hidden", buttonProps.className)}>
            <div
                className={cx(
            ...
            >
                ...
                {shortcut && (
                    <kbd className="tw:flex tw:rounded tw:px-1 tw:py-px tw:font-body tw:text-xs tw:font-medium tw:text-tertiary tw:ring-1 tw:ring-secondary tw:ring-inset">{shortcut}</kbd>
                )}
            </div>
        </button>
    );
...
export const NavAccountCard = ({
    ...
    return (
        <div ref={triggerRef} className="tw:relative tw:flex tw:items-center tw:gap-3 tw:rounded-xl tw:p-3 tw:ring-1 tw:ring-secondary tw:ring-inset">
            <AvatarLabelGroup
                size="md"
